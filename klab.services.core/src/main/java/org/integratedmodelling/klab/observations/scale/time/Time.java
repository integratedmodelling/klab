//package org.integratedmodelling.klab.observations.scale.time;
//
...
//import org.integratedmodelling.klab.api.lang.kim.impl.KimDate;
//import org.integratedmodelling.klab.api.lang.kim.impl.KimQuantity;
...
//
//public class Time extends Extent implements KTime {
//
...
//
//    /**
//     * Return the timepoints for all events starting and ending at the passed resolution in the time
//     * period we represent. If ordinal is true, return 0-based indices based on the next higher
//     * resolution (e.g. month or week number within a year, even if our own resolution is a decade);
//     * otherwise pass the actual start timestamps in milliseconds. If we don't span any of those
...
//            }
//
//            // public static Time create(ITime.Type type, Resolution.Type resolutionType, Double
...
//            return (this.locatedLinearOffset - n) >= 1 ? (Time) parentExtent.getExtent(this.locatedLinearOffset - n) : null;
//        }
//        return null;
//    }
//
...
//            throw new KlabIllegalArgumentException("time: state " + stateIndex + " requested when size == " + multiplicity);
//        }
//
//        if (size() == 1) {
...
//
//    /**
//     * Handles the surprisingly difficult issue of taking the previous time extent that is right
//     * before the passed one, without breaking the temporal contract.
//     * 
//     * @param extent
//     * @return
//     */
...
//
//        /*
//         * merge back the integer part in the offset to obtain a fractional part that is less than
//         * 1. The fraction, if != 0, cannot possibly use the non-regular durations so it will be
//         * adjusted to 30d or 365d for months and years.
//         */
//        if (leftover > 1) {
...
//
//            /*
//             * merge back the integer part in the offset to obtain a fractional part that is less
//             * than 1. The fraction, if != 0, cannot possibly use the non-regular durations so it
//             * will be adjusted to 30d or 365d for months and years.
//             */
//            if (leftover > 1) {
//                BigDecimal bigDecimal = new BigDecimal(leftover);
//                intStep += bigDecimal.longValue();
...
//
//            /*
//             * CHECK only intersecting if we're merging grid with grid. Otherwise if we get here we
//             * have accepted the passed extent as candidate, so we just let it have no effect on the
//             * coverage.
//             */
...
//
//    /**
//     * If this represents a year, return the year it represents; otherwise return the start
//     * millisecond.
//     * 
//     * @return
//     */
