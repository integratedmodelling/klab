...
//     * WeatherFactory.
//     * Below is the "legend" offered by CRU, although not all files are
//     * available online. Some others we ignore due to the impossibility of
...
//
//    /*
//     * vars commented out are advertised as provided but the high-resolution
//     * files are not there, or we decided to not use them:
//     * Weather.FROST_DAYS_IN_MONTH
//     * Weather.RELATIVE_HUMIDITY_PERCENT
//     * Weather.SUNSHINE_DURATION_TOTAL_MINUTES
//     * Weather.WIND_SPEED_M_SEC
//     */
//    public static String[]         cruVariables = {
//            Weather.PRECIPITATION_MM,
//            Weather.MIN_TEMPERATURE_C,
//            Weather.MAX_TEMPERATURE_C,
//            Weather.CLOUD_COVER_PERCENTAGE,
//            Weather.DIURNAL_TEMPERATURE_RANGE_C,
//            Weather.WET_DAYS_IN_PERIOD,
//            Weather.POTENTIAL_EVAPOTRANSPIRATION_MM
//    };
//
//    static {
//
...
//        if (!file.exists()) {
//            throw new KlabValidationException("CRU data for variable " + variable
//                    + " expected in non-existent CRU file " + filename);
//        }
//
//        try {
//
...
//                Array lastTime = vtime
//                        .read(new int[] { vtime.getDimension(0).getLength() - 1 }, new int[] { 1 });
//
...
//
//    /**
//     * Read a year of all vars, interpolating from monthly values. Return map of double[]
//     * with
//     * interpolated and corrected data.
//     * 
//     * @param ws
//     * @param variables
...
//         */
//        prdaily(startPeriod, monthlyData.get(Weather.PRECIPITATION_MM), precData, wetData, monthlyData
//                .get(Weather.WET_DAYS_IN_PERIOD), -1, false);
//
...
//         * do the rest
//         */
//        interp_monthly_means_conserve(startPeriod, monthlyData
//                .get(Weather.MIN_TEMPERATURE_C), tminData, -Double.MAX_VALUE, Double.MAX_VALUE);
//        interp_monthly_means_conserve(startPeriod, monthlyData
//                .get(Weather.MAX_TEMPERATURE_C), tmaxData, -Double.MAX_VALUE, Double.MAX_VALUE);
//
//        /*
...
//
//        interp_monthly_means_conserve(startPeriod, monthlyData
//                .get(Weather.CLOUD_COVER_PERCENTAGE), clouData, 0, 100);
//        interp_monthly_means_conserve(startPeriod, monthlyData
//                .get(Weather.DIURNAL_TEMPERATURE_RANGE_C), tranData, -Double.MAX_VALUE, Double.MAX_VALUE);
//        interp_monthly_totals_conserve(startPeriod, monthlyData
//                .get(Weather.POTENTIAL_EVAPOTRANSPIRATION_MM), petData, -Double.MAX_VALUE, Double.MAX_VALUE);
//
...
//
//    /**
//     * Create a CRU station at each land grid cell, which will cache their data on demand.
//     * 
//     * @throws Exception
//     */
...
//        WeatherStation ws = WeatherKbox.get().retrieve(LAST_STATION_ID);
//        if (ws != null) {
//            return;
//        }
//
...
//
//                    /*
//                     * assumptions, assumptions. We know CRU uses Double.MAX_VALUE for
//                     * NaNs,
//                     * so being this precipitation, this should be a fine NaN test.
//                     */
...
//
//    /**
//     * Distribution of monthly precipitation totals to quasi-daily values. From LPJ code,
//     * method in Dieter & Gerten 2002.
//     * 
//     * @param mval_prec
//     *            total rainfall (mm) for month
//     * @param dval_prec
//     *            actual rainfall (mm) for each day of year
//     * @param mval_wet
//     *            expected number of rain days for month
//     * @param seed
//     *            seed for generating random numbers. Pass -1 for non-repeatable.
//     * @param truncate
//     *            if set to true the function will set small daily values (< 0.1) to zero.
//     */
//    void prdaily(DateTime yeardate, double[] mval_prec, double[] dval_prec, double[] dval_wet, double[] mval_wet, long seed, boolean truncate) {
//
...
//     * data are on monthly basis. The generated daily values will have the same monthly
//     * means as the input. \param mvals The monthly means \param dvals The generated daily
//     * values
//     */
//    void interp_monthly_means_conserve(DateTime yeardate, double[] mvals, double[] dvals, double minimum, double maximum) {
//
...
//                throw new KlabException("interp_monthly_means_conserve: Invalid monthly value given " +
//                        mvals[m] + ", min = " + minimum + ", max = " + maximum);
//            }
//
//            interp_single_month(mvals[prev], mvals[m], mvals[next], date.dayOfMonth()
//                    .getMaximumValue(), dvals, start_of_month, minimum, maximum);
//
...
//     * May be called from input/output module to generate daily climate values when raw
//     * data are on monthly basis. The generated daily values will have the same monthly
//     * totals as the input. \param mvals The monthly totals \param dvals The generated
//     * daily values
//     */
//    void interp_monthly_totals_conserve(DateTime yeardate, double[] mvals, double[] dvals, double minimum, double maximum) {
...
//     * distributed over days with precipitation (or evenly over all days if there is no
//     * precipitation). \see distribute_ndep \param ndry Dry N deposition (monthly mean of
//     * daily deposition) \param nwet Wet N deposition (monthly mean of daily deposition)
//     * \param time_steps Number of days in the month \param dprec Array of precipitation
//     * values \param dndep Output, total N deposition for each day
//     */
//    void distribute_ndep_single_month(double ndry, double nwet, int time_steps, double[] dprec, double[] dndep, int offset) {
//
...
//
//    /**
//     * See distribute_ndep_single_month for details about how the distribution is done.
//     *
//     * @param mndry
//     *            Monthly means of daily dry N deposition
//     * @param mnwet
//     *            Monthly means of daily wet N deposition
//     * @param dprec
//     *            Daily precipitation data
//     * @param dndep
//     *            Output, total N deposition for each day
//     */
...
//        for (int m = 0; m < 12; m++) {
//            DateTime date = new DateTime(yeardate.getYear(), m + 1, 1, 0, 0);
//            distribute_ndep_single_month(mndry[m], mnwet[m], date.dayOfMonth()
//                    .getMaximumValue(), dprec, dndep, start_of_month);
//
//            start_of_month += date.dayOfMonth().getMaximumValue();
//        }
//    }
//
//    /**
//     * The generated daily values will conserve the monthly mean. The daily values are
//     * generated by first choosing values for the beginning, middle and end of the month,
//     * and interpolating linearly between them. The end points will be chosen by taking
//     * the surrounding months into account, and the mid point is then chosen so that we
//     * conserve the mean. Could be used for other interpolations than only monthly to
//     * daily, but comments assume monthly to daily to avoid being too abstract.
//     *
//     * @param preceding_mean
//     *            Mean value for preceding month
//     * @param this_mean
//     *            Mean value for the current month
//     * @param succeeding_mean
//     *            Mean value for the succeeding month
//     * @param time_steps
//     *            Number of days in the current month
//     * @param result
//     *            The generated daily values (array expected to hold at least time_steps
//     *            values)
//     * @param offset
//     *            offset into result
//     */
//    void interp_single_month(double preceding_mean, double this_mean, double succeeding_mean, int time_steps, double[] result, int offset, double minimum, double maximum) {
//
