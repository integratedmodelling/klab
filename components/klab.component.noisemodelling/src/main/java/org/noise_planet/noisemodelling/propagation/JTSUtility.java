...
public class JTSUtility {
    ...
    public static Coordinate getNearestPoint(Coordinate from, LineString to) {
        ...
        for(int i=0; i < coordinates.length - 1; i++) {
            ...
            double closestPointOnSegmentDistance = distance3D(closestPointOnSegment, from);
     ...
     */
    public static double distance3D(Coordinate a, Coordinate c) {
      double dx = a.x - c.x;
      double dy = a.y - c.y;
      double dz = a.z - c.z;
    ...
    public static List<Coordinate> getXAscendingHullPoints(Coordinate[] coordinates) {
        ...
        if(hull.length > 3 && CGAlgorithms.isCCW(hull)) {
            Coordinate temp;
