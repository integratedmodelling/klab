...
public class Probabilities {
	 ...
	 * @return
	 */
	protected Map<Landuses.Landuse, RasterData> createProbabilityMaps(int targetYear, RasterData previousLanduseMap,
			Map<Landuses.Landuse, Integer> demands, CLUEModel model) {
		...
		// prepare map stack
		RasterDataStack mapStack = RasterDataFactory.createStack(model);
		...
		for (CellStack inputCellStack : mapStack.getCellCursor()) {
			if (mapStack.containsInputWithNodata(inputCellStack)) {
				...
				Landuses.Landuse currentLanduse = landuses
						.findByValue(extractLanduseCellValue(inputCellStack.inputValues));
				...
				for (int index = 0; index < orderedLanduseList.size(); index++) {
					...
					double probability = landuseProbabilitiesOfCell.getProbability(currentLanduse, targetLanduse,
	...
	private void determineProbabilitiesForAllLandusesForCell(CellStack cellStack,
			LanduseProbabilities targetProbabilities, Map<Landuses.Landuse, Double> demandWeights) {
		...
		for (Landuses.Landuse landuse : targetProbabilities.getLanduses()) {
			...
			// ease of change is derived from landuse which has been set during creation and
			// is static for all cells of a given land use
	...
	private double getNeighbourhoodWeight(Landuse landuse) {
		...
		// change weight based on surrounding regions (not cells!) which are updated
		// during iterations (landuse changes)
	}

	/**
	 * ASSUMPTION: land use is the first- and age the second element in the cell
	 * stack remove first and second element
	 * 
	 * @param cellStack
	 * @return
	 */
	...
	/**
	 * ASSUMPTION: land use is the first element in the cell stack extract first
	 * element
	 * 
