...
public class RecreationIDBRuntimeEnvironment {
	...
	private GetRequest buildRequest(String URL, String input, String apiKey) {
		return Unirest.get(URL + input).header("accept", "application/json").header("apikey", apiKey);
	}
			...
			if (body == null) {
				...
				try {
					...
					throw new KlabException(
							"Cannot read the output from the Recreation Information Data Base response.", e);
				}
			}
	...
	public String recreationIDBSendRequest(String input, RecreationIDBRequestType requestType, String apiKey)
