...
public class StatsReport {
    ...
    class Filter {
        ...
        public String asSQLRestriction() {
            ...
            if (mustHave != null && !hasTarget(mustHave)) {
                throw new KlabIllegalArgumentException("cannot filter on " + target + " without reporting the same asset");
            }
    ...
    /*
     * costs are associated only with contexts, and their computation must see all assets.
     */
    ...
    /*
     * this holds the keys for the computed report. The sets are sorted after computation.
     */
    Set<AggregationSet> report = new HashSet<>();

    /*
     * As we accumulate each asset, the relative data are accumulated at each of the aggregation
     * levels stored in the aggregators list.
     */
    Map<Aggregator, Map<String, Data>> accountingData = new HashMap<>();

    /*
     * Aggregation sets are built for the levels of aggregation above assets to accumulate totals.
     */
    Map<AggregationSet, Data> aggregatedData = new HashMap<>();
    ...
    /*
     * we aggregate either by period or by target. We keep a list of these and use it to build the
     * report in the order of aggregation specified.
     */
    static class Aggregator {
        ...
        public boolean equals(Object obj) {
            ...
            return aggregationInterval == other.aggregationInterval && aggregationMultiplier == other.aggregationMultiplier
                    && target == other.target;
        }
    ...
    /**
     * Indexable aggregation set that serves as a key for the "chapters" in the report. Contains all
     * keys used to group the data and sorts intelligently based on the order of aggregation.
     * 
     * Asset types use the asset name as key, i.e. different uses of the same asset aggregate.
     * Contexts and queries use the observation ID as key so they only aggregate within the same
     * observation.
     * 
     * @author Ferd
     *
    ...
    class AggregationSet implements Comparable<AggregationSet> {
        ...
        // the following are not keys!
        String observation;
        String context_name;
        boolean sequential = true;
        boolean isDownloadQuery = false;
        int level = aggregators.size();
        ...
        public boolean equals(Object obj) {
            ...
            return end == other.end && Objects.equals(query_id, other.query_id) && Objects.equals(group, other.group)
                    ...
                    && start == other.start && Objects.equals(user, other.user) && Objects.equals(context_id, other.context_id)
        ...
        public void copyDataAt(Aggregator aggregator, AggregationSet target) {
            ...
            if (aggregator.aggregationInterval != null) {
                ...
                target.query_id = this.query_id;
            } else if (aggregator.target == Target.Observables) {
                target.observable = this.observable;
                ...
                target.context_id = this.context_id;
            }
        }
        ...
        public AggregationSet getParent(int level) {
            AggregationSet ret = new AggregationSet();
            for (int i = 0; i <= level; i++) {
                copyDataAt(aggregators.get(i), ret);
                ret.level = level;
            }
            return ret;
        }

        public List<AggregationSet> getParents() {
            List<AggregationSet> ret = new ArrayList<>();
            for (int level = 0; level < aggregators.size() - 1; level++) {
                ret.add(getParent(level));
            }
            return ret;
        }
    ...
    class Data {
        ...
        public Data(Aggregator aggregator) {
        ...
        /**
         * Each data bag will see all assets connected to a particular key, i.e. an asset bag will
         * see a single asset (with N passes), an observation bag will see all assets pertaining to
         * that observation, etc. If there is an asset aggregator, only assets of that type (and all
         * of them) will come through here, no matter the level of aggregation.
         * 
         * If we want to compute costs, we do that on a context base, and we need to see every
         * asset, so it should be forbidden to use an asset classifier. If assets for a given
         * context have individual "pricing", only those prices should be computed; otherwise use
         * the reference database and compute the k.LAB credits based on the averages.
         * 
         * @param result
         * @param set
         */
        ...
        public String toString() {
            ...
            if (aggregator.target != null) {
                if (aggregator.target == Target.Contexts) {
                    ...
                    return "[" + count + " assets; size = " + NumberFormat.getInstance().format(contextSize) + "; complexity = "
                            + NumberFormat.getInstance().format(complexity) + "; total time = "
        ...
        /*
         * Accumulate summary statistics based on the aggregation set passed, which is at a level
         * higher than the maximum disaggregation in the report. Basically we sum up the
         * observations time, the context size and the downloaded bytes, ensuring that each context
         * and each query only gets in once, for anything above a context in level.
         */
        public void accumulateTotals(Parameters<String> result, AggregationSet parent) {

            Aggregator alevel = aggregators.get(parent.level);
            if (alevel.aggregationInterval != null) {
                accumulateTotals(result);
            } else if (alevel.target != null) {
                switch(alevel.target) {
                case Applications:
                case Contexts:
                case Engines:
                case Groups:
        ...
        private void accumulateTotals(Parameters<String> result) {
            if (!this.observations.contains(result.get("query_id", String.class))) {
                this.observations.add(result.get("query_id", String.class));
        ...
        public boolean isTotalizing() {
            return this.aggregator.aggregationInterval != null || this.aggregator.target == Target.Applications
                    || this.aggregator.target == Target.Engines || this.aggregator.target == Target.Users;
        }
    ...
    public boolean compute() {
        ...
        /**
         * if start and/or end isn't specified, retrieve it from the database and set it.
         */
        ...
        if (st.get() < 0 || en.get() < 0) {
            db.scan("SELECT MIN(contexts.created) as min, MAX(contexts.created) as max from contexts;", (rs) -> {
                if (st.get() < 0 && rs.get("min") != null) {
                    st.set(rs.get("min", Long.class));
                }
                if (en.get() < 0 && rs.get("max") != null) {
    ...
    private String getQuery(long start, long end) {
        ...
        if (assetType != null && !computeCosts /*
                                                * TODO && costs are not computed and aggregators do
    ...
    public String compile() {
        ...
        for (AggregationSet classifier : classifiers) {
            List<AggregationSet> parents = classifier.getParents();
            for (int level = 0; level < aggregators.size() - 1; level++) {
                AggregationSet parentSet = parents.get(level);
                Data aData = aggregatedData.get(parentSet);
    ...
    private void accumulateData(Parameters<String> result, AggregationSet set) {
        ...
        for (Aggregator aggregator : aggregators) {
            ...
            for (AggregationSet parent : set.getParents()) {
                Data abag = aggregatedData.get(parent);
                if (abag == null) {
                    abag = new Data(aggregators.get(parent.level));
                    aggregatedData.put(parent, abag);
                }

                abag.accumulateTotals(result, parent);
            }
        }
    }
    ...
    /**
     * Parse a span specification for the report. Used also in other places, so static and public.
     * 
     * @param span
     * @return
     */
