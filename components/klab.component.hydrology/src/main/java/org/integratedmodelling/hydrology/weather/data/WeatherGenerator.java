...
//                    "cannot find enough data to simulate weather in station " + station);
//        }
//
...
//
//        // calulate a0, a1, a2 regression coefficients for wet & dry, min & max temperature standard
//        // deviations
...
//            wet = false;
//        } else {
//            throw new KlabRuntimeException("ERROR:  cannot end with seriesLength=0 (see ReadAgain()).");
//        }
//        return new Triple<Double, Integer, Boolean>((double) sumRain / (double) kount, length, wet);
//    }
//
//    private Triple<Integer, Double, Double> readData(WeatherStation station, double finalMin,
//            double finalMax, boolean b) throws KlabException {
//
...
//            if (holdRain < 0.0) {
//                holdRain = 0.0;
//                // throw new ThinklabRuntimeException("Error: negative precipitation reported.");
//            }
//            finalMin = holdMin;
//            finalMax = holdMax;
//
//            // calculate maximum values to be used as upper bounds in processing phase for the semi-empirical
//            // distributions
//            if (holdRain > EPS) {
//                constructRegMatrix(wetRegMatrix, xTwetMinTemp, xTwetMaxTemp, dayOfYear, holdMin, holdMax);
//                consWetDays++;
//                // if the number of consecutive dry days is nonzero, then it must be the start of a new
//                // series. Therefore, change the month of origin to the current month and set consDryDays to
//                // 0.
//                if (consDryDays > 0) {
//                    monthOrigin = getMonth(dayOfYear);
//                    consDryDays = 0;
//                }
//                // store in the maxConsWetDays array for a given month if the series is the longest instance
//                // ever in that month.
...
//                consDryDays++;
//                // if the number of consecutive wet days is nonzero, then it must be the start of a new
//                // series. Therefore, change the month of origin to the current month and set consWetDays to
//                // 0.
//                if (consWetDays > 0) {
//                    monthOrigin = getMonth(dayOfYear);
//                    consWetDays = 0;
...
//            }
//
//            // if rain amount is on this day is greater than the most rain on this month, store in rainMax
//            // array
...
//        // newBeginning = true: start generating weather on Jan. 1, deriving temeperature and series length
//        // data from the last year of input data.
//        // newBeginning = false: start generating weather on Dec. 1 of the previous year to introduce
//        // randomness into the model. In other words, calculate any year, not just the year after the input
//        // data
//        if (randomStart) {
...
//                    maxTempStdDev = newMaxVar
//                            - createV_Cinv(VCinv, newMinStdev, newMaxStdev, m_minPrevStdDev, m_maxPrevStdDev,
//                                    m_rhoDaily, m_rhoMin, m_rhoMax, m_rhoMinMax, m_rhoMaxMin);
...
//    }
//
//    private void constructRegMatrix(double[][] regMatrix, double[] xTymin, double[] xTymax, int day,
//            double ymin, double ymax)
//    // constructs regression matrices X'*Y and X'*X for arrays declared in the main program. ' denotes a
//    // transposed matrix.
//    // it is an unchecked runtime error for regMatrix, xTymin, and xTymax to be smaller than 3x3, 3x1, and 3x1
...
//        regMatrix[1][1] += Math.cos(2.0 * Math.PI * day / 365.0)
//                * Math.cos(2.0 * Math.PI * day / 365.0);
//        regMatrix[1][2] += Math.cos(2.0 * Math.PI * day / 365.0)
//                * Math.sin(2.0 * Math.PI * day / 365.0);
//        regMatrix[1][3] += Math.cos(2.0 * Math.PI * day / 365.0)
//                * Math.cos(2 * 2.0 * Math.PI * day / 365.0);
//        regMatrix[1][4] += Math.cos(2.0 * Math.PI * day / 365.0)
//                * Math.sin(2 * 2.0 * Math.PI * day / 365.0);
//        regMatrix[1][5] += Math.cos(2.0 * Math.PI * day / 365.0)
//                * Math.cos(3 * 2.0 * Math.PI * day / 365.0);
//        regMatrix[1][6] += Math.cos(2.0 * Math.PI * day / 365.0)
//                * Math.sin(3 * 2.0 * Math.PI * day / 365.0);
//        regMatrix[2][2] += Math.sin(2.0 * Math.PI * day / 365.0)
//                * Math.sin(2.0 * Math.PI * day / 365.0);
//        regMatrix[2][3] += Math.sin(2.0 * Math.PI * day / 365.0)
//                * Math.cos(2 * 2.0 * Math.PI * day / 365.0);
//        regMatrix[2][4] += Math.sin(2.0 * Math.PI * day / 365.0)
//                * Math.sin(2 * 2.0 * Math.PI * day / 365.0);
//        regMatrix[2][5] += Math.sin(2.0 * Math.PI * day / 365.0)
//                * Math.cos(3 * 2.0 * Math.PI * day / 365.0);
//        regMatrix[2][6] += Math.sin(2.0 * Math.PI * day / 365.0)
//                * Math.sin(3 * 2.0 * Math.PI * day / 365.0);
//        regMatrix[3][3] += Math.cos(2 * 2.0 * Math.PI * day / 365.0)
//                * Math.cos(2 * 2.0 * Math.PI * day / 365.0);
//        regMatrix[3][4] += Math.cos(2 * 2.0 * Math.PI * day / 365.0)
//                * Math.sin(2 * 2.0 * Math.PI * day / 365.0);
//        regMatrix[3][5] += Math.cos(2 * 2.0 * Math.PI * day / 365.0)
//                * Math.cos(3 * 2.0 * Math.PI * day / 365.0);
//        regMatrix[3][6] += Math.cos(2 * 2.0 * Math.PI * day / 365.0)
//                * Math.sin(3 * 2.0 * Math.PI * day / 365.0);
//        regMatrix[4][4] += Math.sin(2 * 2.0 * Math.PI * day / 365.0)
//                * Math.sin(2 * 2.0 * Math.PI * day / 365.0);
//        regMatrix[4][5] += Math.sin(2 * 2.0 * Math.PI * day / 365.0)
//                * Math.cos(3 * 2.0 * Math.PI * day / 365.0);
//        regMatrix[4][6] += Math.sin(2 * 2.0 * Math.PI * day / 365.0)
//                * Math.sin(3 * 2.0 * Math.PI * day / 365.0);
//        regMatrix[5][5] += Math.cos(3 * 2.0 * Math.PI * day / 365.0)
//                * Math.cos(3 * 2.0 * Math.PI * day / 365.0);
//        regMatrix[5][6] += Math.cos(3 * 2.0 * Math.PI * day / 365.0)
//                * Math.sin(3 * 2.0 * Math.PI * day / 365.0);
//        regMatrix[6][6] += Math.sin(3 * 2.0 * Math.PI * day / 365.0)
//                * Math.sin(3 * 2.0 * Math.PI * day / 365.0);
//        xTymin[0] += ymin;
...
//                rainBounds[i][j] = rainBounds[i][j - 1] + (0.5 + 0.25 * ((j - 1) / 2)) * rainMax[i]
//                        / NUMINTERVALS;
//        }
//    }
//
...
//    // each bound is slightly larger than the preceeding bound, typically one or two units wider than the
//    // preceeding boumd
//    {
...
//            }
//        }
//        // for each entry, add 1) the previous entry, 2) the base difference of 1, 3) an adjustment based on
//        // any iterations that may have occured, 4) an adjustment based on final adjustment
...
//    // calculates a0, a1, and a2 parameters for the regression given regMatrix (X'*X) and a given X'*Y.
...
//    // calculates normalized residuals of temperatures for every day. takes in flag boolean that indicates
...
//                + parameters[2] * Math.sin(2.0 * Math.PI * index / 365.0) + parameters[3]
//                * Math.cos(2 * 2.0 * Math.PI * index / 365.0) + parameters[4]
//                * Math.sin(2 * 2.0 * Math.PI * index / 365.0) + parameters[5]
//                * Math.cos(3 * 2.0 * Math.PI * index / 365.0) + parameters[6]
//                * Math.sin(3 * 2.0 * Math.PI * index / 365.0);
//    }
//
...
//    }
//
//    private double createV_Cinv(double VC[], double newMinStd, double newMaxStd, double minPrevStd,
//            double maxPrevStd, double rhoDaily, double rhoMin, double rhoMax, double rhoMinMx, double rhoMxMn)
...
//
//            if (big <= ZERO) {
//                throw new KlabRuntimeException("singularity or near-singularity in matrix inversion");
//            }
//
//            indxr[k] = jrow;
//            if (jrow > k)
...
//                "Error: data member out of bounds in determining paramters for temperatures--");
//        // return 9;
//    }
//
...
//                return i - 1;
//        throw new KlabRuntimeException(
//                "Error: data member out of bounds during parameter setting for temperatures.");
//        // return 9;
//    }
//
//}
