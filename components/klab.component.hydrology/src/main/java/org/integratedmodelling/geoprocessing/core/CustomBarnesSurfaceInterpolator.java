...
public class CustomBarnesSurfaceInterpolator extends BarnesSurfaceInterpolator {
    ...
    private Ellipsoid Elli;

    private Coordinate[] inputObs;
    
	private GeodeticCalculator gcalc = null;
     ...
     */
    public CustomBarnesSurfaceInterpolator(Coordinate[] observationData) {
    	super(observationData);
        this.inputObs = observationData;
    }

    private GeodeticCalculator getGeodeticCalculator() {
    	if (gcalc == null) {
    		gcalc = new GeodeticCalculator(Elli);
    	}
    	return gcalc;
    }
    ...
    // Set the crs to compute distances
    public void setEllipsoid(Ellipsoid elli) {
        this.Elli = elli;
    }
    ...
    //When computing goedetic distance, the x,y coordinates need to be reversed due to a bug in the interpolator function
    private double distance(double x, double y, Coordinate p) {
    	
    	getGeodeticCalculator().setStartingGeographicPoint(y, x);
    	getGeodeticCalculator().setDestinationGeographicPoint(p.y, p.x);
        return getGeodeticCalculator().getOrthodromicDistance();
    ...
    private double weight(
    	...
    	getGeodeticCalculator().setStartingGeographicPoint(dataPt.y, dataPt.x);
    	getGeodeticCalculator().setDestinationGeographicPoint(gridPt.y, gridPt.x); 
        double dist = getGeodeticCalculator().getOrthodromicDistance();
