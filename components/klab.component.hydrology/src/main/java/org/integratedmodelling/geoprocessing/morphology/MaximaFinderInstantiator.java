...
public class MaximaFinderInstantiator extends AbstractContextualizer implements IInstantiator, IExpression {
    ...
    public List<IObjectArtifact> instantiate(IObservable semantics, IContextualizationScope context) throws KlabException {
        ...
        if (chmId != null) {
            ...
            if (state == null) {
                throw new IllegalArgumentException("maxima extractor: no input state named '" + chmId + "' found in context");
            }
            algorithm.inDsmDtmDiff = GeotoolsUtils.INSTANCE.stateToCoverage(state, context.getScale(), DataBuffer.TYPE_FLOAT,
                    floatNovalue, false);
            ...
            if (state == null) {
                throw new IllegalArgumentException("maxima extractor: no input state named '" + surfaceId + "' found in context");
            }
            StateSummary summary = Observations.INSTANCE.getStateSummary(state, context.getScale());
            algorithm.inDsmDtmDiff = GeotoolsUtils.INSTANCE.stateToCoverage(state, context.getScale(), DataBuffer.TYPE_FLOAT,
                    floatNovalue, false, (value) -> {
                        if (value instanceof Number && !Double.isNaN(((Number) value).doubleValue())) {
                            ...
                            // that it makes the statement present in thresholdExpression not
                            // corrently applied.
                            // I saw they use for example: threshold = [(max - min) < 500 ? 10000 :
                            // max * 0.65]
                            // max - min are related to the original raster, while the raster will
                            // be
            ...
            if (relativeThreshold > 0) {
                ...
                if (thresholdExpression != null) {
                    ...
                    Object o = threx.eval(context, "min", summary.getRange().get(0), "max", summary.getRange().get(1), "mean",
                            summary.getMean(), "std", summary.getStandardDeviation(), "target", state);

                    if (!(o instanceof Number)) {
                        throw new IllegalStateException("maxima extractor: threshold expression does not evaluate to a number");
                    }
            ...
            throw new IllegalArgumentException("maxima extractor: no input state provided. Give either 'chm' or 'surface'.");
        }
        ...
        algorithm.doCircular = circular; // TODO I changed this to be circular by default, I think
                                         // it should work better for mountain peeks.
        ...
        while(it.hasNext()) {
            ...
            if (feature.getDefaultGeometry() instanceof org.locationtech.jts.geom.Geometry) {
                IScale instanceScale = Scale.substituteExtent(context.getScale(),
                        Shape.create((org.locationtech.jts.geom.Geometry) feature.getDefaultGeometry(), grid.getProjection()));
                ret.add(context.newObservation(semantics, Observables.INSTANCE.getDisplayName(semantics) + "_" + (i++),
