...
/**
 * A stand corresponds to a modelled locality. Each GridCell (ERU) contains a
 * list of stands.
 * 
 * Member variables include an object of type Climate (holding climate,
 * insolation and CO2 data), a object of type Soiltype (holding soil static
 * parameters) and a list of Patch objects. Soil objects (holding soil state
 * variables) are associated with patches, not stands.
 */
...
public class Stand implements IModelObject {
	...
	// true if current crop management hydrology == irrigated, updated during
	// rotation
	public Boolean isirrigated;

	/// true if the stand's main crop pft intercrop==naturalgrass and a pft with
	/// isintercrop==true is in the
	/// pftlist.
	...
	/// used during land cover change involving several calls to
	/// reveiving_stand_change()
	/**
	 * Set to frac_old in reduce_stands(), then modified in donor_stand_change() and
	 * receiving_stand_change().
	 */
	...
	/**
	 * The reason why Stand has its own seed, rather than using for instance a
	 * single global seed is to make it easier to compare results when using
	 * different land cover types.
	 *
	 * Randomness not associated with a specific stand, but rather a whole grid cell
	 * should instead use the seed in the Gridcell class.
	 *
	 * \see randfrac()
	 */
	public long seed;
	...
	/// type of landcover
	/**
	 * \see landcovertype initialised in constructor
	 */
	public LandcoverType landcover;

	/// The year when this stand was created.
	/**
	 * Will typically be year zero unless running with dynamic land cover.
	 *
	...
	/// scaling factor for stands that have grown in area this year (old
	...
	public Stand(Gridcell gridcell, IConfiguration _configuration, List<PFT> pftlist, Soiltype soiltype,
		...
		if (landcover == LandcoverType.CROP || landcover == LandcoverType.URBAN || landcover == LandcoverType.PEATLAND
				|| landcover == LandcoverType.BARREN) {
			npatchL = 1;
	...
	public void init_stand_lu(StandType st, double fraction) {
		...
		for (PFT pftx : pftlist) {
			...
			if (!st.restrictpfts && pftx.landcover == lc
					...
					|| st.naturalgrass && pftx.landcover == LandcoverType.NATURAL && pftx.lifeform == LifeForm.GRASS) {
		...
		/*
		 * FIXME this should just be an exception, but let's allow it for now, reverting
		 * to natural.
		 */
					...
					if (rot == 0) {
						for (Patch p : patches) {
							...
							if (pftlist.get(id).phenology == Phenology.ANY)
								...
								ppft.get_cropphen().eicdate = Utils.stepfromdate(ppft.get_cropphen().sdate, -15);
							}
						}
	...
	void rotate() {
		...
		if (pftid >= 0 && stid >= 0) {
			...
			if (_configuration.getSTs().get(stid).management.get(current_rot).hydrology == HydrologyType.IRRIGATED) {
				isirrigated = true;
	...
	public void doCropRotation() {
		...
		if (landcover == LandcoverType.CROP) {
			...
			if (rotation.ncrops > 1 && isrotationday) {
				...
				/*
				 * // 1. Before firstrotyear, grow only crop1: if(date.year < firstrotyear)
				...
				if (!postpone_rotation) {
					...
					if (rotation.multicrop && rotation.ncrops == 2 && current_rot == 1) {
						...
						if (pft.get(pftid).hdate_force < 0) {
							pft.get(pftid).hdate_force = Utils.stepfromdate(pft.get(old_pftid).sdate_force, -10);
						}
					}
