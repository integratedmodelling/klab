...
/**
 * All-in-one package that will return the mean sea level CO2 concentration at
 * any time since 1980, interpolating NOAA published monthly data with a cubic
 * spline.
 * 
 * Data from http://www.esrl.noaa.gov/gmd/ccgg/trends/global.html#global_data.
 * The static file is copied to the classpath and not downloaded. Remember to
 * update the file in main/resources/data with new observations every now and
 * then.
 * 
 * TODO add scenarios and the ability of moving beyond current. TODO take file
 * from network and resort to data in classpath if retrieval fails. File is at
...
public class CO2Concentration {
	...
	/**
	 * Get the CO2 concentration in ppm at the passed time. The time should be in
	 * milliseconds (since epoch) and start at or after Jan 1st, 1980. See notice
	 * above for source.
	 * 
	 * 
	 * @param time
