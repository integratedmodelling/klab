...
import org.integratedmodelling.ml.legacy.riskwiz.learning.data.Option;
import org.integratedmodelling.ml.legacy.riskwiz.learning.data.OptionHandler;
...
public class DatabaseLoader extends AbstractLoader 
    ...
    private String limitQuery(String query, int offset, int choice) {
        ...
        if (m_orderBy.size() != 0) {
            ...
            if (m_DataBaseConnection.getUpperCase()) {
                order.append(
                        ((String) m_orderBy.elementAt(m_orderBy.size() - 1)).toUpperCase());
            } else {
        ...
        if (choice == 0) {
            limitedQuery = query.replaceFirst("SELECT",
                    "SELECT LIMIT " + offset + " 1");
            limitedQuery = limitedQuery.concat(orderByString);
            return limitedQuery;
        }
        if (choice == 1) {
            limitedQuery = query.concat(
                    orderByString + " LIMIT 1 OFFSET " + offset);
            return limitedQuery;
        }
        ...
        for (int i = 1; i <= numAttributes; i++) {
            ...
            case DatabaseUtils.STRING:
                ...
                if (rs.wasNull()) {
                    ...
                    if (index == null) {
                        index = new Double(
                                m_structure.attribute(i - 1).addStringValue(str));
                    }
                    vals[i - 1] = index.doubleValue();
                }
                break;
