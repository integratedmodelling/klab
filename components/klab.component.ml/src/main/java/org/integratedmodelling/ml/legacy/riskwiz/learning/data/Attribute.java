...
public class Attribute
        ...
        if (attributeValues == null) {
            ...
            for (int i = 0; i < attributeValues.size(); i++) {
                ...
                if (((String) store).length() > STRING_COMPRESS_THRESHOLD) {
                    try {
                        store = new SerializedObject(
                                attributeValues.elementAt(i), true);
                    } catch (Exception ex) {
                        System.err.println(
                ...
                if (m_Hashtable.containsKey(store)) {
                    throw new IllegalArgumentException(
                            "A nominal attribute (" + attributeName + ") cannot"
    ...
    final void delete(int index) {
        ...
        if (!isNominal() && !isString() && !isRelationValued()) { 
            throw new IllegalArgumentException(
                    "Can only remove value of " + "nominal, string or relation-"
                    + " valued attribute!");
        } else {
            m_Values = (FastVector) m_Values.copy();
    ...
    private void setMetadata(ProtectedProperties metadata) {
        ...
        if (m_HasZeropoint && !m_IsRegular) {
            throw new IllegalArgumentException(
                    "A zeropoint attribute must be" + " regular");
        }
        if (m_IsRegular && m_Ordering == ORDERING_SYMBOLIC) {
            throw new IllegalArgumentException(
                    "A symbolic attribute cannot be" + " regular");
        }
        ...
        if (m_HasZeropoint && m_Ordering != ORDERING_ORDERED) {
            throw new IllegalArgumentException(
                    "A zeropoint attribute must be" + " ordered");
        }
    ...
    private void setNumericRange(String rangeString) {
        ...
        if (m_UpperBound < m_LowerBound) {
            throw new IllegalArgumentException(
                    "Upper bound (" + m_UpperBound + ") on numeric range is"
    ...
    public static void main(String[] ops) {
        ...
        try {
            ...
            System.out.println(
                    "\"position\" is numeric: " + position.isNumeric());
            System.out.println(
                    "\"position\" is nominal: " + position.isNominal());
            ...
            System.out.println(
                    "Number of values for \"position\": " + position.numValues());
