...
public final class Utils {
    ...
    public static Properties readProperties(String resourceName)
        ...
        try {
            ...
            /* throw new Exception("Problem reading default properties: "
             + ex.getMessage()); */
            System.err.println(
                    "Warning, unable to load properties file from "
        ...
        if (propFile.exists()) {
            try {
                ...
                throw new Exception(
                        "Problem reading user properties: " + propFile);
            }
        }
        ...
        if (propFile.exists()) {
            try {
                ...
                throw new Exception(
                        "Problem reading local properties: " + propFile);
            }
        }
        ...
        if (i > -1) {
            if (options[i].equals("-" + flag)) {
                if (i + 1 == options.length) {
                    throw new Exception(
                            "No value given for -" + flag + " option.");
                }
                options[i] = "";
    ...
    public static Object forName(Class classType,
        ...
        if (!classType.isAssignableFrom(c)) {
            throw new Exception(
                    classType.getName() + " is not assignable from " + className);
        }
    ...
    public static /* @pure@*/int[] stableSort(double[] array) {
        ...
        while (i < index.length) {
            numEqual = 1;
            for (int j = i + 1; ((j < index.length)
                    && Utils.eq(array[index[i]], array[index[j]])); j++) {
                numEqual++;
            }
            if (numEqual > 1) {
                helpIndex = new int[numEqual];
    ...
    private static int select(/* @non_null@*/double[] array, /* @non_null@*/int[] index, 
        ...
        if (left == right) {
            ...
            if ((middle - left + 1) >= k) {
                ...
                return select(array, index, middle + 1, right,
                        k - (middle - left + 1));
            }
        }
    }
    ...
    public static void main(String[] ops) {
        ...
        try {
            ...
            System.out.println(
                    "Get position of flag -f: " + Utils.getOptionPos('f', ops));
            System.out.println("Get flag -f: " + Utils.getFlag('f', ops));
            System.out.println(
                    "Get position of option -o: " + Utils.getOptionPos('o', ops));
            ...
            System.out.println(
                    "-6.78: " + Utils.doubleToString(-6.78214234, 6, 2));
            ...
            System.out.println(
                    "5.55555 rounded to 2 decimal places: "
                            + Utils.roundDouble(5.55555, 2));
      
            // Arrays
            System.out.println(
                    "Array-Dimensions of 'new int[][]': "
                            + Utils.getArrayDimensions(new int[][] {}));
            System.out.println(
                    "Array-Dimensions of 'new int[][]{{1,2,3},{4,5,6}}': "
                            + Utils.getArrayDimensions(
                                    new int[][] { { 1, 2, 3}, { 4, 5, 6}}));
            String[][][] s = new String[3][4][];

            System.out.println(
                    "Array-Dimensions of 'new String[3][4][]': "
