...
public class MCAComponent {
     ...
     */
    public static IObservable getCriterionValueObservable(IObservable criterionObservable, @Nullable IObservable targetObservable) {

        IObservable.Builder builder = ((Observable) criterionObservable).getBuilder()
                .as(UnarySemanticOperator.VALUE);
        if (targetObservable != null) {
            builder = builder.withGoal(((Observable) targetObservable).getBuilder()
    ...
    public static List<IAlternative> rank(List<IAlternative> alternatives, Collection<ICriterion> criteria, IStakeholder observer, Method method) {
     ...
     * @return
     */
    public static boolean extractAssessment(IArtifact target, List<IAlternative> alternatives, Collection<ICriterion> criteria, List<IStakeholder> observers, Method method, IComputationContext context) {
        ...
        for (int i = 1; i < context.getModel().getObservables().size(); i++) {
            CriterionDescriptor descriptor = getCriterionAnnotation(context.getModel().getObservables()
                    .get(i), true, false);
            if (descriptor != null) {
                ...
                for (IArtifact alternative : alternativesArtifact) {
                    if (alternative instanceof IDirectObservation) {
                        for (IState state : ((IDirectObservation) alternative).getStates()) {
                            CriterionDescriptor crit = getCriterionAnnotation(state
                        ...
                        context.getMonitor()
                                .error("MCA: stated alternatives must be direct observations. Exiting.");
                        return false;
                    }
                }
            } else {
                context.getMonitor()
                        .error("MCA: no criteria annotated in model or in alternatives. Exiting.");
                return false;
            }
        }
        ...
        /*
         * build all descriptors: if 1+ criterion annotations have weights, don't try to observe the criteria in 
         * stakeholders.
         */
        ...
        for (CriterionDescriptor c : criterionDescriptors) {
            ...
            if (criteriaIds.contains(c.getName())) {
                context.getMonitor()
                        .error("MCA: criterion " + c.getName() + " has multiple definitions");
                return false;
            }

            Criterion criterion = new Criterion(c.observable, c.annotation.getName().equals("benefit")
                    ? Type.BENEFIT
                    : Type.COST);

            if (!c.inAlternative && !c.input) {
                IArtifact state = context.getArtifact(c.getName());
                if (state == null || !(state instanceof IState)
                        || !c.observable.is(IKimConcept.Type.QUANTIFIABLE)
                        || !c.observable.is(IKimConcept.Type.ORDERING)) {
                    context.getMonitor()
                            .error("MCA: criterion " + c.getName()
                                    + " is not a numeric or ranked observation");
                    return false;
                }
            }
            ...
            if (c.annotation.get(IServiceCall.DEFAULT_PARAMETER_NAME) != null) {
                ...
                if (value instanceof Number) {
                    ...
                    for (Object o : ((Map<?, ?>) value).keySet()) {
                        if (o instanceof IConcept && ((Map<?, ?>) value).get(o) instanceof Number) {
                            ...
                            if (c.weights.containsKey(o)) {
                                context.getMonitor()
                                        .error("MCA: multiply defined criterion weight in " + c.getName());
                                return false;
                            }
                            c.weights.put((IConcept) o, ((Number) ((Map<?, ?>) value).get(o)).doubleValue());
                        } else {
                            context.getMonitor()
                                    .error("MCA: misused criterion weight map for " + c.getName()
                    ...
                    context.getMonitor()
                            .error("MCA: misused criterion annotation for " + c.getName()
        ...
        if (globalMultipleWeights && globalSingleWeights) {
            context.getMonitor()
                    .error("MCA: misused criterion annotations: only maps or single weights must be specified within an assessment");
            return false;
        }

        /*
         * if stakeholders are defined through annotations, build them and configure them
         */
        if (stakeholdersArtifact != null) {
            if (globalSingleWeights || globalMultipleWeights) {
                context.getMonitor()
                        .error("MCA: cannot define stakeholder artifacts when implicit weights are used in criteria annotations. Exiting.");
                return false;
            }
            ...
            for (IArtifact stakeholder : stakeholdersArtifact) {
                if (stakeholder instanceof IDirectObservation) {
                    ...
                    context.getMonitor()
                            .error("MCA: stated stakeholders must be direct observations. Exiting.");
                    return false;
                }
            }
            ...
            // define stakeholders based on each criterion's storage; warn about missing weights or
            // uneven specs. Stakeholder are marked as not present in context so that they can be
            // created.
            for (IConcept sh : stakeholderStatedObservables) {
                ...
                for (CriterionDescriptor c : criterionDescriptors) {
                    if (c.weights.containsKey(sh)) {
                        ...
                        context.getMonitor().warn("MCA: inconsistent weight specifications: some stakeholder types are not represented in all criteria");
            ...
            // define a single objective stakeholder based on each criterion's global weight; warn about
            // missing weights or
            // uneven specs
        ...
        /*
         * go for the alternatives. Each should have an observation of each criterion. If not, 
         * we should have a configurable strategy, but for now assume it's unrankable and 
         * warn.
         */
        ...
        /*
         * Validate everthing: criteria must be represented in stakeholders, alternatives should have all criteria; fill gaps
         * with warnings.
         */
    ...
    private static CriterionDescriptor getCriterionAnnotation(IObservable observable, boolean isInput, boolean isInAlternative) {
