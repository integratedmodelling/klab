...
public abstract class TableInterpreter implements ITableInterpreter {
	...
	public void categorize(IResource resource, IParameters<String> parameters, IMonitor monitor) {
		...
		File codelistFile = new File(((Resource) resource).getPath() + File.separator + "code_"
				+ parameters.get("categorization") + ".json");

		if (file.exists() && !codelistFile.exists()) {
			upgradeCategorization(resource, parameters, monitor);
		} else if (!file.exists() && !codelistFile.exists()) {
	...
	public void upgradeCategorization(IResource resource, IParameters<String> parameters, IMonitor monitor) {
		// TODO Auto-generated method stub
		ITable<?> table = this.getTable(resource, null, monitor);
		Set<Object> categories = new LinkedHashSet<>();
		for (Object o : table.asList(parameters.get("dimension"))) {
	...
	public IGeometry recomputeGeometry(IResource resource, Map<String, String> parameters, IMonitor monitor) {
		...
		if (parameters.containsKey("space.encoding") && !parameters.get("space.encoding").trim().isEmpty()) {
			...
			if (strategy == null) {
				...
				if (parameters.get("space.encoding").contains("->")) {
					...
					ICodelist codelist = table.getCodelist(parameters.get("space.encoding"));
					strategy = new DimensionScanner<IShape>(resource, new String[] { parameters.get("space.encoding") },
							IShape.class);
					if (codelist != null) {
						if (codelist.isAuthority()) {
							spaceEncoding = "\u03c31(" + codelist.size() + "){authority=" + codelist.getAuthorityId()
									+ "}";
						} else if (codelist.getRootConceptId() != null) {
							if (codelist.isAuthority()) {
								spaceEncoding = "S1(" + codelist.size() + "){baseidentity=" + codelist.getRootConceptId()
										+ "}";
							}
						} else {
							throw new KlabIllegalStateException(
									"cannot use attribute " + parameters.get("space.encoding")
											+ " as spatial: codelist has no authority or base concept associated");
						}
					} else {
						throw new KlabIllegalStateException("cannot use attribute " + parameters.get("space.encoding")
								+ " as spatial extent: attribute has no codelist mapping");
					}
