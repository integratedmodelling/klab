...
public class DimensionScanner<T> {
	...
	/*
	 * for temporal indexing to specific columns or rows. For now this is only used
	 * when one column refers to one year.
	 */
	...
	public DimensionScanner<T> contextualize(ITable<?> table, T extent, IContextualizationScope scope) {
		...
		if (ITime.class.isAssignableFrom(this.extent) && extent instanceof ITime) {
			...
			if (auxiliaryResource != null && spatialContextualizer == null) {
				...
				VisitingDataBuilder builder = new VisitingDataBuilder().keepStates(scope.getScale()).startState("space",
						null, scope);
			...
			if (time != null && this.temporalDimensions != null) {
				...
				if (index == null) {
					...
					if (this.indices.isEmpty() && !mappings.isEmpty()) {
						...
						if (this.columnName.contains(",")) {
							/*
							 * Two columns, a "between" condition of the time tracker for two index columns
							 */
						...
						return ((AbstractTable<?>) table).newFilter(
								...
								new Object[] { (this.dimension == Dimension.COLUMN ? this.columnName : this.rowName),
										filtered });
						...
						return ((AbstractTable<?>) table)
								.newFilter(this.dimension == Dimension.COLUMN ? Filter.Type.INCLUDE_COLUMNS
