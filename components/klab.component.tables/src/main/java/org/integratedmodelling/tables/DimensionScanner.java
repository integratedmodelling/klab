...
public class DimensionScanner<T> {
        ...
        for (int i = 1; i < definition.length; i++) {
            if (Urns.INSTANCE.isUrn(definition[i])) {
                if (this.auxiliaryResource != null) {
                    throw new KlabValidationException("cannot have more than one auxiliary resource in a table encoding");
                }
    ...
    public DimensionScanner<T> contextualize(ITable<?> table, T extent, IContextualizationScope scope) {
        ...
        if (ITime.class.isAssignableFrom(this.extent) && extent instanceof ITime) {
            ...
            if (auxiliaryResource != null && spatialContextualizer == null) {
                ...
                VisitingDataBuilder builder = new VisitingDataBuilder().keepStates(scope.getScale()).startState("space", null,
                        scope);
                IKlabData data = Resources.INSTANCE.getResourceData(auxiliaryResourceUrn, builder, IArtifact.Type.OBJECT,
                        "result", scope.getScale(), scope.getMonitor());
    ...
    public Filter locate(ITable<?> table, ILocator locator) {
        ...
        if (this.spatialContextualizer != null) {
            ...
            return ((AbstractTable<?>) table).newFilter(Filter.Type.COLUMN_MATCH, new Object[]{this.columnName, this.codes});
            ...
            if (time != null && this.temporalDimensions != null) {
                ...
                if (index == null) {
                    ...
                    if (this.indices.isEmpty() && !mappings.isEmpty()) {
                        ...
                        if (this.columnName.contains(",")) {
                            /*
                             * Two columns, a "between" condition of the time tracker for two index
                             * columns
                             */
                            String[] fields = this.columnName.split(",");
                            return (Filter) ((AbstractTable<?>) table).newFilter(Filter.Type.TIME_BETWEEN_FIELDS, null)
                                    .set("between", (Object[]) fields);
                        }

                        return ((AbstractTable<?>) table).newFilter(
                                this.dimension == Dimension.COLUMN ? Filter.Type.COLUMN_MATCH : Filter.Type.ROW_MATCH,
                                new Object[]{(this.dimension == Dimension.COLUMN ? this.columnName : this.rowName), filtered});
