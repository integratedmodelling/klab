...
public abstract class TableAdapter implements IResourceAdapter {
    ...
    /**
     * Storage for stuff that is too expensive to recompute at each resource use. Should be linked
     * to resource URN and URN only. Individual resources can store context-dependent data in their
     * own runtime storage at
        ...
        if (ret == null) {
            ...
            if (recomputeExtents) {
                if (resource.getParameters().containsKey("space.encoding")
                        && !resource.getParameters().get("space.encoding", String.class).trim().isEmpty()
                    ...
                    runtimeData.put(resource.getUrn() + "_space", new DimensionScanner<IShape>(resource, parts, IShape.class));
                }
                if (resource.getParameters().containsKey("time.encoding")
                        && !resource.getParameters().get("time.encoding", String.class).trim().isEmpty()
                    ...
                    runtimeData.put(resource.getUrn() + "_time", new DimensionScanner<ITime>(resource, parts, ITime.class));
                }
            }
