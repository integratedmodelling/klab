...
/**
 * H2-based cache - hopefully faster than the other based on DB. Can be created
 * from a resource after partial ingestion or pre-built by passing a table and a
 * URN.
 * 
 * @author Ferd
 *
...
public class SQLTableCache {
	...
	private void createStructure() {
		...
		/**
		 * Re-entrant or we can't test anything. TODO should be an option for when
		 * things are expensive or temporary.
		 */
	...
	/**
	 * Return the field width for the attribute if it's text, using any specs set in
	 * the resource parameters or defaults. Return -1 if no width should be
	 * specified (e.g. the attribute is numeric).
	 * 
	 * @param attribute
	 * @return
	 */
	public int getWidth(Attribute attribute) {
		Integer ret = resource == null ? -1
	...
	/**
	 * Dump the data as CSV with the specified row limit. If rows == 0, dump the
	 * headers only.
	 * 
	 * @param rows
	 * @param writer
