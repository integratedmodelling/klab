...
/**
 * For internal use only - not returned to the client.
 *
 * An object of this class is created based on a JWT token which has been submitted in a web request.
 *
 * The field names represent the chosen vocabulary we're using in k.LAB;
 * they might be different in JWT and/or Spring parlance, and therefore the calls
 * coming in and out of this class might feel a bit awkward - for instance, a JWT
 * "subject" is set here as a "username" and is returned using the Spring method getPrincipal().
 */
public class AuthenticationToken extends AbstractAuthenticationToken {
    ...
    /**
     * The time at which the token was issued - this would be the JWT token in the case of a typical login
     * by either username or cert file.
     */
    private LocalDateTime issuedAt;

    /**
     * The ID of the Integrated Modelling partner which owns the directory containing the user being authenticated.
     * For example, "im" is the ID for the main directory
     *
     * Spring equivalent: [none]
     * JWT equivalent: issuer ("iss" claim)
     */
    private final Credentials partnerId;

    /**
     * A collection of partner-specific permissions being granted with this Authentication.
     * For example, a PartnerAuthority [im, public-network] would indicate a "public-network"
     * permission level granted by the "im" partner directory.
     */
    ...
    /**
     * The username of the logged in user. Usernames will be unique within a partner directory,
     * but there may be duplicates between partners, so usernames should always be identified with
     * respect to the directory in which they are stored.
     */
    private final Credentials username;

    public AuthenticationToken(String partnerId, String username) {
        this(partnerId, username, null);
    }

    public AuthenticationToken(String partnerId, String username, Collection<Role> roles) {
        super(roles);
        ...
        // convenience code: mimic what the parent constructor did, but with the <Role> generic type.
        if (roles == null) {
