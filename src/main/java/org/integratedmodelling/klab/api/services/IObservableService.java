...
public interface IObservableService {
   ...
   * incarnate all the possible features for an observable. Get a builder from either
   * {@link IObservableService#declare(IConcept)},
   * {@link IObservableService#declare(String, IConcept)} or
   * {@link IObservableService#declare(String, Set)} (or any of the counterparts that specify the
   * ontology for the result) and call any methods in sequence before calling {{@link #build()} to
   * actually create the concept. The builder is smart and fast when concepts that already exist due
   * to previous declarations are requested.
   * 
