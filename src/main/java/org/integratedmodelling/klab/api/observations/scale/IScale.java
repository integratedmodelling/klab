...
public interface IScale extends IObservationTopology {
     ...
     * to a slice for an extent along it. Used in {@link #getIndex(Locator...)} and
    ...
    public interface Locator {
        ...
        /**
         * If the locator only covers the granule partially, return a value less than one, reflecting the
         * amount of active coverage. This will only return anything other than 1 when computed by a
         * IState.Mediator, which matches two scales and may find partial coverage when checking one index
         * against another. It should normally return 1 and never return 0.
         * 
    ...
    /**
     * Returned by getIndex, it returns offset indices along only one of the scale extents. It's also capable
     * of checking if the correspondent location is masked (i.e. not really part of the topology) or active.
     * 
     * @author Ferd
     *
     */
    public interface Index extends List<Integer> {
        ...
        /**
         * Check if the passed offset (relative to this index's dimension) correspond to a "live" observation
         * when the extents have regular topologies but may have inactive subdivisions.
         * 
         * @param offset
     ...
     * considering the sliceIndex-th part of the field from a total number of slices = sliceNumber. Used for
    ...
    /**
     * Take an array of objects that can locate a position in each extent, using the order of the extents in
     * the scale, and return the overall offset of the correspondent state (or -1 if not compatible). The
    ...
    /**
     * Get a scale that has either a 1-dimensional extent for the passed concept or doesn't have the extent at
     * all (if offset < 0). Ensure this scale remembers its offset and previous multiplicity along the
     * extent's dimension so that it will respond properly to getOffset() below.
     * 
     * TODO Possible improvement: allow passing an IExtent instead of an int, to accommodate variable scales
     * and ease the use of the API in some circumstances. That will require a getExtentOffset(IExtent) method.
     * 
     * @param extent
     * @param offset
     * @return the subscale
     ...
     * corresponding to the same granule on the full scale it derives from. If called on a full scale just
     * return the passed offset.
     * 
    ...
    /**
     * Take in another scale and complete what's left of our specs by merging in its details. E.g., we're a
     * bounding box, we get a grid without extent, and we become a grid in that bounding box. Will only be
     * called during resolution, so the queries should have selected compatible scales, but throw an exception
     * if anything is not compatible.
     * 
     * @param scale
