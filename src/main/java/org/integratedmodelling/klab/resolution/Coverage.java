...
//   * 
//   */
//  private static final long serialVersionUID      = 2540679045505081613L;
//
//  /*
...
//    double         coverage;
//
//    CExt(Dimension.Type domain, IExtent original, IExtent current, double coverage) {
//      this.domain = domain;
...
//        IExtent orig = ((AbstractExtent) e).getExtent();
//        IExtent curr = coverage > 0 ? orig : null;
...
//           */
//          IExtent other = its.current == null ? its.original : its.current;
//          IExtent current = my.current == null ? other : my.current.merge(other, LogicalConnector.UNION);
...
//        if (its.domain.equals(my.domain)) {
//          IExtent other = its.current == null ? its.original : its.current;
//          IExtent current = my.current == null ? other : my.current.merge(other, LogicalConnector.INTERSECTION);
//          double ncoverage = current.getCoveredExtent() / my.original.getCoveredExtent();
//          ret.coverage *= ncoverage;
//          ret.current.add(new CExt(my.domain, my.original, current, ncoverage));
//          extents.add((IExtent) current);
...
//   */
//  public ITopologicallyComparable<?> getOriginalExtent(Dimension.Type c) {
//    for (CExt cc : current) {
//      if (cc.domain.equals(c)) {
//        return cc.current;
//      }
...
//
//  @Override
//  public ICoverage merge(ITopologicallyComparable<?> coverage, LogicalConnector how) {
//    
...
//        return or((Coverage)coverage);
//    } else if (how == LogicalConnector.INTERSECTION) {
//      return and((Coverage)coverage);
//    }
