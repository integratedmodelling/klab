...
public enum Resolver {
    ...
    public ResolutionScope resolve(IResolvable resolvable, ResolutionScope scope)
        ...
        if (resolvable instanceof Observable) {
            return resolve((Observable) resolvable, scope, ((Observable) resolvable).is(Type.COUNTABLE)
                    ? Mode.INSTANTIATION
                    : Mode.RESOLUTION);
     ...
     * @throws KlabException
     */
    public ResolutionScope resolve(Observer observer, ResolutionScope scope) throws KlabException {
        ...
        ResolutionScope ret = resolve(observer.getObservable(), scope
    ...
    public ResolutionScope resolve(Observable observable, ResolutionScope scope, Mode mode) {
        ...
        try {
            for (IRankedModel model : Models.INSTANCE.resolve(observable, scope)) {
                ResolutionScope mret = resolve((RankedModel) model, scope);
                ret = ret == null ? mret : ret.or(mret);
                if (ret.isComplete()) {
                    break;
                }
            }
     ...
     * @throws KlabException
     */
    public ResolutionScope resolve(Model model, ResolutionScope scope) throws KlabException {

        ResolutionScope ret = scope.getChildScope(model);
        for (IObservable observable : model.getDependencies()) {
            ResolutionScope dep = resolve((Observable) observable, ret, observable.is(Type.COUNTABLE)
                    ? Mode.INSTANTIATION
                    : Mode.RESOLUTION);
            try {
                ret = ret.and(dep);
     ...
     * @return
     */
    public IPrioritizer<org.integratedmodelling.klab.data.rest.resources.Model> getPrioritizer(ResolutionScope context) {
