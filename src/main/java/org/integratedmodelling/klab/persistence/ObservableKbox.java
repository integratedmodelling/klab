 ...
 * Design principles:
 * </p>
 * * This is a hybrid kbox that depends on the reasoner knowing all concepts in it. For
 * this reason, it holds an index of the definitions of any compound observable concept
 * ever stored in it, and assigns an integer ID to it. The table is read on startup so
 * that all concepts are known to the reasoner, and any that are not will not be
 * retrievable.
 * <p>
 * * The main table is CONCEPTS, containing simply the ID of the type and its definition.
 * The method {@link #getCompatibleTypeIds(IConcept)} returns all the compatible type IDs
 * for an observable concept that have ever been stored in the database. A query for any
 * instance stored by derived kboxes with its ID in the set will return compatible
 * observables.
 * </p>
 * For convenience, the kbox also maintains a METADATA table for POD objects and exposes
 * simple methods to store/retrieve/delete metadata beans.
 * </p>
 * TODO For now the table does not maintain reference counts, so it is possible that
 * concepts IDs are referenced that are no longer represented because the corresponding
 * observations that have been deleted. Refcounting is easy to implement but costly at
 * store/delete, so let's see how problematic this becomes.
 * 
...
public abstract class ObservableKbox extends H2Kbox {
     ...
     * check for empty database - if this is not there, either nothing needs to be done or
    ...
    class ObservableSchema implements Schema {
        ...
        public String getCreateSQL() {
            return "CREATE TABLE concepts (" + "oid LONG PRIMARY KEY, " + "definition VARCHAR(1024), "
                    + "refcount LONG"
    ...
    /**
     * Get the ID correspondent to the passed concept, and if unavailable return -1. Does
     * not use the database so it's very fast.
     * 
     * @param c
     * @return
     */
    ...
    /**
     * Check that the passed observable has been inserted, and if not make sure it is
     * represented in the database. Return the stable ID to use for storing records that
     * use it.
     * 
     * @param observable
     * @return
    ...
    /**
     * Determine all the compatible MODEL concepts for which observables have been stored,
     * and return the set of their IDs.
     * 
     * If the core type is concrete, only that core type is looked up in the observable's
     * parents, so that models that observe that type (potentially with other traits not
     * adopted by the observable and in any compatible context) are found. If the core
     * type is abstract, any child is OK as long as trait, roles, inherency and context
     * are compatible.
     * 
     * @param observable
     * @return
     */
    public Set<Long> getCompatibleTypeIds(IConcept observable, IResolutionScope context) {

        // KLAB.info("ASKED FOR TYPES COMPATIBLE WITH " + observable + " = " +
        ...
        /*
         * add self if it's its own core observable. This may not be equal just because
         * it's in another ontology but with identical definition, so we also add it to
         * the compat list below.
         */
         ...
         * models if the observable is abstract or the context requires generic matching ('any' dependencies).
         * The initial set of candidates is weeded out of all incompatible or unrepresented concepts later.
         */
        Set<IConcept> candidates = Reasoner.INSTANCE.getParentClosure(main);

        if (main.isAbstract() || context.isGeneric()) {
    ...
    protected void storeMetadataFor(long oid, Metadata metadata) {
        ...
        for (String s : metadata.getKeys()) {
            ...
            try {
                /*
                 * OK, must execute these right away unfortunately - so if something goes
                 * wrong with the object's storage these will remain in the DB.
                 */
                PreparedStatement prsql = database.getConnection().prepareStatement(sql);
                prsql.setObject(1, metadata.getDataAsMap().get(s), Types.JAVA_OBJECT);
                prsql.executeUpdate();
            } catch (Exception e) {
                throw new KlabRuntimeException(e);
            }
    ...
    public int removeIfOlder(INamespace namespace) throws KlabException {
        ...
        if (timestamp > dbTimestamp) {
            ...
            monitor.debug("Refreshing observations in " + namespace.getName() + ": stored  "
