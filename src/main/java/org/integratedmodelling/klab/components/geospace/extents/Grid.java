...
public class Grid extends Area implements IGrid {
     ...
     */
    static Orientation[] swCorner          = new Orientation[] {
            Orientation.N,
            Orientation.NE,
            Orientation.E };
    static Orientation[] seCorner          = new Orientation[] {
            Orientation.W,
            Orientation.NW,
            Orientation.N };
    static Orientation[] nwCorner          = new Orientation[] {
            Orientation.S,
            Orientation.SE,
            Orientation.W };
    static Orientation[] neCorner          = new Orientation[] {
            Orientation.W,
            Orientation.SW,
            Orientation.S };
     ...
     */
    static Orientation[] sEdge             = new Orientation[] {
            Orientation.W,
            Orientation.NW,
            Orientation.N,
            Orientation.NE,
            Orientation.E };
    static Orientation[] eEdge             = new Orientation[] {
            Orientation.S,
            Orientation.SW,
            Orientation.W,
            Orientation.NW,
            Orientation.N };
    static Orientation[] nEdge             = new Orientation[] {
            Orientation.W,
            Orientation.SW,
            Orientation.S,
            Orientation.SE,
            Orientation.E };
    static Orientation[] wEdge             = new Orientation[] {
            Orientation.S,
            Orientation.SE,
            Orientation.E,
            Orientation.NE,
    ...
    static Orientation[] mooreNeighborhood = new Orientation[] {
            Orientation.W,
            Orientation.NW,
            Orientation.N,
            Orientation.NE,
...
//         setAdjustedEnvelope(shape, linearResolution);
    ...
    public IGrid cutToShape(Shape shape) throws KlabException {
        ...
        com.vividsolutions.jts.geom.Envelope genv = Envelope
                .create(getEast(), getWest(), getSouth(), getNorth(), projection).envelope;
...
//        if (shape.getProjection().getCoordinateReferenceSystem().getCoordinateSystem().getAxis(0).getUnit()
//                .toString().equals("m")) {
//
...
//                    + dx, env.getMinY()
//                            - dy, env.getMaxY() + dy, shape.getProjection().getCoordinateReferenceSystem());
//
//            // After doing the calculations in the original CRS,
//            // shape and envelope is transformed to default.
//
//            this.shape = shape.transform(Geospace.getDefaultProjection());
//            try {
//                this.envelope = Envelope.create(envelop_.transform(Geospace.getDefaultProjection().getCoordinateReferenceSystem(), true));
//            } catch (Exception e) {
//                // TODO: Shouldn't happen
//                e.printStackTrace();
//            }
//            this.projection = Geospace.getDefaultProjection();
//
//        }
//        // Case 2: CRS uses degrees
//        else {
//            // lets get height and width in meters
//            double height = Geospace.haversine(env.getMinY(), env.getMinX(), env.getMaxY(), env.getMinX());
//
//            double width = Geospace.haversine(env.getMinY(), env.getMinX(), env.getMinY(), env.getMaxX());
//            x = (int) Math.ceil(width / squareSize);
//            y = (int) Math.ceil(height / squareSize);
//
...
//        }
//
//        if (!this.projection.equals(Geospace.getDefaultProjection())) {
//            this.shape = this.shape.transform(Geospace.getDefaultProjection());
//            try {
//                this.envelope = this.envelope.transform(Geospace.getDefaultProjection(), true);
//            } catch (Exception e) {
//                // TODO: Shouldn't happen
//                e.printStackTrace();
//            }
