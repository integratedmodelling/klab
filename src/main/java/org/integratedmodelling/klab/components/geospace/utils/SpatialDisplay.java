...
public class SpatialDisplay {
  ...
  class SLDesc {
    ...
    void addFeature(Geometry shape) {
      ...
      if (featureBuilder == null) {
        try {
          ...
          fType = DataUtilities.createType("Location", "the_geom:" + shape.getGeometryType()
    ...
    Layer getLayer() {
      ...
      DefaultFeatureCollection featureCollection =
          new DefaultFeatureCollection("fc_" + name, fType);
  ...
  /**
   * Add a shape to the standard layer "shapes_xxx" where xxx is the geometry type of the shape. CRS
   * of shape isn't checked and must be same as passed scale.
   * 
   * @param shape
   */
  ...
  private Style createPolygonStyle() {
    ...
    Stroke stroke = styleFactory.createStroke(filterFactory.literal(Color.GRAY),
        filterFactory.literal(1), filterFactory.literal(0.5));

    // create a partial opaque fill
    Fill fill = styleFactory.createFill(filterFactory.literal(Color.LIGHT_GRAY),
  ...
  private Style createLineStyle() {
    Stroke stroke =
        styleFactory.createStroke(filterFactory.literal(Color.GREEN), filterFactory.literal(1));
