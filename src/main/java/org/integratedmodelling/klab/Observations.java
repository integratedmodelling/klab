...
public enum Observations implements IObservationService {
        ...
        if (agentClass != null) {
            try {
                constructor = agentClass
                        .getConstructor(String.class, IObservable.class, IScale.class, IMonitor.class);
            } catch (Exception e) {
            ...
            try {
                result = (Subject) constructor.newInstance(observer.getName(), observer.getObservable(), Scale
                        .create(observer.getBehavior().getExtents(monitor)), monitor);
            } catch (Exception e) {
                throw new KlabInternalErrorException("Unable to generate new instance of Java class '"
                        + agentClass.getCanonicalName()
                        + "' for agent type '" + observer.getObservable().getLocalName() + "'");
            }
        } else {

            result = Subject.create(observer.getName(), (Observable) observer.getObservable(), Scale
                    .create(observer.getBehavior().getExtents(monitor)), monitor);
    ...
    public Subject createSubject(Observable observable, String name, Scale scale, Namespace namespace, Monitor monitor, DirectObservation context)
    ...
    public IObservation createObservation(IObservable observable, IScale scale, String namespace, IMonitor monitor, IDirectObservation context) throws KlabException {
        IObservation ret = null;
        if (observable.is(Type.SUBJECT)) {
            ret = createSubject((Observable) observable, observable
                    .getLocalName(), (Scale) scale, Namespaces.INSTANCE
                            .getNamespace(namespace), (Monitor) monitor, (DirectObservation)context);
