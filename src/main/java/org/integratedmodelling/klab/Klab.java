...
public enum Klab implements IRuntimeService {
  ...
  public IStorageProvider getStorageProvider() {
    ...
    String providerClass = Configuration.INSTANCE.getProperties()
        .getProperty(IConfigurationService.STORAGE_PROVIDER_CLASS);
    int nproviders = 0;
    if (storageProvider == null) {
      for (IComponent component : Extensions.INSTANCE.getComponents()) {
        if (component instanceof IStorageProvider) {
          if (providerClass != null) {
            if (component.getClass().getCanonicalName().equals(providerClass)) {
              storageProvider = (IStorageProvider) component;
              break;
            }
    ...
    if (nproviders > 1 && providerClass == null) {
      throw new KlabRuntimeException(
          "multiple storage providers found: please configure the class of the desired provider");
    }

    if (storageProvider == null) {
      throw new KlabRuntimeException("no storage provider found: please install a storage plug-in");
    }
