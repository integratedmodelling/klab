...
public enum OWL {
  ...
  public IConcept getConceptFor(IRI iri) {
    ...
    if (ret == null) {
      throw new KlabRuntimeException(
          "internal: OWL IRI " + iri + " not corresponding to a known ontology");
    }
  ...
  private void initialize(IMonitor monitor) throws KlabException {
    ...
    /*
     * FIXME manual mapping until I understand what's going on with BFO, whose concepts have a IRI
     * that does not contain the namespace.
     */
     ...
     */
    this.systemConcepts.put(new SemanticType("owl:Thing"),
        manager.getOWLDataFactory().getOWLThing());
    this.systemConcepts.put(new SemanticType("owl:Nothing"),
        manager.getOWLDataFactory().getOWLNothing());

    if (this.loadPath == null) {
      throw new KlabIOException(
          "owl resources cannot be found: knowledge load directory does not exist");
    }
  ...
  public String getConceptSpace(IRI iri) {
    ...
    if (ret == null) {
      /*
       * happens, whenever we depend on a concept from a server ontology not loaded yet. Must find a
  ...
  public void restrictSome(IConcept target, IProperty property, IConcept filler) {
    target.getOntology().define(Collections
        .singleton(Axiom.SomeValuesFrom(target.getName(), property.toString(), filler.toString())));
  }

  public void restrictAll(IConcept target, IProperty property, LogicalConnector how,
      Collection<IConcept> fillers) {
    ...
    OWLClassExpression union =
        how.equals(LogicalConnector.UNION) ? factory.getOWLObjectUnionOf(classes)
            : factory.getOWLObjectIntersectionOf(classes);
    OWLClassExpression restriction =
        factory.getOWLObjectAllValuesFrom(((Property) property)._owl.asOWLObjectProperty(), union);
    manager.addAxiom(((Ontology) target.getOntology()).ontology,
        factory.getOWLSubClassOfAxiom(((Concept) target)._owl, restriction));
  }

  public void restrictSome(IConcept target, IProperty property, LogicalConnector how,
      Collection<IConcept> fillers) {
    ...
    OWLClassExpression union =
        how.equals(LogicalConnector.UNION) ? factory.getOWLObjectUnionOf(classes)
            : factory.getOWLObjectIntersectionOf(classes);
    OWLClassExpression restriction =
        factory.getOWLObjectSomeValuesFrom(((Property) property)._owl.asOWLObjectProperty(), union);
    manager.addAxiom(((Ontology) target.getOntology()).ontology,
        factory.getOWLSubClassOfAxiom(((Concept) target)._owl, restriction));
  }
  ...
  public void restrictAll(IConcept target, IProperty property, IConcept filler) {
    target.getOntology().define(Collections
        .singleton(Axiom.AllValuesFrom(target.getName(), property.toString(), filler.toString())));
  }

  public void restrictAtLeast(IConcept target, IProperty property, LogicalConnector how,
      Collection<IConcept> fillers, int min) {
  ...
  public void restrictAtLeast(IConcept target, IProperty property, IConcept filler, int min) {
    target.getOntology().define(Collections.singleton(
        Axiom.AtLeastNValuesFrom(target.getName(), property.toString(), filler.toString(), min)));
  }

  public void restrictAtMost(IConcept target, IProperty property, LogicalConnector how,
      Collection<IConcept> fillers, int max) {
  ...
  public void restrictAtMost(IConcept target, IProperty property, IConcept filler, int max) {
    target.getOntology().define(Collections.singleton(
        Axiom.AtMostNValuesFrom(target.getName(), property.toString(), filler.toString(), max)));
  }

  public void restrictExactly(IConcept target, IProperty property, LogicalConnector how,
      Collection<IConcept> fillers, int howmany) {
  ...
  public void restrictExactly(IConcept target, IProperty property, IConcept filler, int howMany) {
    target.getOntology().define(Collections.singleton(Axiom.ExactlyNValuesFrom(target.getName(),
        property.toString(), filler.toString(), howMany)));
  }
