...
public class AgERA5Repository extends CopernicusCDSDatacube {
    ...
    public enum Statistic {
        ...
        MAX_24H("24_hour_maximum", "max"),
        MIN_24H("24_hour_minimum", "min"),
        DAY_MEAN("day_time_mean", "day"),
        NIGHT_MEAN("night_time_mean", "night"),
        MEAN_24H("24_hour_mean", "mean"),
        NIGHT_MIN("night_time_minimum", "night_min"),
        DAY_MAX("day_time_maximum", "day_max");
    ...
    public enum Variable {
        ...
        CLOUD_COVER(null /* percent hours covered/24 */, "clouds",
                ...
                EnumSet.of(Statistic.MEAN_24H), "cloud_cover", EnumSet.noneOf(Timepoint.class),
                "cloud_cover"),
        ...
        SOLID_PRECIPITATION_DURATION_FRACTION(null /*
                ...
                EnumSet.noneOf(Statistic.class), "solid_precipitation_fraction",
                EnumSet.noneOf(Timepoint.class),
        ...
        Variable(String unit, String shortname, String description, Set<Statistic> statistics,
                String codename,
    ...
    protected String getOriginalDataFilename(String variable, int tick, File containingDirectory) {
        ...
        return template == null
                ...
                : (template.replace("XXXXXXXX",
                        "_" + String.format("%4d%02d%02d", start.getYear(), start.getMonth(), start.getDay())
                                + "_")
                        + ".nc");
    }

    @Override
    protected boolean createAggregatedLayer(String variable, int startTick, int endTick,
            ITime.Resolution resolution,
    ...
    protected String getAggregatedLayer(String variable, int startTick, int endTick) {
        ...
        return variable + "__"
                + String.format("%4d%02d%02d", start.getYear(), start.getMonth(), start.getDay()) + "_"
    ...
    /**
     * TODO must have precipitation available and insert the necessary processing in the granules.
     * Strategy should copy the one for precipitation and add the processing plus any wait time for
     * it.
     * 
     * @param time
     * @return
    ...
    private boolean requireRainyDaysCoverage(int month, int year, Granule granule) {
        ...
        final File retfil = new File(
                folder + File.separator + Variable.RAINY_DAYS_PER_MONTH.cdsname + "_" + month + "_" + year
                        + ".tiff");
        ...
        if (!retfil.exists() && !queued.contains(id)) {
            ...
            for (Pair<Integer, Integer> tick : getTicks(
                ...
                sourcePrecipitation
                        .add(getDataFile(Variable.LIQUID_PRECIPITATION_VOLUME.codename, tick.getFirst()));
            }
            queued.add(id);
            ...
            executor.execute(new Thread(){
                ...
                public void run() {
                    ...
                    Logging.INSTANCE.info(
                            "Creating monthly counts of days with precipitation for month " + month + " of "
                                    + year);

                    if (NetCDFUtils.aggregateGrid(sourcePrecipitation, retfil,
                            Time.resolution(1, Resolution.Type.MONTH), Aggregation.SUM, Double.NaN,
                            (value) -> {
                                return value > STORM_PRECIPITATION_THRESHOLD_MMDAY ? 1.0 : 0.0;
                            })) {
                        if (!getGeoserver().createCoverageLayer(AgERA5Repository.this.getName(), layerName,
                                retfil,
                                null)) {
                            Logging.INSTANCE
                                    .warn("Geoserver ingestion of " + retfil + " returned a failure code");
                        } else {
                        ...
                        Logging.INSTANCE.info(
                                "Creation of monthly counts of days with precipitation for month " + month
                                        + " of " + year + " successful");
                    }
                }
