...
public class OpenEOResourceValidator implements IResourceValidator {
    ...
    public Builder validate(String urn, URL url, IParameters<String> userData, IMonitor monitor) {
        ...
        if (userData.containsKey("namespace") || url != null) {
            try {
                ...
                /*
                 * TODO If process has the "self" link, store as namespace parameter. Should also
                 * handle the "latest-version" link.
                 */

                /*
                 * TODO geometry should be in the metadata. No OpenEO standard to express
                 * spatial/temporal coverage I think, and final coverage depends on the process even
                 * if the indivual datasets contain coverage. I don't see any way to add arbitrary
                 * metadata though.
                 * 
                 * Set geometryDefined = true if passed.
                 * 
                 * Another way (more general) would be to later drag/drop a GeoJSON file on the
                 * existing resource, probably with a standardized name, in addition to making the
                 * geospace editor interactive. Time can be edited directly in the editor.
                 */

                /*
                 * TODO analyze the process to see if there are potential intermediate products that
                 * could be exposed.
                 */
                ...
                if (process.getReturns() != null) {
                    ...
                    monitor.warn("OpenEO process " + process.getId() + " does not declare a return type: number assumed");
        ...
        if (!geometryDefined) {
             ...
             * 
             * TODO add ? as a value for all space/time hooks that we haven't resolved from the
             * process definition
             */
        ...
        if (geometry.isScalar()) {
            ...
            /*
             * TODO validate S/T specifications in parameters to match the declared geometry.
             * Conventions could be "spatial_extent" (GeoJSON) for space and temporal_extent (list
             * of two RFC 3339 time instants, [start end) ) for time, as in the standard processes.
             */
        }
    ...
    // TODO check conventions, handle more cases. Should probably only support spatial_extent and
    // temporal_extent
