...
public class KlabNavigatorActionProvider extends CommonActionProvider {
    ...
    public KlabNavigatorActionProvider() {
        ...
        action("Deactivate project", "Ignore the project until reset (requires engine restart)", "dontgothere.png",
                EProject.class, (project) -> KlabNavigatorActions.deactivate(project));
        ...
        action("Copy shape", "Copy the resource's bounding box to the clipboard", "copy.gif", EResource.class,
                (resource) -> Eclipse.INSTANCE.copyToClipboard(ResourceUtils.extractShapeSpecification(resource.getResource())))
                        .activate();
    ...
    <T extends ENavigatorItem> ActionDescriptor checkbox(String title, String tooltip, String icon, Class<T> applicable,
            Consumer<T> action) {
        ActionDescriptor ret = action(title, tooltip, icon, applicable, action);
        ret.checkbox = true;
        return ret;
    }
    ...
    public class KlabAction extends Action {
        ...
        public KlabAction(ActionDescriptor descriptor) {
            ...
            this.setImageDescriptor(ResourceManager.getPluginImageDescriptor(Activator.PLUGIN_ID, "icons/" + descriptor.icon));
