...
import org.integratedmodelling.klab.ide.auth.Authentication;
...
public class Activator extends AbstractUIPlugin {
    ...
    public void start(BundleContext context) throws Exception {
        ...
        Kim.INSTANCE.setValidator(new Validator(){
            ...
            public Pair<String, Boolean> getIdentityInformation(String authority, String identity, boolean formatted) {
                ...
                return Pair.create(errors + (error ? "\n\n" : "") + id.getDescription() + (notifications.isEmpty() ? "" : "\n\n")
                        + notifications, true);
            }
        ...
        /*
         * Read certificate, establish groups and worldview.
         */
        this.authentication = new Authentication();

        String worldview = this.authentication.getWorldview();

        this.engineStatusMonitor = new EngineMonitor(EngineMonitor.ENGINE_DEFAULT_URL, () -> engineOn(), () -> engineOff(),
                initialSessionId){
        ...
        /*
         * Pre-build workspaces and set up any overriding of others from projects in Eclipse
         * workspace. The IDE only has the worldview and the user workspace; the engine has more,
         * which can be put in the workspace of the IDE if desired.
         */
        Kim.INSTANCE.prebuildWorkspaces(getProjectFiles(worldview), getWorldviewFiles(worldview));

        /*
         * load the workspace. This is the only point where the ECore -> k.IM translation is
         * invoked.
         */
        ...
        KimLoader worldviewLoader = new KimLoader(
                ...
                getWorldviewFiles(worldview));

        // set the worldview as the external knowledge repo for the workspace
        this.loader = new KimLoader(KimActivator.getInstance().getInjector(KimActivator.ORG_INTEGRATEDMODELLING_KIM_KIM),
                worldviewLoader);

        // load the workspace, which may include the worldview
        this.loader.loadProjectFiles(getProjectFiles(worldview));
    ...
    public void reloadWorkspace() {
        this.loader.loadProjectFiles(getProjectFiles(this.authentication.getWorldview()));
    }
    ...
    private Collection<File> getProjectFiles(String worldview) {
        List<File> projectFiles = new ArrayList<>();
        for (IProject project : Eclipse.INSTANCE.getProjects()) {
            File projectRoot = project.getLocation().toFile();
            String wv = Kim.INSTANCE.getWorldview(projectRoot);
            if (worldview == null || wv == null || worldview.equals(wv)) {
                projectFiles.add(projectRoot);
                // this preloads all resources with offline status so they can be updated when
                // the engine is connected
                klab.synchronizeProjectResources(project.getName(), projectRoot);
            }
        }
        return projectFiles;
    }

    private Collection<File> getWorldviewFiles(String worldview) {
        ...
        if (file.isDirectory()) {
            for (File pfile : file.listFiles()) {
                if (Kim.INSTANCE.isKimProject(pfile)) {
                    String wv = Kim.INSTANCE.getWorldviewProject(pfile);
                    if (worldview == null || wv == null || worldview.equals(wv)) {
                        ret.add(pfile);
                    }
                }
    ...
    private void engineOn() {
        ...
        /*
         * offer to import any k.LAB local projects that are not in the workspace and have the
         * engine load those projects it does not have. TODO may also offer to import the worldview
         * for developers.
         */
        Display.getDefault().asyncExec(() -> {
            ...
            for (ProjectReference project : this.engineStatusMonitor.getCapabilities().getLocalWorkspaceProjects()) {
                if (Eclipse.INSTANCE.getProject(project.getName()) == null
                        && /*
                            * TODO OBSOLETE - REMOVE THIS AFTER DEALING WITH EXISTING INSTALLATIONS
                            */!project.getName().equals("local")) {
            ...
            /*
             * find out which projects in our workspace are not known to the engine and send them
             * over.
             */
            List<File> projectFiles = new ArrayList<>();
            for (IProject project : Eclipse.INSTANCE.getProjects()) {
    ...
    public static void reply(IMessage original, Object... object) {
        if (get().engineStatusMonitor.isRunning()) {
            get().engineStatusMonitor.getBus()
                    .post(Message.create(get().engineStatusMonitor.getSessionId(), object).inResponseTo(original.getId()));
        }
    }
    ...
    public static void reply(String originalMessageId, Object... object) {
        if (get().engineStatusMonitor.isRunning()) {
            get().engineStatusMonitor.getBus()
                    .post(Message.create(get().engineStatusMonitor.getSessionId(), object).inResponseTo(originalMessageId));
        }
    }
