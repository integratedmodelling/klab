...
public class PaletteItem extends Canvas {
	...
	public static interface SelectListener {
		void onSelect(Item item, boolean enabled, boolean shiftPressed);

		void onExpand(Item item, boolean enabled, boolean shiftPressed);
	}
	...
	public PaletteItem(Composite parent, Item item) {
		...
		addMouseListener(this.mouseListener = new MouseListener() {
			...
			public void mouseDown(MouseEvent e) {
				if ((item.isSelectable()) && listener != null && e.x > 5
						&& e.x < 19 && e.y > 5 && e.y < 17) {
	...
	public Color getColor() {
		...
		// TODO use CSS color names and color utils from engine/core
		case "red":
