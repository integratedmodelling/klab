...
public class KimHighlightingCalculator extends DefaultSemanticHighlightingCalculator {
  ...
  public void provideHighlightingFor(XtextResource resource, IHighlightedPositionAcceptor acceptor,
    ...
    // ensure that we only reconsider this when the node is for a position further away than the
        ...
        if (object instanceof Assignment && node.getOffset() > start) {
          ...
          if (rule.getName().equals("Annotation")) {
            ...
            if (node.getSemanticElement() instanceof Function) {
              known = Kim.INSTANCE
                  .isFunctionKnown(((Function) node.getSemanticElement()).getName());
            }
            acceptor.addPosition((start = node.getOffset()), node.getLength(),
            ...
            if (!inactive
                && ((ModelStatement) node.getSemanticElement()).getBody().getUrn() != null) {
              UrnDescriptor ud = Kim.INSTANCE.getUrnDescriptor(
                  ((ModelStatement) node.getSemanticElement()).getBody().getUrn().getName());
            ...
            if (text != null && !text.isEmpty()) {
              ...
              if (!text.contains(":")) {
                ...
                text = (namespace == null ? "UNDEFINED" : namespace.getName()) + ":" + text;
              }
