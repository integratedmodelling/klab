...
public class KimWorkspace implements IKimWorkspace {
    ...
    /**
     * Register all project locations configured in, without loading the resources in them.
     * Reentrant - won't read projects that are already registered.
     */
    ...
    /**
     * Constructor for a logical workspace with a default file location at the passed subspace which
     * serves as a name and also creates or registers a directory in the k.LAB data dir. Projects
     * are created manually and explicitly after creation, and may be in the file workspace or not.
     * 
    ...
    /**
     * Constructor for a file-based workspace. This one will use the root file location as an actual
     * physical workspace and register any k.IM projects in it. Meant to be created by the API user
     * (e.g. an engine) and not through the global Kim instance, so it will register itself into it.
     * 
     * The names decide whether Kim register these as the worldview or the r/w user workspace.
     * 
     * @param root
     * @param worldview if null, all projects are loaded; otherwise only those that have no
     *        worldview or the same worldview either as a definition or a requirement.
     * @param overridingProjects
     */
    public KimWorkspace(File root, String worldview, String name) {
        this.root = root;
        this.name = name;
        try {
            this.url = normalize(root.toURI().toURL());
        ...
        for (File sub : root.listFiles()) {
            if (Kim.INSTANCE.isKimProject(sub)) {
                String wv = Kim.INSTANCE.getWorldview(sub);
                if (wv == null) {
                    wv = Kim.INSTANCE.getWorldviewProject(sub);
                }
    ...
    private void loadNamespaceIds(File file, String prefix) {
        ...
        if (file.isDirectory()) {
            ...
            namespaceIds
                    .add(prefix + (prefix.isEmpty() || prefix.endsWith(".") ? "" : ".") + MiscUtilities.getFileBaseName(file));
        }
    }
    ...
    /**
     * Return the list of projects sorted by dependency. This is lenient - if a project doesn't have
     * a dependency we just warn.
     * 
     * @return
     */
    ...
    /**
     * Return all the pre-loaded namespace IDs based on the presence of the corresponding files in
     * the project space. Used to predefine correct imports and analyze dependencies during and not
