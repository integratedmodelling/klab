...
public class ComputableResource extends KimStatement implements IContextualizable {
	...
	public boolean isFinal() {
		...
		case RESOURCE:
			...
			return validatedResource instanceof IResource ? ((IResource) validatedResource).getInputs().isEmpty()
					: false;
		case SERVICE:
	...
	public IContextualizable contextualize(IArtifact target, IContextualizationScope scope) {
		if (getType() == Type.RESOURCE) {
			...
			IResource resource = resourceService.contextualizeResource(getResource(), urn.getParameters(),
					scope.getScale(), target, scope);
