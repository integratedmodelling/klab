...
public class KimProject implements IKimProject {
  ...
  public KimProject(KimWorkspace workspace, File dir) {
    ...
    File pfile =
        new File(this.root + File.separator + "META-INF" + File.separator + "klab.properties");
    if (pfile.exists()) {
...
//  @Override
  public String getNamespaceIdFor(EObject o) {
    ...
    if (wuri.startsWith(PLATFORM_URI_PREFIX) && kuri.startsWith("file:")) {
      ...
      if (resolver != null) {
        String ws = resolver.resolveResourceUriToWorkspaceRootDirectory(o.eResource().getURI())
            .toURI().toString();
