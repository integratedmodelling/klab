...
public enum Kim {
    ...
    /**
     * This ID is used for a common project where all namespaces read from isolated files will be
     * added.
     */
    ...
    /**
     * Core concepts (observation ontology) are now explicitly declared for use in worldviews, and are
     * validated through a callback.
     */
    private Map<String, EnumSet<Type>>                  coreConcepts      = new HashMap<>();

    /**
     * Workspace for libraries. May be null. Loaded automatically at startup if present, with code
     * generation only in the engine.
     */
    ...
    public static class ConceptDescriptor implements IConceptDescriptor {
        ...
        public ConceptDescriptor(String id, EnumSet<Type> type, IKimConceptStatement macro,
                String string) {
            this.name = id;
            this.flags = type;
            this.documentation = string;
    ...
    public static interface Validator {
        ...
        /**
         * Return any errors, warnings and info messages caused by the passed function. Only called if
         * {@link #isFunctionKnown(String)} returns true.
         * 
         * @param functionCall
         * @param expectedType
        ...
        /**
         * Return any errors, warnings and info messages caused by the passed annotation or its usage.
         * Only called if {@link #isAnnotationKnown(String)} returns true.
         * 
        ...
        /**
         * If the core type is known, return the type of declarable this core type represents. If it is
        ...
        /**
         * Main method to notify that a new or updated namespace is available. The passed bean holds all
         * the info required to register the namespace in the runtime.
         * 
         * @param namespace
    ...
    public KimConcept declareConcept(ConceptDeclaration declaration) {
        return declaration == null ? null : KimConcept.normalize(declaration);
    }
    ...
    public KimObservable declareObservable(ObservableSemantics declaration) {
        return declaration == null ? null : KimObservable.normalize(declaration);
    }
    ...
    public Object parseValue(Value value, IKimNamespace namespace) {
        if (value.getLiteral() != null) {
            ...
            return new KimServiceCall(value.getFunction());
...
//          return parseMap(value.getMap(), namespace);
 ...
 public Parameters parseMap(Metadata map, IKimNamespace namespace) {
   ...
   public Object parseLiteral(Literal literal, IKimNamespace namespace) {
     ...
     * @param overridingProjects the overriding projects
    ...
    public KimWorkspace getLibrary(String libname, File... overridingProjects) {
        ...
        if (!workspaceInited) {
            ...
            libWorkspace = new KimWorkspace(new File(System.getProperty("user.home") + File.separator
                    + ".klab" + File.separator + libname));
    ...
    public EnumSet<Type> getType(String string) {
        ...
        case "electric-potential":
            return EnumSet
                    .of(Type.ELECTRIC_POTENTIAL, Type.QUALITY, Type.INTENSIVE_PROPERTY, Type.OBSERVABLE);
        case "charge":
    ...
    public void setupTestConcepts() {
        setConceptDescriptor("geography:Elevation", new ConceptDescriptor("geography:Elevation", "The height of the terrain over sea level.", Type.OBSERVABLE, Type.QUALITY, Type.EXTENSIVE_PROPERTY, Type.LENGTH), true);
        setConceptDescriptor("geography:Slope", new ConceptDescriptor("geography:Slope", Type.OBSERVABLE, Type.QUALITY, Type.INTENSIVE_PROPERTY, Type.ANGLE), true);
        setConceptDescriptor("hydrology:HydrologicalSoilGroup", new ConceptDescriptor("hydrology:HydrologicalSoilGroup", Type.OBSERVABLE, Type.QUALITY, Type.CLASS), true);
        setConceptDescriptor("infrastructure:City", new ConceptDescriptor("infrastructure:City", Type.OBSERVABLE, Type.SUBJECT, Type.COUNTABLE, Type.DIRECT_OBSERVABLE), true);
        setConceptDescriptor("infrastructure:Building", new ConceptDescriptor("infrastructure:Building", Type.OBSERVABLE, Type.SUBJECT, Type.COUNTABLE, Type.DIRECT_OBSERVABLE), true);
        setConceptDescriptor("earth:Earthquake", new ConceptDescriptor("earth:Earthquake", Type.OBSERVABLE, Type.EVENT, Type.COUNTABLE, Type.DIRECT_OBSERVABLE), true);
        setConceptDescriptor("hydrology:SurfaceWaterFlow", new ConceptDescriptor("hydrology:SurfaceWaterFlow", Type.OBSERVABLE, Type.PROCESS, Type.DIRECT_OBSERVABLE), true);
        setConceptDescriptor("im:Tall", new ConceptDescriptor("im:Tall", Type.TRAIT, Type.ATTRIBUTE, Type.ORDERING, Type.SUBJECTIVE), true);
        setConceptDescriptor("im:Small", new ConceptDescriptor("im:Small", Type.TRAIT, Type.ATTRIBUTE, Type.ORDERING, Type.SUBJECTIVE), true);
        setConceptDescriptor("im:Normalized", new ConceptDescriptor("im:Normalized", Type.TRAIT, Type.ATTRIBUTE), true);
        setConceptDescriptor("im:Large", new ConceptDescriptor("im:Large", Type.TRAIT, Type.ATTRIBUTE, Type.ORDERING, Type.SUBJECTIVE), true);
        setConceptDescriptor("im:Potential", new ConceptDescriptor("im:Potential", Type.TRAIT, Type.ATTRIBUTE), true);
        setConceptDescriptor("im:Level", new ConceptDescriptor("im:Level", Type.TRAIT, Type.ATTRIBUTE, Type.ORDERING, Type.ABSTRACT, Type.SUBJECTIVE), true);
        setConceptDescriptor("im:High", new ConceptDescriptor("im:High", Type.TRAIT, Type.ATTRIBUTE, Type.ORDERING, Type.SUBJECTIVE), true);
        setConceptDescriptor("im:Moderate", new ConceptDescriptor("im:Moderate", Type.TRAIT, Type.ATTRIBUTE, Type.ORDERING, Type.SUBJECTIVE), true);
        setConceptDescriptor("im:Low", new ConceptDescriptor("im:Low", Type.TRAIT, Type.ATTRIBUTE, Type.ORDERING, Type.SUBJECTIVE), true);
        setConceptDescriptor("materials:Concrete", new ConceptDescriptor("materials:Concrete", Type.TRAIT, Type.IDENTITY), true);
    }
    ...
    /**
     * Validate a function call by returning a list of the correspondent notifications. If the list is
    ...
    public EnumSet<Type> checkCoreConcept(String string, EnumSet<Type> type) {
        ...
        if (coreConcepts.containsKey(string)) {
            ...
            /**
             * use whatever type the callback returned. Without callback, we just take the first stated
             * type as the truth.
             */
            if (validatorCallback == null) {
                ret = type;
        ...
        /*
         * if no match between the "truth" and the requested type, return 0 which will cause an error.
         */
