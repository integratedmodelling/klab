...
public class KimBehavior implements IKimBehavior {
  ...
  public List<KimNotification> addAction(ActionSpecification action, IKimActiveStatement context) {
    ...
    KimAction act = new KimAction(action);
    boolean ok = true;
    ...
    if (action.isTrigger()) {
      ...
      for (FunctionOrID domain : action.getDomain()) {
        if (domain.getFunction() != null) {
          extents.add(new KimServiceCall(domain.getFunction()));
        } else {
          extents.add(new KimServiceCall(domain, domain.getFunctionId(), new HashMap<>()));
        }
        for (IServiceCall extent : extents) {
          for (KimNotification note : Kim.INSTANCE.validateFunctionCall(extent, Sets
              .immutableEnumSet(IPrototype.Type.SPATIALEXTENT, IPrototype.Type.TEMPORALEXTENT))) {
    ...
    for (int i = 0; i < action.getActions().size(); i++) {
      ...
      KimAction targetAction = i == 0 ? act : new KimAction(action);
