	...
	def checkObservableSemantics(ObservableSemantics semantics) {
		...
		if (declaration !== null) {
			...
			if (semantics.by !== null) {
				...
				var by = KimConcept.normalize(semantics.getBy(), declaration)
				...
				if (flags.isEmpty) {
					...
					if (macro !== null && macro.fields.contains(Field.INHERENT)) {
						...
						if (!ctype.containsAll(rtype.type)) {
							error(
								"The inherent type (of) does not match the type requested by the " + macro.name +
			...
			if (declaration.caused !== null) {
				...
				if (flags.isEmpty) {
					...
					if (macro !== null && macro.fields.contains(Field.CAUSED)) {
						...
						var ctype = Kim.intersection(rtype.type, flags)
						if (!ctype.containsAll(rtype.type)) {
							error("The caused type (to) does not match the type requested by the " + macro.name +
								" macro", declaration.caused, null, KimPackage.CONCEPT_DECLARATION__CAUSED)
							error = true
						} else {
