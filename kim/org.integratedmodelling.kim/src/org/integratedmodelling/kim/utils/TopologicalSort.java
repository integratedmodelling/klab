 ...
 * sorted order before the expanded node, we have the desired property of the topological sort.
 *
 * This process can be augmented to detect a cycle in the original graph. As we do the search, we'll
...
public final class TopologicalSort {
  /**
   * Given a directed acyclic graph, returns a topological sorting of the nodes in the graph. If the
   * input graph is not a DAG, throws an IllegalArgumentException.
   *
  ...
  public static <T> List<T> sort(DirectedGraph<T> g) {
    ...
    /*
     * We'll also maintain a third set consisting of all nodes that have been fully expanded. If the
     * graph contains a cycle, then we can detect this by noting that a node has been explored but
     * not fully expanded.
     */
  ...
  /**
   * Recursively performs a DFS from the specified node, marking all nodes encountered by the
   * search.
   *
   ...
   */
  private static <T> void explore(T node, DirectedGraph<T> g, List<T> ordering, Set<T> visited,
      Set<T> expanded) {
    /*
    ...
    if (visited.contains(node)) {
      /*
       * There are two cases to consider. First, if this node has already been expanded, then it's
       * already been assigned a position in the final topological sort and we don't need to explore
       * it again. However, if it hasn't been expanded, it means that we've just found a node that
       * is currently being explored, and therefore is part of a cycle. In that case, we should
       * report an error.
       */
