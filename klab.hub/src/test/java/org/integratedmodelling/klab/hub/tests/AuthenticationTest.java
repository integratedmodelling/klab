...
//		request.setPassword("password");
    ...
    public void pass_get_user_engine_certificate() throws URISyntaxException, IOException {
        ...
        EngineAuthenticationRequest engineRequest = new EngineAuthenticationRequest(
                cert.getProperty(KlabCertificate.KEY_USERNAME), cert.getProperty(KlabCertificate.KEY_SIGNATURE),
                cert.getProperty(KlabCertificate.KEY_CERTIFICATE_TYPE), cert.getProperty(KlabCertificate.KEY_CERTIFICATE),
                cert.getLevel(), cert.getProperty(KlabCertificate.KEY_AGREEMENT));
        ...
        ResponseEntity<EngineAuthenticationResponse> engineAuth = restTemplate.exchange(authUri, HttpMethod.POST,
                authRequestEntity, EngineAuthenticationResponse.class);

        ArrayList<HubNotificationMessage> messages = engineAuth.getBody().getMessages();

        List<HubNotificationMessage> errors = messages.stream().filter(msg -> msg.getType().equals(Type.ERROR))
                .collect(Collectors.toList());

        List<HubNotificationMessage> warnings = messages.stream().filter(msg -> msg.getType().equals(Type.WARNING))
                .collect(Collectors.toList());

        List<HubNotificationMessage> infos = messages.stream().filter(msg -> msg.getType().equals(Type.INFO))
                .collect(Collectors.toList());
    ...
    public void pass_get_node_certificate() throws URISyntaxException, IOException {
        ...
        URI authUri = new URI(authUrl);
        ResponseEntity<NodeAuthenticationResponse> engineAuth = restTemplate.exchange(authUri, HttpMethod.POST, authRequestEntity,
                NodeAuthenticationResponse.class);
        Assert.assertEquals(200, engineAuth.getStatusCodeValue());
    }
