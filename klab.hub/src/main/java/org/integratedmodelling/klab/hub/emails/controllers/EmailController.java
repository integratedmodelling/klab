...
public class EmailController {
    ...
    public ResponseEntity< ? > sendEmail(@RequestBody KlabEmail email) {
        ...
        // if
        // (Arrays.stream(emailConfig.getAuthorizedEmailAddresses()).anyMatch(email.from::equals)) {
        // to and replayTo are known
        for(String userEmail : email.to) {
            if (!userRepository.findByEmailIgnoreCase(userEmail).isPresent()) {
                response = ResponseEntity.status(HttpStatus.NOT_FOUND).body("Recipient must be known users");
                break;
            }
        }
        for(String userEmail : email.replayTo) {
            if (!userRepository.findByEmailIgnoreCase(userEmail).isPresent()
                ...
                response = ResponseEntity.status(HttpStatus.NOT_FOUND).body("Reply to must be known users or authorized email");
                break;
            }
        }
        /*
        ...
        */
        ...
        try {
            emailManager.send(email.from, email.to, email.replayTo, email.subject, email.content, email.type != EmailType.TEXT,
                    email.attachments);
            return ResponseEntity.status(HttpStatus.OK).body("Email sent");
        } catch (SendEmailException see) {
            return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR).body(see.getMessage());
        }
    }
}
