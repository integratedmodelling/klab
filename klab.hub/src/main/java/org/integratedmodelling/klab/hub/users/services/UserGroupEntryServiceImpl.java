...
public class UserGroupEntryServiceImpl implements UserGroupEntryService {
    ...
    public UserGroupEntryServiceImpl(UserRepository userRepository, MongoGroupRepository groupRepository,
            AgreementService agreementService, AgreementRepository agreementRepository) {
        super();
    ...
    public void setUsersGroupsByNames(UpdateUsersGroups updateRequest) {
        ...
        for(String username : updateRequest.getUsernames()) {
            users.add(userRepository.findByNameIgnoreCase(username).map(user -> {
    ...
    private List<MongoGroup> getDependendOnGroups(String groupName) {
        List<String> dependedOnGroupNames = groupRepository.findByNameIgnoreCase(groupName)
                .orElseThrow(() -> new GroupDoesNotExistException(groupName)).getDependsOn();
    ...
    private Collection<Agreement> getAgreementsOfUserWithGroup(User user, String groupName) {
        return user.getAgreements().stream().map(AgreementEntry::getAgreement)
                .filter(ag -> ag.isValid() && ag.getGroupEntries().contains(groupName)).collect(Collectors.toUnmodifiableSet());
    }
    ...
    private Optional<Date> calculateMaxExpirationOfGroupForUser(User user, MongoGroup group) {
        ...
        Date maxFoundExpiration = agreementsWithGroup.stream().map(Agreement::getExpirationDate).max(Date::compareTo).get();
        ...
        if (defaultExpirationDateOfGroup != 0) {
            ...
            maxFoundExpiration = maxFoundExpiration.before(defaultExpiration) ? maxFoundExpiration : defaultExpiration;
        }
    ...
    public void removeUsersGroupsByNames(UpdateUsersGroups updateRequest) {
        ...
        for(String username : updateRequest.getUsernames()) {
            userRepository.findByNameIgnoreCase(username).ifPresent(user -> {
    ...
    private Set<GroupEntry> createGroupEntries(Set<String> groupnames, LocalDateTime expiration) {
        ...
        for(String groupname : groupnames) {
            groupRepository.findByNameIgnoreCase(groupname).ifPresent(grp -> {
    ...
    private Set<GroupEntry> createComplimentaryGroupEntries(LocalDateTime expiration) {
        Set<GroupEntry> groupEntries = new HashSet<>();
        groupRepository.findComplimentaryGroups().forEach(grp -> groupEntries.add(new GroupEntry(grp, expiration)));
        return groupEntries;
    }
    ...
    public List<String> getUsersWithGroup(String group) {
        ...
        return userRepository.getUsersByGroupEntriesWithGroupId(id).stream().map(User::getName).collect(Collectors.toList());
    }
