...
public interface UserRepository extends ResourceRepository<User, String> {
    ...
    @Aggregation(pipeline = {"{" + "    $group: {" + "      _id: {" + "      	month: {"
            + "       	 $month: $registrationDate" + "      	}," + "     	 year: {" + "      	  $year: $registrationDate"
            + "    	  	}" + "      }," + "      count: {" + "        $sum: 1" + "      }," + "		month:{"
            + "			$first: {$month: $registrationDate}" + "		}," + "		year:{"
            + "			$first: {$year: $registrationDate}" + "    }," + "    dateString: {" + "			$first: {"
            + "				 $dateToString: {" + "    				date: $registrationDate,"
            + "    				format: '%Y-%m'" + "				}" + "			}" + "    	  }" + "    }" + "  }"})

    public List<GroupUsersByDate> groupByMonthYear();

    @Aggregation(pipeline = {"{" + "    $group: {" + "      _id: {" + "     	 year: {" + "      	  $year: $registrationDate"
            + "    	  	}" + "      }," + "      count: {" + "        $sum: 1" + "      }," + "		year:{"
            + "			$first: {$year: $registrationDate}" + "    }," + "    dateString: {" + "			'$first': {"
            + "				 '$dateToString': {" + "    				'date': '$registrationDate',"
            + "    				'format': '%Y'" + "				}" + "			}" + "    	  }" + "    }" + "  }"})
