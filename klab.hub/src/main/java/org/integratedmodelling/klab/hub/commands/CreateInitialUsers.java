...
public class CreateInitialUsers {
    ...
    private static final User system = testUser("system", "password", "test.bc3@integratedmodelling.org", "Joe", "Robot",
            Role.ROLE_USER, Role.ROLE_ADMINISTRATOR, Role.ROLE_SYSTEM);

    private static final User hades = testUser("hades", "password", "hades@integratedmodelling.org", "Hades", "of Greece",
            Role.ROLE_USER, Role.ROLE_ADMINISTRATOR);

    private static final User developerS = testUser("kristina", "kristina", "kristina.sanchez@bc3research.org", "Hercules",
            "of Rome", Role.ROLE_USER, Role.ROLE_ADMINISTRATOR);

    private static final User developerE = testUser("enrico", "password", "enrico.girotto@bc3research.org", "Enrico", "of Venice",
            Role.ROLE_USER);

    private static final User achilles_activeMissingLdap = testUser("achilles", "password", "achilles@integratedmodelling.org",
            "Achilles", "of Greece", Role.ROLE_USER);

    private static final User triton_pendingMissingLdap = testUser("triton", "password", "triton@integratedmodelling.org",
            "Triton", "of Greece", Role.ROLE_USER);
    ...
    private List<User> getInitialUsers() {
        ...
        entries.add(alice);
        for(int i = 0; i < 100; i++) {
            User u = testUser("user" + i, "password", "user-" + i + "@integratedmodelling.org", "Name" + i, "Last" + i,
                    Role.ROLE_USER);
            ...
            if (x <= 80) {
                u.addGroupEntries(im); // 80% has IM and ARIES. If no IM and ARIES, no groups for
                                       // now
