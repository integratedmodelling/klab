...
public class NodeAuthResponeFactory {
    ...
    public NodeAuthResponeFactory(NodeService nodeService, MongoGroupRepository groupRepository,
            LicenseConfigService configService) {
        this.nodeService = nodeService;
        this.groupRepository = groupRepository;
        this.configService = configService;
        // this solve the issue of unknown properties observationReferences, but avoid send the
        // observable linked to the group
    ...
    public NodeAuthenticationResponse getRespone(NodeAuthenticationRequest request, String ip) {
        ...
        default:
            if (IPUtils.isLocalhost(ip)) {
                // You are running locally with a hub, so it is assumed that the hub is a
                // development hub
                return local(request);
            } else {
    ...
    private NodeAuthenticationResponse process(String cipher, MongoNode node, Set<Group> groups,
        ...
        if (nodeProperties.equals(cipherProperties)) {
            ...
            AuthenticatedIdentity authenticatedIdentity = new GetNodeAuthenticatedIdentity(nodeIdentity, groups).execute();
    ...
    private NodeAuthenticationResponse local(NodeAuthenticationRequest request) {
        ...
        mongoGroups.forEach(mongoGroup -> groups.add(objectMapper.convertValue(mongoGroup, Group.class)));

        INodeIdentity node = authenticateLocal(request.getName());

        Logging.INSTANCE.info("authorized installed node " + node.getName());

        IdentityReference userIdentity = new IdentityReference(node.getName(), node.getParentIdentity().getEmailAddress(),
                now.toString());

        AuthenticatedIdentity authenticatedIdentity = new AuthenticatedIdentity(userIdentity, groups, tomorrow.toString(),
                node.getId());
