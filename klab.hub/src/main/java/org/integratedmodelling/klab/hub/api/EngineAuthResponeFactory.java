...
//	    		}
	...
	private void checkForExpiringAgreement(Agreement agreement, ArrayList<HubNotificationMessage> messages) {
        ...
        if (agreement.isExpirable() && agreement.getExpiredDate().toInstant().isBefore(nowPlus30Days)) {
            HubNotificationMessage msg = HubNotificationMessage.MessageClass
                    .EXPIRING_AGREEMENT.build("Agreement set to expire on: " + agreement.getExpiredDate(), new Parameters((Pair<ExtendedInfo, Object>[])(new Pair[] {
                            new Pair<ExtendedInfo, Object>(HubNotificationMessage.ExtendedInfo.EXPIRATION_DATE, agreement.getExpiredDate())
                    })));
            messages.add(msg);   
        }
