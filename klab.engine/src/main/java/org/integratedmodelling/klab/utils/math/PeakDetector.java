...
public final class PeakDetector {
	 ...
	 * @author Joren Six
	 */
	public final static class Peak implements Comparable<Peak> {
		/**
	...
	public interface PeakDetectionStrategy {
		 ...
		 * @param windowSize
		 *            The windowSize in number of bins.
		 * @param thresholdFactor
		 *            A number between 0 and 100 that determines a threshold.
	...
	public static final PeakDetectionStrategy LOCALHEIGHTSCORE = new PeakDetectionStrategy() {
		...
		public List<Peak> detect(Histogram histogram, int windowSize, int thresholdFactor) {
			// 1. CALCULATE SCORES
...
//	 * creates a histogram (with the same resolution (bin widths) as the original)
//	 * with peaks in the form of gaussian curves.
//	 * 
//	 * @param peaks
//	 *            A list With Peaks
//	 * @return a histogram with peak information. Can be used to match with other
//	 *         histograms (files)
//	 */
...
//	}
	 ...
	 * by the standard deviation (of the height in the window). The window size is
	 * defined in number of classes in the given histogram.
	 * 
	 * @param histogram
	 ...
	 * @param windowSize
	 *            The window size used to calculate the mean, standard deviation and
	 *            score.
	 * @return The local height score.
	 */
	public static double calculateLocalHeightScore(final Histogram histogram, final int index, final int windowSize) {
	...
	public static List<Peak> detectNumberOfPeaks(final Histogram histogram, final int numberOfPeaks) {
		...
		Collections.sort(entryList, new Comparator<Map.Entry<Double, Integer>>() {
			public int compare(final Entry<Double, Integer> firstEntry, final Entry<Double, Integer> secondEntry) {
	...
	public static List<Peak> detect(final Histogram histogram) {
		...
		Collections.sort(entryList, new Comparator<Map.Entry<Integer, Integer>>() {
			public int compare(final Entry<Integer, Integer> firstEntry, final Entry<Integer, Integer> secondEntry) {
				final Integer firstEntryCount = firstEntry.getValue();
				final Integer secondEntryCount = secondEntry.getValue();
				return secondEntryCount.compareTo(firstEntryCount);
			}
