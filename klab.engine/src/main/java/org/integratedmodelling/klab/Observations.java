...
public enum Observations implements IObservationService {
    ...
    private StateSummary computeStateSummary(IState state, ILocator locator) {
        ...
        if (ret.getNodataPercentage() < 1) {
            Builder histogram = Histogram.builder(statistics.getMin(), statistics
                    .getMax(), state.getDataKey() == null ? 10 : state.getDataKey().size());
            for (Iterator<Double> it = state.iterator(locator, Double.class); it.hasNext();) {
                Double d = it.next();
                if (d != null) {
                    histogram.add(d);
    ...
    public ObservationReference createArtifactDescriptor(IObservation observation, IObservation parent, ILocator locator, int childLevel, boolean collapseSiblings, boolean isMain) {
        ...
        if (observation.getObservable().getUnit() != null) {
            ...
            ret.setLabel(ret.getLabel() + " [" + observation.getObservable().getRange().getLowerBound()
                    + " to "
        ...
        if (space != null) {
            ...
            ret.getMetadata()
                    .put("Total area", NumberFormat.getInstance()
                            .format(space.getShape().getArea(Units.INSTANCE.SQUARE_KILOMETERS))
                            + " km2");
            ...
            if (observation instanceof IState && space.isRegular() && space.size() > 1) {
                ...
                ret.getMetadata().put("Grid size", grid.getCellCount() + " (" + grid.getXCells() + " x "
                        + grid.getYCells() + ") cells");
                ret.getMetadata()
                        .put("Cell size", NumberFormat.getInstance().format(grid.getCellWidth()) + " x "
            ...
            /*
             * For now only one adapter is kept per format. Later we may offer the option by using a 
            ...
            for (IObservation child : ((IDirectObservation) observation).getChildren(IObservation.class)) {
                ...
                ret.getChildren()
                        .add(createArtifactDescriptor(child, observation, locator, childLevel > 0
                                ? childLevel--
                                : childLevel, collapseSiblings, false));
            }
        }
        ...
        if (observation instanceof IState) {
            ...
            if (observation.getScale().size() == 1) {
                ret.setLiteralValue(formatValue(observation.getObservable(), ((IState) observation)
                        .get(observation.getScale().getLocator(0))));
        ...
        if (adapterId == null) {
            ...
            if (adapter == null) {
                throw new IllegalArgumentException("unknown adapter " + adapterId
                        + " to export output format " + outputFormat);
            }
        }
