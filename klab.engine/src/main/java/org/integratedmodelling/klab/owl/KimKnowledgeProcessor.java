...
public enum KimKnowledgeProcessor {
    ...
    private @Nullable Concept buildInternal(final IKimConceptStatement concept, final Namespace namespace, ConceptStatement kimObject, final IMonitor monitor) {
        ...
        for (ParentConcept parent : ((KimConceptStatement) concept).getParents()) {
            ...
            if (concepts.size() == 1) {
                ...
                case INTERSECTION:
                    expr = OWL.INSTANCE.getIntersection(concepts, namespace
                            .getOntology(), ((Concept) concepts.get(0)).getTypeSet());
                    break;
                case UNION:
                    expr = OWL.INSTANCE.getUnion(concepts, namespace
                            .getOntology(), ((Concept) concepts.get(0)).getTypeSet());
                    break;
                case FOLLOWS:
        ...
        for (IKimConcept inherited : ((KimConceptStatement) concept).getTraitsInherited()) {
            ...
            if (trait == null) {
                monitor.error("inherited " + inherited.getName()
                        + " does not identify known concepts", inherited);
                return null;
            }
            try {
        ...
        for (ApplicableConcept link : concept.getSubjectsLinked()) {
            if (link.getOriginalObservable() == null && link.getSource() != null) {
                ...
                Observables.INSTANCE.defineRelationship(main, declare(link.getSource(), monitor), declare(link
                        .getTarget(), monitor));
            } else {
                // TODO
            }
    ...
    public @Nullable Observable declare(final IKimObservable concept, final IMonitor monitor) {
        ...
        if (concept.getNonSemanticType() != null) {
            Concept nsmain = OWL.INSTANCE
                    .getNonsemanticPeer(concept.getModelReference(), concept.getNonSemanticType());
        ...
        if (concept.getClassifier() != null || concept.getAggregator() != null) {
            ...
            IKimConcept modifier = concept.getAggregator() == null ? concept.getClassifier()
                    : concept.getAggregator();
            ...
            if (concept.getAggregator() != null) {
                ...
                IConcept classifiedType = Types.INSTANCE
                        .getTypeByTrait(ret, by, downTo, (Ontology) (Configuration.INSTANCE
                                .useCommonOntology() ? Reasoner.INSTANCE.getOntology()
                                        : null));
    ...
    private @Nullable Concept declareInternal(final IKimConcept concept, final IMonitor monitor) {
        ...
        Builder builder = new ObservableBuilder(main, (Ontology) (Configuration.INSTANCE.useCommonOntology()
        ...
        try {
            ...
            if (concept.getOperands().size() > 0) {
                ...
                ret = concept.getExpressionType() == Expression.INTERSECTION
                        ? OWL.INSTANCE.getIntersection(concepts, ret.getOntology(), concept.getOperands()
                                .get(0).getType())
                        : OWL.INSTANCE.getUnion(concepts, ret.getOntology(), concept.getOperands().get(0)
                                .getType());
            }
            ...
            ret.getOntology().define(Collections.singletonList(Axiom.AnnotationAssertion(ret
                    .getName(), NS.CONCEPT_DEFINITION_PROPERTY, concept.getDefinition())));
