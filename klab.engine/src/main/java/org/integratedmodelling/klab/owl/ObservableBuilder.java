...
public class ObservableBuilder implements IObservable.Builder {
    ...
    public Builder without(IConcept... concepts) {
        ...
        for (IConcept concept : concepts) {
            Pair<Collection<IConcept>, Collection<IConcept>> tdelta = Concepts.INSTANCE
                    .copyWithout(ret.traits, concept);
            ...
            Pair<Collection<IConcept>, Collection<IConcept>> rdelta = Concepts.INSTANCE
                    .copyWithout(ret.roles, concept);
    ...
    public Builder withoutAny(IConcept... concepts) {
        ...
        for (IConcept concept : concepts) {
            Pair<Collection<IConcept>, Collection<IConcept>> tdelta = Concepts.INSTANCE
                    .copyWithoutAny(ret.traits, concept);
            ret.traits = new ArrayList<>(tdelta.getFirst());
            ret.removed.addAll(tdelta.getSecond());
            for (int i = 0; i < tdelta.getSecond().size(); i++) {
                removedRoles.add(ComponentRole.TRAIT);
            ...
            Pair<Collection<IConcept>, Collection<IConcept>> rdelta = Concepts.INSTANCE
                    .copyWithoutAny(ret.roles, concept);
            ret.roles = new ArrayList<>(rdelta.getFirst());
            ret.removed.addAll(rdelta.getSecond());
            for (int i = 0; i < tdelta.getSecond().size(); i++) {
                removedRoles.add(ComponentRole.ROLE);
    ...
    public Concept makeOccurrence(IConcept concept, boolean addDefinition) {
        ...
        if (!concept.is(Type.DIRECT_OBSERVABLE)) {
            monitor.error("occurrences (probability of presence) can be observed only for subjects, events, processes and relationships", declaration);
        }
    ...
    public Concept makeProportion(IConcept concept, @Nullable IConcept comparison, boolean addDefinition, boolean isPercentage) {
        ...
        if (conceptId == null) {
            ...
            EnumSet<Type> newType = Kim.INSTANCE
                    .getType(isPercentage ? UnarySemanticOperator.PERCENTAGE.name()
    ...
    public Concept buildConcept() throws KlabValidationException {
        ...
        if (traits != null && traits.size() > 0) {
            ...
            for (IConcept t : traits) {
                ...
                if (Traits.INSTANCE.getTraits(main).contains(t)) {
                    monitor.error("concept " + Concepts.INSTANCE.getDisplayName(main)
                            + " already adopts trait "
                ...
                if (!baseTraits.add(base)) {
                    monitor.error("cannot add trait " + Concepts.INSTANCE.getDisplayName(t) + " to concept "
                            + main
                            + " as it already adopts a trait of type "
                            + Concepts.INSTANCE.getDisplayName(base), declaration);
                }
        ...
        if (inherent != null) {
            ...
            if (other != null && !Observables.INSTANCE.isCompatible(inherent, other)) {
                monitor.error("cannot set inherent type of " + Concepts.INSTANCE.getDisplayName(inherent)
                        + " to "
        ...
        if (compresent != null) {
            ...
            if (other != null && !Observables.INSTANCE.isCompatible(compresent, other)) {
                monitor.error("cannot set compresent type of " + Concepts.INSTANCE.getDisplayName(compresent)
                        + " to "
                        + Concepts.INSTANCE.getDisplayName(main)
        ...
        if (causant != null) {
            ...
            if (other != null && !Observables.INSTANCE.isCompatible(causant, other)) {
                monitor.error("cannot set causant type of " + Concepts.INSTANCE.getDisplayName(causant)
                        + " to "
                        + Concepts.INSTANCE.getDisplayName(main)
        ...
        if (adjacent != null) {
            ...
            if (other != null && !Observables.INSTANCE.isCompatible(adjacent, other)) {
                monitor.error("cannot set adjacent type of " + Concepts.INSTANCE.getDisplayName(adjacent)
                        + " to "
                        + Concepts.INSTANCE.getDisplayName(main)
        ...
        if (cooccurrent != null) {
            ...
            if (other != null && !Observables.INSTANCE.isCompatible(cooccurrent, other)) {
                monitor.error("cannot set co-occurrent type of "
        ...
        ret.getOntology()
                .define(Collections.singletonList(Axiom
        ...
        if (identities.size() > 0) {
            Traits.INSTANCE
                    .restrict(ret, Concepts.p(NS.HAS_IDENTITY_PROPERTY), LogicalConnector.UNION, identities);
        }
        if (realms.size() > 0) {
        ...
        if (attributes.size() > 0) {
            Traits.INSTANCE
                    .restrict(ret, Concepts.p(NS.HAS_ATTRIBUTE_PROPERTY), LogicalConnector.UNION, attributes);
        }
    ...
    private Ontology checkOntology() {

        Graph<IOntology, DefaultEdge> graph = new DefaultDirectedGraph<IOntology, DefaultEdge>(DefaultEdge.class);
        if (main != null) {
            graph.addVertex(main.getOntology());
        }
        for (IConcept c : traits) {
            graph.addVertex(c.getOntology());
        }
        for (IConcept c : roles) {
            graph.addVertex(c.getOntology());
        }
        if (inherent != null) {
            graph.addVertex(inherent.getOntology());
        }
        if (context != null) {
            graph.addVertex(context.getOntology());
        }
        if (caused != null) {
            graph.addVertex(caused.getOntology());
        }
        if (causant != null) {
            graph.addVertex(causant.getOntology());
        }
        if (compresent != null) {
            graph.addVertex(compresent.getOntology());
        }
        if (goal != null) {
            graph.addVertex(goal.getOntology());
        }
        if (cooccurrent != null) {
            graph.addVertex(cooccurrent.getOntology());
        }
        if (adjacent != null) {
            graph.addVertex(adjacent.getOntology());
        }

        if (graph.vertexSet().size() == 1) {
            return main.getOntology();
        }

        Set<IOntology> os = new HashSet<>(graph.vertexSet());
        for (IOntology a : graph.vertexSet()) {
            for (IOntology b : os) {
                if (a.getImports(true).contains(b)) {
                    graph.addEdge(b, a);
                }
            }
        }

        IOntology ret = main.getOntology();
        try {

            TopologicalOrderIterator<IOntology, DefaultEdge> sorter = new TopologicalOrderIterator<>(graph);
            while (sorter.hasNext()) {
                ret = sorter.next();
            }
        } catch (Exception e) {
//            System.out.println("CAZZO circular dependency in " + graph);
            for (IOntology o : graph.vertexSet()) {
                if (graph.incomingEdgesOf(o).size() == 0) {
                    ret = o;
                    break;
                }
            }
