...
public class Codelist implements ICodelist {
    ...
    public Codelist(CodelistReference reference) {
        ...
        this.rootConcept = reference.getRootConceptId() == null
                ? null
                : Concepts.c(reference.getRootConceptId());
        for (Pair<String, String> pair : reference.getDirectMapping().getMappings()) {
            direct.put(
                    Utils.asType(pair.getFirst(),
                            Utils.getClassForType(reference.getDirectMapping().getKeyType() == null
                                    ? Type.TEXT
                                    : reference.getDirectMapping().getKeyType())),
                    Utils.asType(pair.getSecond(),
                            Utils.getClassForType(reference.getDirectMapping().getValueType() == null
                                    ? Type.TEXT
        ...
        for (Pair<String, String> pair : reference.getInverseMapping().getMappings()) {
            reverse.put(
                    Utils.asType(pair.getFirst(),
                            Utils.getClassForType(reference.getInverseMapping().getKeyType() == null
                                    ? Type.TEXT
                                    : reference.getInverseMapping().getKeyType())),
                    Utils.asType(pair.getSecond(),
                            Utils.getClassForType(reference.getInverseMapping().getValueType() == null
                                    ? Type.TEXT
    ...
    public Object value(Object key) {
        ...
        Object value = isEmpty(ret) ? key : ret.iterator().next();
        if (value != null && getType() != null) {
            if (getType() == Type.CONCEPT) {
                value = Concepts.INSTANCE.getConcept(value.toString());
            } else {
                value = Utils.asType(value, Utils.getClassForType(getType()));
            }
        }
        return value;
    }
    ...
    public String key(Object value) {
        ...
        Collection<Object> ret = reverse.get(Utils.asType(value,
                reference.getInverseMapping().getKeyType() == null
                        ? String.class
