...
public class LocalDataBuilder implements IKlabData.Builder {
    ...
    public Builder startState(String name) {
        ...
        if (context.getArtifact(name) instanceof IState) {
            ...
            throw new IllegalArgumentException("cannot set the builder context to " + name
    ...
    public Builder startObject(String artifactName, String objectName, IScale scale) {
        ...
        if (observable == null) {
            throw new IllegalArgumentException("data builder: cannot find semantics for the artifact named "
                    + artifactName);
        }
        return new LocalDataBuilder((IDirectObservation) context
                .newObservation(observable, objectName, scale), this);
    }
