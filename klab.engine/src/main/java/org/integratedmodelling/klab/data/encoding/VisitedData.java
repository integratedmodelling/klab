...
public class VisitedData implements IKlabData {
	...
	public VisitedData(VisitingDataBuilder builder) {
		...
		for (Descriptor state : builder.states) {
			if (this.state == null) {
				throw new KlabUnimplementedException("states in visited resources");
//				this.state = new DataArtifact(state.artifactName, state., scale, data)
			} else {
				// TODO!
				throw new KlabUnimplementedException("states in visited resources");
			}
		...
		for (Descriptor object : builder.objects) {
			if (this.object == null) {
				this.object = new ObjectArtifact(object.name,
						object.scale instanceof IScale ? (IScale) object.scale : Scale.create(object.scale), object.metadata);
			} else {
