...
public enum Units implements IUnitService {
	...
	private Units() {
		...
		// he is doing
		UnitFormat.getInstance().label(SI.CELSIUS, "Celsius");
		UnitFormat.getInstance().label(NonSI.DEGREE_ANGLE, "degree_angle");
	...
	/**
	 * Analyze an observable in a scale and return the needed transformation to
	 * aggregate the values. If the transformation isn't stable, this will need to
	 * be repeated for each locator, otherwise the result can be reused within the
	 * same contextualization.
	 * 
	 * @return
	 */
