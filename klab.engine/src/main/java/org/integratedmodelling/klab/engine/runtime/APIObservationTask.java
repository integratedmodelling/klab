...
/**
 * Carries on observation requests received through the public API, handling
 * task orchestration and notification through the ticket system.
 * 
 * Handles both estimates and observations. If estimates are requested, the
 * results are entirely sent through the tickets.
 * 
 * @author Ferd
 *
 */
public class APIObservationTask extends Thread {
	...
	private Pair<Double, String> getUserCost(long klabCredits) {
		...
		/*
		 * TODO these must come from the user profile. Currency code KLB is legitimate
		 * for k.LAB credits, at least until we all go live in k.Land.
		 */
		String currency = "KLB";
	...
	/**
	 * Estimation of a task should compute the estimate in k.LAB credits, then
	 * convert into whatever the price of a credit is for the user. Each user should
	 * have their currency and a price per credit in that currency.
	 */
	private void estimate() {

		/*
		 * if price isn't fixed, compute the job size in k.LAB credits and assign a
		 * price.
		 */
		long cost = 0;
		...
		/*
		 * remember the estimate for charging if the estimate is accepted. The request
		 * will be stored with the estimate field set to false and the cost set to 0+.
		 */
	...
	/**
	 * Make the observation and return the actual cost computed during
	 * contextualization.
	 */
	private long observe() {
		...
		if (contextObservation == null && contextRequest != null) {
			IObservable contextObservable = Observables.INSTANCE.declare(contextRequest.getContextType());
			ITask<ISubject> contextObservationTask = session.observe(contextObservable, scale);
		...
		for (IObservable observable : observables) {
			...
			observationTask.addObservationListener((task, obs) -> {
				if (obs != null) {
					String artifacts = ticket.getData().get("artifacts") == null ? ""
							: ticket.getData().get("artifacts");
		...
		ticket.resolve("charge", "" + userCost.getFirst(), "currency", userCost.getSecond(), "actualcost",
				"" + actualCost);
