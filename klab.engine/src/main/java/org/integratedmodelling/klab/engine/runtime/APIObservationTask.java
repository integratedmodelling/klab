...
public class APIObservationTask extends Thread {
    ...
    // state computed during the observation, kept here to navigate the finals and the closures
    ...
    private APIObservationTask(ObservationRequest request, ISession session, IDirectObservation ctx,
            ITicket ticket) {
    ...
    public void run() {
        ...
        if ((contextRequest != null && contextRequest.isEstimate())
            ...
            Accounting.INSTANCE.charge(session.getUser(), estimatedCost < 0 ? actualCost : estimatedCost,
                    actualCost);
        }
    }
    ...
    private void estimate() {
        ...
        if (contextRequest != null) {
            ...
            cost += Accounting.INSTANCE.estimate(new ObservedConcept(contextObservable, Mode.RESOLUTION),
                    scale);
    ...
    private long observe() {
        ...
        // at the end this will either be the previously accepted estimated cost or the actual cost
        // in KLB
        ...
        ticket.resolve("charge", "" + userCost.getFirst(), "currency",
                userCost.getSecond(), "actualcost", "" + actualCost);
