...
public class SearchContext implements IIndexingService.Context {
    ...
    /*
     * we send this one up the chain of accepted matches to collect
     * the meaning so far.
     */
    public class Meaning {
        ...
        /**
         * True when we hit a semantic boundary, such as an infix operator or
    ...
    private SearchContext acceptToken(SearchMatch match) {
        ...
        if (match.getUnaryOperator() != null) {
            ...
            case PROPORTION:
                ...
                ret.constraints.add(Constraint.with(IKimConcept.Type.TRAIT));
                ret.constraints.add(Constraint.allTraits(false).applyingTo(IKimConcept.Type.TRAIT));
                break;
            case RATIO:
                // TODO second piece (over)
                ret.constraints.add(Constraint.with(IKimConcept.Type.QUALITY));
                ret.constraints.add(Constraint.allTraits(false).applyingTo(IKimConcept.Type.QUALITY));
                break;
            case TYPE:
                ret.constraints.add(Constraint.with(IKimConcept.Type.COUNTABLE));
                ret.constraints.add(Constraint.allTraits(false).applyingTo(IKimConcept.Type.OBSERVABLE));
                break;
            case UNCERTAINTY:
                ret.constraints.add(Constraint.with(IKimConcept.Type.QUALITY));
                ret.constraints.add(Constraint.allTraits(false).applyingTo(IKimConcept.Type.QUALITY));
            ...
            case INTERSECTION:
            case UNION:
                ret.constraints.add(Constraint.with(meaning.getSemantics()));
                ret.constraints.add(Constraint.allTraits(false).applyingTo(meaning.getSemantics()));
                break;
            case BY:
                break;
            default:
            ...
            case IN:
                ret.nextTokenType = meaning.getSemantics().contains(IKimConcept.Type.MONEY) ? TokenClass.CURRENCY
                        : TokenClass.UNIT;
                ret.constraints.add(Constraint.unit(meaning.getSemantics()));
                break;
            case OF:
                break;
            case PER:
            ...
            case SAMEAS:
                ...
                ret.constraints.add(Constraint.with(meaning.getSemantics()));
                ret.constraints.add(Constraint.value(meaning.getSemantics()));
                break;
            case TIMES:
                ret.nextTokenType = TokenClass.DOUBLE;
                break;
            ...
            case WITHOUT:
                if (Kim.INSTANCE.isNumeric(meaning.getSemantics())) {
                    ...
                    //	                ret.constraints.add(Constraint.traitsIncarnating(getAcceptedSemantics()));
                }
                break;
            default:
                break;
            ...
            if (meaning.getObservable() != null) {
                ret.constraints.add(Constraint.modifiersFor(meaning.getSemantics()));
            } else {
                // add traits with another base trait
                ret.constraints.add(Constraint.otherTraits(meaning.getTraits()).applyingTo(meaning.getSemantics()));
                ret.constraints.add(Constraint.allObservables(false).applicableTo(meaning.getTraits()));
            }
