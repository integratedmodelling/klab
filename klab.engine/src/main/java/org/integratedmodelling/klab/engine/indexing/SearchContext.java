...
public class SearchContext implements IIndexingService.Context {
    ...
    static class Constraint {
        ...
        public boolean filter(Match match) {
            ...
            if (this.semantics != null) {
                ...
                EnumSet<IKimConcept.Type> intersection = Kim
                        .intersection(((SearchMatch) match).getSemantics(), this.semantics);
    ...
    public SearchContext accept(Match m) {
        ...
        if (match.getUnaryOperator() != null) {
            ...
            /*
             * TRAIT constraint: we can put this in the else above, which will cause traits in the front only and inhibit trait definition after
             * the observable is set. Probably good design discipline although it may be annoying. 
             */
    ...
    private boolean haveObservable() {
        ...
        if (acceptedMatch.semantics != null
                && acceptedMatch.semantics.contains(IKimConcept.Type.OBSERVABLE)) {
            return true;
        }
    ...
    public static Context createNew(Set<Type> matchTypes, Set<org.integratedmodelling.kim.api.IKimConcept.Type> semanticTypes) {
        SearchContext ret = new SearchContext();
