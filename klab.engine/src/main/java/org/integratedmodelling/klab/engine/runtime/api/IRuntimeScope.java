...
public interface IRuntimeScope extends IContextualizationScope {
     ...
     * @return
     */
    IRuntimeScope getContextScope(Actuator actuator, IResolutionScope scope, IScale scale, IMonitor monitor);
     ...
     */
    IRuntimeScope createChild(IScale scale, IActuator target, IResolutionScope scope,
            IMonitor monitor);
    ...
    <T extends IArtifact> T resolve(IObservable observable, IDirectObservation scope, ITaskTree<?> task,
            Mode mode, IActuator parentActuator);
