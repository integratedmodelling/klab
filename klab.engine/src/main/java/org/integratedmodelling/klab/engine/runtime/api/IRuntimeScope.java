...
public interface IRuntimeScope extends IContextualizationScope {
	...
	/**
	 * Collects all the runtime data relative to an actuator: observation (with all
	 * its layers if state), overall scale, partial scale if any, and the
	 * actuator(s) that have this as target. Indexed by observable as some actuators
	 * (e.g. change or processes) will have a different target than their own
	 * observation.
	 * 
	 * @author Ferd
	 *
	...
	/**
	 * Get the actuator data for this observable. Used to streamline access to all
	 * runtime info.
	 * 
	 * @param observable
	 * @return
	 ...
	 *                 computation.
	 * @param target   the main target. If the actuator is partial, the target may
	 *                 be wrapped to handle the rescaling.
	 * @param scope
	 * @param monitor
	 * @param pattern
	 ...
	 * context. Only the names in the catalog that are known to the current actuator
	 * are added.
	 * 
	 * @param <T>
	 * @param cls
	...
	/**
	 * Scale of contextualization corresponding to the target(s) of this actuator;
	 * if actuator is a partial, the merged partial scale.
	 * 
	 * @param actuator
	 * @return
	 */
	...
	/**
	 * Incarnate an emergent pattern into an observation of the passed observable.
	 * Called by the reasoner upon detection of a known pattern when observations
	 * are made.
	 * 
	 * @param concept
	 * @param pattern
