...
public class EnginePublicController implements API.PUBLIC {
    ...
    public TicketResponse.Ticket contextRequest(@RequestBody ContextRequest request,
         ...
         */
        ITicket ticket = s.openTicket(request.isEstimate() ? ITicket.Type.ContextEstimate : ITicket.Type.ContextObservation,
                "url", CREATE_CONTEXT, "user", user.getUsername(), "geometry", request.getGeometry(), "urn",
    ...
    public TicketResponse.Ticket observationRequest(@RequestBody ObservationRequest request,
         ...
         * TODO estimation mechanism. For now everything is feasible and nothing is costed.
         */
        ITicket ticket = s.openTicket(request.isEstimate() ? ITicket.Type.ObservationEstimate : ITicket.Type.ObservationInContext,
                "url", OBSERVE_IN_CONTEXT, "user", user.getUsername(), "urn", request.getUrn(), "email", user.getEmailAddress(),
                "feasible", "true");
    ...
    public void exportData(@PathVariable String export, @RequestHeader(name = "Klab_Authorization") String session,
        ...
        case PROVENANCE_SIMPLIFIED:
            if (MediaType.APPLICATION_JSON_VALUE.equals(format)) {
                response.getWriter()
                        .write(((Provenance) obs.getScope().getProvenance()).getElkGraph(target == Export.PROVENANCE_FULL));
                done = true;
            } else if (MediaType.TEXT_PLAIN_VALUE.equals(format)) {
                // this will currently throw an unimplemented exception
                response.getWriter()
                        .write(((Provenance) obs.getScope().getProvenance()).getKimCode(target == Export.PROVENANCE_FULL));
                done = true;
            }
            break;
        case REPORT:
    ...
    private void outputGrid(IObservation obs, HttpServletResponse response, ILocator locator, boolean addStyle) {
        ...
        if (adapter != null) {
            ...
            if (importer != null) {
                try {
                    File temp = importer.exportObservation(File.createTempFile("obs", addStyle ? ".zip" : ".tiff"), obs, locator,
                            "tiff", Klab.INSTANCE.getRootMonitor());
                    if (temp.exists()) {
    ...
    private void outputImage(IObservation obs, HttpServletResponse response, Export target, String viewport, ILocator locator)
            throws IOException {
