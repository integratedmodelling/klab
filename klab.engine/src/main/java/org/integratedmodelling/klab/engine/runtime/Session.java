 ...
 * in Spring security.
 * 
 * TODO this must become a front-end for session actors, managed uniformly
 * within the runtime singleton.
 * 
 * @author ferdinando.villa
 *
 */
public class Session extends GroovyObjectSupport
		implements ISession, IActorIdentity<KlabMessage>, UserDetails, IMessageBus.Relay {
	...
	/*
	 * Tasks completed in this session. Allows for tracking completed tasks on the
	 * remote engine. Content may be a IScript or a ITask.
	 */
	...
	/**
	 * Store a completed task. It may be a ITask or a IScript, which only have the
	 * Future identity in common.
	 * 
	 * @param task
	 */
		...
		if (Urns.INSTANCE.isUrn(urnOrDummy)) {
			...
			if (ref != null) {
				...
				monitor.send(Message.create(this.token, IMessage.MessageClass.ResourceLifecycle,
						...
						((Resource) ref).getReference()).inResponseTo(message));
			} else {
				monitor.send(Message.create(this.token, IMessage.MessageClass.ResourceLifecycle,
						IMessage.Type.ResourceUnknown, urnOrDummy).inResponseTo(message));
			}
			return;
		}

		/*
		 * if message didn't contain a URN, send back a network descriptor with all
		 * nodes we can publish to at the moment of the call.
		 */
			...
			public void run() {
				...
				if (operation != null) {
					...
					res = adapter.getValidator().performOperation(resource, request.getOperation(),
							Parameters.create(request.getParameters()), Resources.INSTANCE.getCatalog(resource),
							rmonitor);
				}
		...
		if (request.getOperation() == CRUDOperation.CREATE) {
			...
			if (resource != null) {
				monitor.send(Message.create(token, IMessage.MessageClass.ResourceLifecycle,
						IMessage.Type.ResourceCreated, ((Resource) resource).getReference()).inResponseTo(message));
			}
			...
			if (codelist != null) {
				monitor.send(Message.create(token, IMessage.MessageClass.ResourceLifecycle,
						IMessage.Type.CodelistCreated, ((Codelist) codelist).getReference()).inResponseTo(message));
			}
	...
	private void handleAuthorityQueryRequest(AuthorityQueryRequest request, IMessage message) {
		...
		monitor.send(Message
				.create(this.token, IMessage.MessageClass.UserInterface, IMessage.Type.AuthoritySearchResults, ret)
				.inResponseTo(message));
	}
	...
	private void handleMatchAction(SearchMatchAction action, IMessage message) {
		...
		if (message.getType() == IMessage.Type.SemanticMatch) {
			...
			if (matches != null && matches.getMatches().size() > action.getMatchIndex()/* which would be weird */) {
