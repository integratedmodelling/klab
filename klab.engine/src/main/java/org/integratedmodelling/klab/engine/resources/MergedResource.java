...
/**
 * Built from a model statement with multiple URNs, either specifying models or resources. The local
 * resource catalog has a special table for these.
 * 
 * @author Ferd
 *
 */
public class MergedResource implements IResource {
    ...
    /*
     * Indexed by start time, it's still possible for the closest layer to be undefined
     */
    ...
    /**
     * Call with a model statement after verifying that more than one URN is mentioned. The
     * constructor automatically registers the resource with the local resource catalog.
     * 
     * TODO honor any @interpolation annotation on the model or anything else defining how the
     * resources are merged. (e.g. a default value when none responds - we could do the same with
     * normal models actually).
     * 
     * TODO build a temporal index like in file-based states and use that to pick the (set of)
     * resources @T.
     * 
     * @param statement
     */
    ...
    private void validate() {
        ...
        /*
         * default resolution if nothing can be understood from the data, which is very unlikely.
         */
    ...
    /**
     * Pick the specific resource(s) to use for the passed scale. TODO return a collection of
     * resources and use the next to either fill in for any nodata in the previous, or add/average.
     * 
     * Return an empty collection if there is no new information to be added to the passed artifact.
     * This should take into consideration having data to cover the specific timeslice we are being
     * asked, checking that the artifact was not already contextualized up to the time limit of the
     * last available resource.
     * 
     * TODO should take a storage parameter and return null when the resource has been already used
     * to contextualize the latest step, to prevent superfluous data access.
     * 
     * @param scale
     * @return
     */
    public ContextualizedResource contextualize(IScale scale, IArtifact artifact, IContextualizationScope scope) {
         ...
         * 
         * // TODO anchor the locator to the resolution time switch (this.resolution.getType()) {
         * case CENTURY: break; case DAY: break; case DECADE: break; case HOUR: break; case
         * MILLENNIUM: break; case MILLISECOND: break; case MINUTE: break; case MONTH: break; case
         * SECOND: break; case WEEK: break; case YEAR: break; default: break;
         * 
         * } } else
         */
    ...
    /**
     * Check if all resources are online. Called as a special case by the resource service.
     * 
     * @return
     */
