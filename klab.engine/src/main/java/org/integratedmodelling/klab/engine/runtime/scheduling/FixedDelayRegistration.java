...
class FixedDelayRegistration<T> extends OneShotRegistration<T> {
	...
	public FixedDelayRegistration(int rounds, Function<Long, T> callable, long delay, int scheduleRounds,
			int scheduleOffset, Consumer<Registration<?>> rescheduleCallback) {
	...
	public void run() {
		super.run();
		// TODO check for irregular intervals
