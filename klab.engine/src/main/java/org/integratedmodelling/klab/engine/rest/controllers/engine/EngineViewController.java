...
public class EngineViewController {
     ...
     */
    @RequestMapping(
            value = API.ENGINE.OBSERVATION.VIEW.DESCRIBE_OBSERVATION,
            method = RequestMethod.GET,
            produces = "application/json")
    @ResponseBody
    public IObservationReference describeObservation(Principal principal, @PathVariable String observation, @RequestParam(
            required = false) Integer childLevel, @RequestParam(
                    required = false) boolean collapseSiblings, @RequestParam(
                            required = false) String locator) {
     ...
     */
    @RequestMapping(
            value = API.ENGINE.OBSERVATION.VIEW.SUMMARIZE_OBSERVATION,
            method = RequestMethod.GET,
            produces = "application/json")
    @ResponseBody
    public StateSummary summarizeObservation(Principal principal, @PathVariable String observation, @RequestParam(
            required = false) String locator) {
    ...
    @RequestMapping(
            ...
            produces = "application/json")
    @ResponseBody
    public IObservationReference getObservationSiblings(Principal principal, @PathVariable String observation, @RequestParam(
            required = false) Integer offset, @RequestParam(required = false) Integer count, @RequestParam(
                    required = false) Integer childLevel, @RequestParam(required = false) String locator) {
     ...
     * proposed the format. At the moment it will scan the adapters and propose the observation, calling the
    ...
    public void getObservationData(Principal principal, @PathVariable String observation, @RequestParam(
            required = false) String viewport, @RequestParam(
                    required = false) String locator, @RequestParam ObservationReference.GeometryType format, @RequestParam(
                            required = false) String outputFormat, @RequestParam(
                                    required = false) boolean folder, HttpServletResponse response)
            throws Exception {
