...
public class KlabActor extends AbstractBehavior<KlabMessage> {
	...
	private void executeInstantiation(Instantiation code, Scope scope) {
		...
		if (code.getActions().size() > 0) {
			...
			for (Triple<IKActorsValue, IKActorsStatement, String> adesc : code.getActions()) {
				actions.matches.add(new Pair<Match, IKActorsStatement>(new Match(adesc.getFirst(), adesc.getThird()),
						adesc.getSecond()));
			}
	...
	private void executeCall(Call code, Scope scope) {
		 ...
		 */
		if (scope.valueScope != null) {
			scope.valueScope = Actors.INSTANCE.invokeReactorMethod(scope.valueScope, messageName, code.getArguments(),
					scope, identity);
			return;
		}
	...
	protected Behavior<KlabMessage> handleAddComponentToGroupMessage(AddComponentToGroup message) {
		...
		if (actorBehavior != null) {
			...
			if (semaphore != null) {
				...
				/*
				 * TODO send the view message to the view with the group definition from the
				 * view scope
				 */
	...
	protected Behavior<KlabMessage> handleLoadBehaviorMessage(Load message) {
		...
		if (message.forwardApplicationId != null) {
			...
			/*
			 * start running anything that starts automatically in a thread and exit,
			 * otherwise we won't be able to run any other message to this actor until
		...
		new Thread() {
			...
			public void run() {
				...
				try {
					...
					boolean rootView = message.scope.viewScope == null ? false
