...
public class Grid extends Area implements IGrid {
	...
	/**
	 * A trivial grid mask with almost no memory footprint that automatically
	...
	public double getLinearResolutionMeters() {
		if (this.linearResolutionMeters_ == 0) {
			this.linearResolutionMeters_ = (getFirstCell().getStandardizedLength()
					+ getFirstCell().getStandardizedLength()) / 2.0;
		}
	 ...
	 * 
	 * Depending on the requested resolution and the configuration, this can change
	 * the envelope or just adapt the resolution to best fit the region context.
	 * 
