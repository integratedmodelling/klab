...
public class ConcaveHullInstantiator extends AbstractContextualizer implements IExpression, IInstantiator {
    ...
    public List<IObjectArtifact> instantiate(IObservable semantics, IContextualizationScope context) throws KlabException {
        ...
        if (exprDescriptor != null) {
            ...
            // if (exprDescriptor.isScalar(input) && context.getArtifact(input, IState.class) !=
            // null) {
        ...
        for (ILocator locator : context.getScale()) {
            ...
            if (fractionState != null) {
                ...
                o = Boolean.FALSE;
                double d = fractionState.get(locator, Double.class);
                if (!Double.isNaN(d)) {
                ...
                // }
                o = expression.eval(context, /* parameters, */ "scale", locator);
                if (o == null) {
                    o = Boolean.FALSE;
                }
        ...
        ret.add(context.newObservation(semantics, Observables.INSTANCE.getDisplayName(semantics) + "_0",
                Scale.substituteExtent(context.getScale(), Shape.create(hull, grid.getProjection())),
