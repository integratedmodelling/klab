...
public class ValueOperatorResolver implements IResolver<IState>, IProcessor, IExpression {
    ...
    public ValueOperatorResolver(IArtifact classified, ValueOperator operator, Object valueOperand, IArtifact stateOperand) {
    ...
    public IState resolve(IState ret, IContextualizationScope context) throws KlabException {
        ...
        if (stateOperand != null && !(stateOperand instanceof IState)) {
            throw new IllegalArgumentException("Operations are not possible if the second argument is not a literal or a state");
        }
        ...
        /*
         * TODO some values are extensive. Others aren't. Put this check under Observables after
         * it's all understood.
         */
        ...
        for (ILocator locator : context.getScale()) {
            ...
            case TOTAL:
                ...
                break;
            case WHERE:
                if (Observations.INSTANCE.isNodata(other) || (other instanceof Boolean && !((Boolean) other))) {
                    value = null;
                }
                break;
            case WITHOUT:
