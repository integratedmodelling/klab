...
public class Shape extends AbstractExtent implements IShape {
    ...
    public static Shape create(Envelope envelope) {
        return create(envelope.getMinX(), envelope.getMinY(), envelope.getMaxX(), envelope
                .getMaxY(), (Projection) envelope.getProjection());
    }
    ...
    public static Shape create(Collection<Geometry> geometries, IProjection projection) {
        ...
        return create(Geospace.gFactory
                .createGeometryCollection(geometries.toArray(new Geometry[geometries.size()])), projection);
    }
    ...
    public Shape transform(IProjection otherProjection) throws KlabValidationException {
        ...
        try {
            g = JTS.transform(geometry, CRS
                    .findMathTransform(projection.crs, ((Projection) otherProjection).crs));
        } catch (Exception e) {
    ...
    private void parseWkt(String s) throws KlabValidationException {
        ...
        try {
            ...
            throw new KlabValidationException("error parsing " + (wkt ? "WKT" : "WBT") + ": "
                    + e.getMessage());
        }
    ...
    public Shape simplifyIfNecessary(int maxCoordinates, int nDivisions) {
        if (geometry.getNumPoints() > maxCoordinates) {
            double distance = Math
                    .sqrt(Math.pow(getEnvelope().getWidth(), 2) + Math.pow(getEnvelope().getHeight(), 2))
                    / (double) nDivisions;
    ...
    public static Shape join(IShape a, IShape b, IShape.Type type, boolean includeSources) {
        ...
        switch (type) {
        case LINESTRING:
            merged = aj.getFactory()
                    .createLineString(new Coordinate[] {
                            aj.getCentroid().getCoordinates()[0],
