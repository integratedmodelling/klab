...
public class PolylineDecoder {
	 ...
	 * @return
	 */
	public static Shape decode(String encoded, double precision) {

		List<Coordinate> track = decodeBase64(encoded,precision);
	...
	/**
	 * Decodes a route with multiple stopping points, each trip leg is an element of the encoded list. 
	 * 
	 * @param encoded
	 * @param precision
	 * @return
	 */
	public static Shape decode(List<String> encoded, double precision) {
		
		List<Coordinate> track = encoded.stream().map(string -> decodeBase64(string, precision)).flatMap(List::stream).collect(Collectors.toList());
		LineString linestring = Geospace.gFactory.createLineString(track.toArray(new Coordinate[track.size()]));

		return Shape.create(linestring, Projection.getLatLon());
	}
	
	private static List<Coordinate> decodeBase64(String encoded, double precision){
	...
	public static void main(String[] args) {
		System.out.println(decode("yjq~FprhvO`qDbIfuEogJ", 1E5));
		...
		System.out.println(decode(list,1E6));
	}
