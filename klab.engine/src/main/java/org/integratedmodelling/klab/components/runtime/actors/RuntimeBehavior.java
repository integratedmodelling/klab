...
public class RuntimeBehavior {
		...
		void run(KlabActor.Scope scope) {
			...
			if (!arguments.getUnnamedKeys().isEmpty()) {
				...
				identity.getParentIdentity(Session.class).getState().submit(
						getUrnValue(arguments.get(arguments.getUnnamedKeys().get(0)), scope), (task, observation) -> {
							if (observation == null) {
			...
			new Thread() {
				...
				public void run() {
					...
					try {
						File file = null;
						if (dtabs) {
							file = TableArtifact.exportMultiple(
									identity.getParentIdentity(Session.class).getState().getTables(), file);
						} else {
	...
	public static class When extends KlabActionExecutor {
		...
		public void dispose() {
			scope.getMonitor().getIdentity().getParentIdentity(ISession.class).getState()
					.removeGlobalListener(this.listener);
		}
	}
