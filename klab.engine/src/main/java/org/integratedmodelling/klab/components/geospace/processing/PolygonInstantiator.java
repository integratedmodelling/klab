 ...
 * @deprecated as much slower than native JAI vectorizer. Keep for situations where native code
 *             can't be run (e.g. when SPI calls are made and a Spring jar breaks them).
 */
public class PolygonInstantiator implements IExpression, IInstantiator {
    ...
    public PolygonInstantiator(IParameters<String> parameters, IContextualizationScope context) throws KlabValidationException {
    ...
    public List<IObjectArtifact> instantiate(IObservable semantics, IContextualizationScope context) throws KlabException {
        ...
        if (skipped > 0) {
            context.getMonitor().info("feature extractor: skipped " + skipped + " features not conformant with passed options");
        }
