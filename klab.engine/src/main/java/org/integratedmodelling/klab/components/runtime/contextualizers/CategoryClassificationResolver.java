    ...
    public IState resolve(IState ret, IContextualizationScope context) throws KlabException {
        ...
        if (!(classified instanceof IState) || !(classfc instanceof IState)) {
            throw new IllegalArgumentException(
                    "Category classification is not possible: state for " + classifier
                            + " not found or not a state");
        }
        ...
        for (ILocator locator : context.getScale()) {
            ...
            if (aggregator == null) {
                aggregator = new Aggregator(ret.getObservable(), context.getScale());
        ...
        for (ILocator locator : ret.getScale()) {
            ...
            if (aggregator != null) {
                ret.set(locator, totals.get(data));
            }
        }

        /*
         * TODO set the table into the documentation outputs TODO add different tables according
         * to @summarize annotations or other options
         */
    ...
    private void addDocumentationTags(Map<Object, Object> cache, IState classifier) {
        ...
        for (Object key : cache.keySet()) {
            ...
            for (Object item : new Object[]{
                    key instanceof IConcept
                            ? Concepts.INSTANCE.getDisplayLabel((IConcept) key)
                            : key.toString(),
        ...
        docTags.add(new IDocumentationProvider.Item(){
            ...
            public String getTitle() {
                return Concepts.INSTANCE.getDisplayName(((IState) classified).getObservable())
                        + " aggregated by "
