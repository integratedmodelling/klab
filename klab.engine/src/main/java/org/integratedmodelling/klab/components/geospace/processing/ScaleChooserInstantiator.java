...
/**
 * Abstract instantiator that uses a list of increasingly detailed spatial resources and chooses the
 * one with extents that are most likely to incarnate the object in the specific region. Any spatial
 * URNs supplied should normally have the intersect=false parameter to prevent breaking up shapes to
 * fit the context.
 * <p>
 * Resources must be supplied in a <b>sorted</b> list from coarser to finer scaled.
 * 
 * @author Ferd
 *
 */
public abstract class ScaleChooserInstantiator extends AbstractContextualizer implements IInstantiator {
    ...
    /**
     * If true, always cover the context completely: either by choosing a larger object or by adding
     * even small proportion coverage with potentially very large objects.
     */
    ...
    public List<IObjectArtifact> instantiate(IObservable semantics, IContextualizationScope context) throws KlabException {
        ...
        if (chosen == null) {
            context.getMonitor().warn("k.LAB resources for " + Concepts.INSTANCE.getDisplayName(semantics.getType())
                    + " did not respond or did not match the context");
            return ret;
        }
        ...
        if (chosen < getResourceUrns().length) {
            ...
            IKlabData data = Resources.INSTANCE.getResourceData(getResourceUrns()[chosen], new VisitingDataBuilder(),
                    IArtifact.Type.OBJECT, "result", context.getScale(), context.getMonitor());
        ...
        context.getMonitor()
                .debug("Scale-dependent instantiator selected " + keep.size() + " objects out of a pool of " + tmp.size());
        ...
        for (Triple<String, IScale, IMetadata> data : keep) {
            ...
            if (boundingBox) {
                ...
                scale = Scale.substituteExtent(scale, grid == null ? bbox : Space.create((Shape) bbox, grid, alignGrid));
            }
