...
public class RuntimeContext extends Parameters<String> implements IRuntimeContext {
    ...
    public IConfiguration newConfiguration(IConcept configurationType, Collection<IObservation> targets, IMetadata metadata) {
        ...
        ResolutionScope scope = Resolver.INSTANCE
                .resolve(observable, this.resolutionScope, Mode.RESOLUTION, scale, model);
    ...
    public ICountableObservation newObservation(IObservable observable, String name, IScale scale, IMetadata metadata)
        ...
        if (dataflow == null) {
            ...
            ResolutionScope scope = Resolver.INSTANCE
                    .resolve(obs, this.resolutionScope, Mode.RESOLUTION, scale, model);
            if (scope.getCoverage().isRelevant()) {
        ...
        ret = (ICountableObservation) dataflow.withMetadata(metadata)
                .run(scale, ((Monitor) monitor).get(subtask));
        if (ret != null) {
    ...
    @Override
    public IRelationship newRelationship(IObservable observable, String name, IScale scale, IObjectArtifact source, IObjectArtifact target, IMetadata metadata) {

        if (!observable.is(Type.RELATIONSHIP)) {
            throw new IllegalArgumentException("RuntimeContext: cannot create a relationship of type "
                    + observable.getType());
        }
        ...
        if (dataflow == null) {
            ...
            ResolutionScope scope = Resolver.INSTANCE
                    .resolve(obs, this.resolutionScope, (Subject) source, (Subject) target, scale, model);
        ...
        if (actuator.getScale() != null) {
            ...
            if (actuator.isPartition()) {
                ...
                if (mergingActuator == null) {
                    throw new IllegalStateException("internal: cannot find merging actuator named "
                ...
                if (merging instanceof IState) {
                    ...
                    ret.target = Observations.INSTANCE
                            .getStateView((IState) merging, actuator.getScale(), ret);
                } else {
                    ret.target = merging;
                }
    ...
    public IRuntimeContext createContext(IScale scale, IActuator actuator, IResolutionScope scope, IMonitor monitor) {
        ...
        ret.targetName = ((Actuator) actuator).isPartition() ? ((Actuator) actuator).getPartitionedTarget()
                : actuator.getName();
        ret.resolutionScope = (ResolutionScope) scope;
        ret.artifactType = Observables.INSTANCE
                .getObservableType(((Actuator) actuator).getObservable(), true);
        ret.scale = scale;
        ret.semantics = new HashMap<>();
    ...
    public IArtifact createTarget(Actuator actuator, IScale scale, IResolutionScope scope, IDirectObservation rootSubject) {
        ...
        if (/* this.catalog.get(actuator.getName()) == null && */ !actuator.isPartition()) {
            targetObservables
                    .put(actuator.getName(), new Pair<>(actuator.getObservable(), actuator.getMode()));
        }
        ...
        if (actuator.getModel() != null) {
            for (int i = 1; i < actuator.getModel().getObservables().size(); i++) {
                ...
                targetObservables
                        .put(output.getLocalName(), new Pair<>((Observable) output, output.is(Type.COUNTABLE)
                                ? Mode.INSTANTIATION
                                : Mode.RESOLUTION));
            }
        }
        ...
        for (IComputableResource computation : actuator.getComputation()) {
            if (computation.getTarget() != null
                ...
                targetObservables.put(computation.getTarget()
                        .getLocalName(), new Pair<>((Observable) computation.getTarget(), computation
                                .getComputationMode()));
            }
        }
        ...
        for (String name : targetObservables.keySet()) {
            ...
            if (observable.is(Type.COUNTABLE) && mode == Mode.INSTANTIATION) {
                ...
                if (observable.is(Type.RELATIONSHIP)) {
                    observation = DefaultRuntimeProvider.createRelationship(observable, scale, actuator
                            .getDataflow()
                            .getRelationshipSource(), actuator.getDataflow().getRelationshipTarget(), this);
                } else {
                ...
                if (parent != null && actuator.getDataflow().getModel() != null) {
                    for (String attr : actuator.getDataflow().getModel().getAttributeObservables().keySet()) {
                        ...
                        if (metadata != null) {
                            ...
                            if (obj != null) {
                                IState state = (IState) DefaultRuntimeProvider
                                        .createObservation(actuator.getDataflow().getModel()
                                                .getAttributeObservables().get(attr), scale, this);
                        ...
                        if (!done) {
                            ...
                            if (artifact instanceof IState) {
                                ...
                                IState stateView = Observations.INSTANCE.getStateViewAs(actuator.getDataflow()
                                        .getModel().getAttributeObservables()
                                        .get(attr), (IState) artifact, scale, this);
