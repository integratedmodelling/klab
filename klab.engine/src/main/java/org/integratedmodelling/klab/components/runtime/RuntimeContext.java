...
public class RuntimeContext extends Parameters<String> implements IRuntimeContext {
    ...
    public ICountableObservation newObservation(IObservable observable, String name, IScale scale)
        ...
        // TODO this resolution must consider and compile in all the states that are defined through the attributes and/or values set in the
        // instantiator. The dataflow must create those from the specs.
        ...
        if (scope.getCoverage().isRelevant()) {
            ...
            // attributes or computables in the context. FIXME this must be passed to
            // the resolver for incorporation before the resolution is done.
    ...
    @Override
    public IRelationship newRelationship(IObservable observable, String name, IScale scale, IObjectArtifact source, IObjectArtifact target) {
    ...
    public IArtifact createTarget(Actuator actuator, IScale scale, IResolutionScope scope, IDirectObservation rootSubject) {
        ...
        for (String name : targetObservables.keySet()) {
            ...
            if (observable.is(Type.COUNTABLE) && mode == Mode.INSTANTIATION) {
                ...
                observation = DefaultRuntimeProvider.createRelationship(observable, scale, scope
                        .getRelationshipSource(), scope.getRelationshipTarget(), this);
                ...
                /*
                 * TODO if current model is an instantiator, run post-instantiation actions (e.g. states from the instantiator model) before
                 * resolution
                 * 
                 * No - when it gets here model is null (already resolving) or the resolver. Must lookup an instantiator upstream before the 
