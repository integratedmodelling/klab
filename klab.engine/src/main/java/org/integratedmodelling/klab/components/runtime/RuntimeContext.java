...
public class RuntimeContext extends Parameters<String> implements IRuntimeContext {
    ...
    public IRuntimeContext createChild(IObservable indirectTarget) {
        ...
        // if appropriate, the target has been created by the upstream createChild(Actuator), so we just set
        // it from the catalog.
    ...
    public IRelationship newRelationship(IObservable observable, String name, IScale scale, IObjectArtifact source, IObjectArtifact target) {
        ...
        ResolutionScope scope = Resolver.INSTANCE
                .resolve(obs, this.resolutionScope, (Subject) source, (Subject) target, scale);
    ...
    public IArtifact createTarget(Actuator actuator, IScale scale, IResolutionScope scope, IDirectObservation rootSubject) {
        ...
        } else if (actuator.getObservable().is(Type.COUNTABLE) && rootSubject != null
        ...
        for (IComputableResource computation : actuator.getComputation()) {
            if (computation.getTarget() != null && !computation.getTarget().is(Type.COUNTABLE)
                ...
                targetObservables
                        .put(computation.getTarget().getLocalName(), (Observable) computation.getTarget());
            }
        }
