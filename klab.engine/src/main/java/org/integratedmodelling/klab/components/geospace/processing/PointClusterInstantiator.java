...
public class PointClusterInstantiator extends AbstractContextualizer implements IExpression, IInstantiator {
    ...
    public PointClusterInstantiator(IParameters<String> parameters, IContextualizationScope context)
        ...
        if (!(scale.isSpatiallyDistributed() && scale.getDimension(Type.SPACE).size() > 1
            ...
            throw new KlabValidationException("point clustering only works on regular distributed spatial extents (grids)");
        }
        ...
        if (parameters.containsKey("radius")) {
            this.radius = context.getScale().getSpace().getEnvelope().metersToDistance(parameters.get("radius", Double.class));
...
//
                ...
                if (!(o instanceof Boolean)) {
                    throw new KlabValidationException("cluster extractor: feature extraction selector must return true/false");
                }
