...
public class ObjectClassificationResolver implements IResolver<IState>, IProcessor, IExpression, IDocumentationProvider {
    ...
    public static IServiceCall getServiceCall(IObservable classified, IConcept classifier, Set<ValueOperator> modifiers)
        ...
        return KimServiceCall.create(FUNCTION_ID, "observable", new ObservedConcept(classified), "classifier", classifier);
    }
    ...
    public IState resolve(IState ret, IContextualizationScope context) throws KlabException {
        ...
        if (glocator == null && grid != null) {
            ...
            /*
             * TODO subscribe to artifact to revise the locator if objects are added or removed. The
             * slow and easy solution would be to just set a flag and if it's true recreate the
             * locator even if it's not null.
             */
