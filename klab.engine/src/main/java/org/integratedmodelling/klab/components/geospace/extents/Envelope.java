...
public class Envelope implements IEnvelope {
     ...
     * multiplier of 4.
     * 
     * @param roundTo meters to round to. Also the minimum resolution if we can't get enough
     *  screen pixels. 
    ...
    /**
     * Return a "good" grid resolution by zoom level including both the 
     * actual meters per pixel and the readable unit string for it. For defaults and used when 
     * observing raw datasets. Reasoning: give the envelope enough resolution to show enough 
     * square pixels to occupy half the screen by computing the grid size
     * against the m/pixel for the zoom level.
     * <p>
     * Meters per pixel at zoom level z are equal to 156412/(2^z).
     * 
     * @param roundTo meters to round to. Also the minimum resolution if we can't get enough
     *  screen pixels. 
     * @param multiplier multiplier for the literal resolution. A good value for modern screens is around 4.
     * @return resolution and the correspondent unit string.
     */
