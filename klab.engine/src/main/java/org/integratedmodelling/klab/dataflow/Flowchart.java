...
public class Flowchart {
    ...
    private Element compileActuator(Actuator actuator, Element parent) {
        ...
        /*
         * Compile the outputs first: if a resolver, one output per actuator contained in the graph
         * plus any model output if there is a model; otherwise, put ports for all model outputs
         * except processes and only connect those that get used.
         */
    ...
    private Element compileComputation(Pair<IServiceCall, IContextualizable> computation, Element parent,
        ...
        if (computation.getSecond().getServiceCall() != null) {
            ...
            analyzeServiceCall(computation.getSecond().getServiceCall(), context, ret, parent,
                    computationTarget,
            ...
            Logging.INSTANCE
                    .warn("INTERNAL: unhandled computation in dataflow graph: " + computation.getSecond());
        }
        ...
        if (prototype != null) {
            ...
            for (Argument arg : prototype.listImports()) {
                ...
                if (name != null && (elementsByName.containsKey(name)
                        || parent.datapaths.containsKey(arg.getName()))) {
                ...
                if (importArgs.contains(arg)) {
                    if (parameter instanceof IConcept || parameter instanceof IObservable) {
                        ...
                        for (IActuator dependency : context.getActuators()) {
                            if (concept.getSemanticDistance(
                                    ((Actuator) dependency).getObservable().getType()) >= 0) {
