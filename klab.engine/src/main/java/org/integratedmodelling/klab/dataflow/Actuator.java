...
public class Actuator implements IActuator {
	...
	/**
	 * Scope keeps status for all actuators during contextualization, so that
	 * clients can inquire.
	 * 
	 * @author Ferd
	 *
	...
	/**
	 * The catalog of reference name -> local name for all observables referenced in
	 * this actuator
	 */
	...
	/**
	 * Filled in after the dataflow is compiled, contains the name mappings that
	 * apply to this actuator only when the aliased name of a referenced artifact is
	 * different from its stable name in the catalog. Used to remap the names in
	 * each actuator before computation. These are kept here rather than collected
	 * while computing, because actuator are topologically sorted into one flat list
	 * before execution.
	 * 
	 */
	 ...
	 * produce the final list of contextualizers. Each list contains a service call
	 * and its local target name, null for the main observable.
	 */
	 ...
	 * to initialize them.
	 * 
	 * @param target the final artifact being computed. If this actuator handles an
	 *               instantiation, the passed target is null and will be set to the
	 *               first object in the result chain, or to the empty artifact if
	 *               no instances are created. In the end the method will always
	...
	public IArtifact compute(IArtifact target, IRuntimeScope scope) throws KlabException {
		 ...
		 * service calls, so it can survive dataflow serialization/deserialization. The
		 * second element is the contextualized resource, the third is the original,
		 * which is the one that must end up in the schedule.
		 */
		...
		for (Pair<IServiceCall, IContextualizable> service : computationStrategy) {
			...
			/*
			 * ensure the resource matches if needed. Don't do anything if we're scheduling
			 * an occurrent, as the contextualization won't be used.
			 */
		...
		for (Triple<IContextualizer, IContextualizable, IContextualizable> contextualizer : computation) {
			...
			/*
			 * run the contextualizer on its contextualized target (second element), unless
			 * we're contextualizing an occurrent. This may get a null and instantiate a new
			 * target artifact.
			 */
			...
			/*
			 * include the computed resource in the report. Empty resources will be
	 ...
	 * computes last, just in case overlaps happen. Note: All partitions of the same
	 * observable must go after the dependencies
	 * 
	 * @param actuator
	 * 
	 * @return
	 ...
	 * resolver was found). TODO ensure that @probe annotations are honored.
	 * 
