            ...
            session.getMonitor()
                    .send(Message.create(session
                            .getId(), IMessage.MessageClass.ObservationLifecycle, IMessage.Type.NewObservation, Observations.INSTANCE
                                    .createArtifactDescriptor((IObservation) ret, ctx.getContextObservation()
                                            .equals(ret) ? null
                                                    : ctx.getContextObservation(), ITime.INITIALIZATION, -1)
    ...
    public void notifyNewObservation(IObservation observation) {
        ...
        if (observation instanceof IState && computationStrategy.size() > 0) {
            ...
            if (lastResource.getClassification() != null || lastResource.getAccordingTo() != null) {
                ...
                if (observation instanceof IKeyHolder && ((ComputableResource) lastResource)
                        .getValidatedResource(LookupTable.class).isKey()) {
