...
public class Actuator implements IActuator {
    ...
    /*
     * The coverage is generic and is set to the coverage of the generating models. The dataflow will
     * set the actual scale of computation into partialScale just before calling compute().
     */
    ...
    public IArtifact compute(IArtifact target, IRuntimeContext runtimeContext) throws KlabException {
        ...
        for (Pair<IServiceCall, IComputableResource> service : computationStrategy) {
            ...
            if (((ComputableResource) service.getSecond()).getModifiedParameters() != null) {
                function.getParameters()
                        .putAll(((ComputableResource) service.getSecond()).getModifiedParameters());
            }
        ...
        if (!ret.isEmpty() && (mode == Mode.INSTANTIATION || ret instanceof IState)) {
            ...
            if (confdesc != null) {
                ...
                configuration = ctx.newConfiguration(confdesc.getFirst(), confdesc
                        .getSecond(), /* TODO metadata */ new Metadata());
            }
        }
            ...
            if (!input) {
                ...
                if (parent != null && !dataflow.wasNotified(parent)) {
                    ...
                    IObservationReference parentReference = Observations.INSTANCE
                            .createArtifactDescriptor(parent, parent
                                    .getContext(), ITime.INITIALIZATION, 0, false, isMain)
                            .withTaskId(ctx.getMonitor().getIdentity().getId());

                    session.getMonitor().send(Message.create(session
                            .getId(), IMessage.MessageClass.ObservationLifecycle, IMessage.Type.NewObservation, parentReference));
                ...
                for (int i = 0; i < notifiables.size(); i++) {
                    ...
                    if (!dataflow.wasNotified(obs)) {
                        ...
                        IObservationReference observation = Observations.INSTANCE
                                .createArtifactDescriptor(obs, parent, ITime.INITIALIZATION, 0, false, isMain
                                        && last)
                                .withTaskId(ctx.getMonitor().getIdentity().getId());

                        session.getMonitor().send(Message.create(session
                                .getId(), IMessage.MessageClass.ObservationLifecycle, IMessage.Type.NewObservation, observation));
    ...
    private IArtifact runContextualizer(IContextualizer contextualizer, IObservable observable, IComputableResource resource, IArtifact ret, IRuntimeContext ctx, IScale scale)
            throws KlabException {
        ...
        session.getMonitor().send(Message.create(session
                .getId(), IMessage.MessageClass.TaskLifecycle, IMessage.Type.DataflowStateChanged, state));
        ...
        if (contextualizer instanceof IStateResolver) {
            ...
            ret = Klab.INSTANCE.getRuntimeProvider()
                    .distributeComputation((IStateResolver) contextualizer, (IState) ret, addParameters(ctx, self, resource), scale
            ...
            List<IObjectArtifact> objects = ((IInstantiator) contextualizer)
                    .instantiate(observable, addParameters(ctx, self, resource));
            ...
            if (objects != null) {
                ...
                for (IObjectArtifact object : objects) {
                     ...
                     * continue
                     */
                    if (object instanceof ObservedArtifact
                            && ((ObservedArtifact) object).isMarkedForDeletion()) {
...
//        ret.setScale(this.getPartialScale() == null ? scale : this.getPartialScale());
            ...
            if (ret.getArtifact(input.getName()) != null) {
                ...
                for (Pair<IContextualizer, IComputableResource> mediator : mediation) {
                    ...
                    if (targetArtifactId.equals(input.getAlias())) {
                         ...
                         */
                        IArtifact mediated = runContextualizer(mediator.getFirst(), this.observable, mediator
                                .getSecond(), artifact, ret, ret.getScale());
    ...
    protected String encode(int offset) {
        ...
        return ret + ofs + (input ? "import " : "")
                + (isPartition() ? "partition" : getType().name().toLowerCase())
            ...
            for (int i = 0; i < computationStrategy.size(); i++) {
                ret += (nout == 0 ? (ofs + "   compute" + (cout < 2 ? " " : ("\n" + ofs + "     ")))
                        ...
                        + (computationStrategy.get(i).getSecond().getTarget() == null ? ""
                                : (" as "
                                        + computationStrategy.get(i).getSecond().getTarget().getLocalName()))
    ...
    public void notifyNewObservation(IObservation observation) {
        ...
        if (observation instanceof IState && computationStrategy.size() > 0) {
            IComputableResource lastResource = computationStrategy.get(computationStrategy.size() - 1)
                    .getSecond();
            if (lastResource.getClassification() != null || lastResource.getAccordingTo() != null) {
                if (observation instanceof IKeyHolder) {
                    ((IKeyHolder) observation).setDataKey(((ComputableResource) lastResource)
                            .getValidatedResource(IClassification.class));
                }
            } else if (lastResource.getLookupTable() != null) {
                if (observation instanceof IKeyHolder
                        && ((ComputableResource) lastResource).getValidatedResource(LookupTable.class)
                                .isKey()) {
                    ((IKeyHolder) observation)
                            .setDataKey(((ComputableResource) lastResource)
                                    .getValidatedResource(ILookupTable.class));
                }
            }
        }
