...
//	private Mode notificationMode = INotification.Mode.Normal;
	 ...
	 * temporal transitions with a schedule that also includes their child
	 * dataflows.
	 */
	...
	public IArtifact run(IScale scale, Actuator parentComputation, IMonitor monitor) throws KlabException {
		...
		IArtifact ret = null;
		try {
			ret = Klab.INSTANCE.getRuntimeProvider().compute(this, scale.initialization(), resolutionScope, monitor)
					.get();
		} catch (Throwable e) {
...
//	}
	...
	private void computeLocalNames(Actuator actuator, HashMap<String, String> hashMap) {
		...
		for (IActuator a : actuator.getSortedChildren(actuator, false)) {
			/*
			 * TODO CHECK: unsure if the names should propagate downstream, but I think
			 * they're just local to each actuator and re-imported at all levels if need be.
			 * If this changes, we should pass hashMap instead.
			 */
