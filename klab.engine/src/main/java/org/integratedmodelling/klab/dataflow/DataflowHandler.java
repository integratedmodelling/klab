 ...
 * 
 * The derivation from Parameters is not necessary here, but because this serves as base class for
 * AbstractContextualizationScope and Java has no multiple inheritance, we add it here for lack of
 * better options.
 * 
 * @author Ferd
 *
...
public class DataflowHandler extends Parameters<String> {
    ...
    /**
     * These are created and run during resolution, before the root dataflow exists. Normally used
     * to resolve abstract predicates. They are "orphans" when created, and the root dataflow will
     * adopt them as soon as it is created.
     */
    ...
    /**
     * Return or create the dataflow needed to resolve the passed observable in the passed mode,
     * scale, scope and context. Cache the result for the observable and coverage so that no
     * resolution will be required for compatible, previously computed ones. TODO the caching DOES
     * prevent better specific dataflows to be resolved after more generic ones, so it should be
     * conditional to an option.
     * 
     * @param observable
     * @param mode
     * @param scale
    ...
    /**
     * Return the synthesized actuator structure for a dataflow that was built incrementally by
     * successive resolutions. Compared to the original dataflow, this one won't contain
     * sub-dataflows and will report complete coverages. It is the graph that should be used for
     * serialization and visualization.
     * 
     * @return the root node and the full, normalized actuator graph. Because this is executed on
     *         the root dataflow, the root node is guaranteed unique.
     */
    ...
    public Dataflow getRootDataflow() {
        return rootDataflow;
    }
