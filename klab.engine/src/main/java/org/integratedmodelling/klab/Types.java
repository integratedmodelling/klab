 ...
 * <p> <ul> Straight classifications: the classified type is a class and it's classified
 * into its subclasses. In data annotations, this produces as many models as traits
 * exposed by the class, with classifiers that extract the trait from the subclasses.
 * Class must be abstract, subtypes may be but that will trigger further resolution.
 ...
 * observable it is inherent of. <li>Classification of direct non-trait and resolves to
 * observability (accepting only the deniable observability expressed as the concept
 * itself with/out 'no') without 'by' clause. <li>Classification of direct non-trait 'by'
 * trait. Creates trait contextualized 'of' (direct thing - must be compatible) 'within'
 * context and observes its observability. A further contextualized 'by x of z' should be
 * admitted - say by Temperature Level of Stream' that reverses the inherency and makes is
 * a Level of (Temperature within Stream) within (context). <li>Classification of quality
 * 'by' trait. If quantity by ordering, a 'use discretization' error will ensue unless
 * classify is discretize, in which case it will resolve to a categorized ranking rather
 * than a class. Otherwise <li>Classification of </ul> <p> All classifications
 * contextualize universals as qualities, so they are subject to the same rules of all
 * qualities: they must have an inherent object and if that is not the context of
...
public enum Types implements ITypeService {
    ...
    public void setExposedTraits(IConcept type, Collection<IConcept> exposedTraits) {
        OWL.INSTANCE.restrictSome(type, Concepts
                .p(NS.EXPOSES_TRAIT_PROPERTY), LogicalConnector.UNION, exposedTraits);
    }
    ...
    public IClassification createClassificationFromMetadata(IConcept rootClass, String metadataEncodingProperty, String metadataFormat)
        ...
        for (IKnowledge c : ret.getConcept().getSemanticClosure()) {
            ...
            if (metadataFormat != null && (o != null && !(o instanceof Double && Double.isNaN((Double) o)))) {
                ...
                default:
                    throw new KlabValidationException("cannot interpret value " + o + " as "
                            + metadataFormat);
                }
            }
