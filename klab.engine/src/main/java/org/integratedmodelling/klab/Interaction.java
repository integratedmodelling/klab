...
public enum Interaction implements IInteractionService {
    ...
    private InteractiveParameter getParameterDescriptor(IAnnotation annotation, IObservable observable, String parameter) {
        InteractiveParameter p = null;
        ...
        if (prototype != null) {
            ...
            throw new KlabUnimplementedException("Sorry, for now only known annotations can have interactive parameters.");
        }
        return p;
    }
    ...
    /**
     * Submit the passed parameters, wait for a user's response which will directly modify the the
     * passed resources. Returns the annotation parameters, which cannot be directly set into
     * resources, as triples (annotationId, parameterId, value.toString).
     * 
     * This one <b>may return null</b> if the user has chosen to cancel the computation. If instead
     * the choice is to proceed with default, it will either return an empty collection or all the
     * field with their default values.
     * 
     * @param resources
     * @param fields
     * @return a collection of parameters, modified or their defaults. If null is returned, the user
     *         has asked to cancel the run.
     */
    public Collection<Triple<String, String, String>> submitParameters(List<Pair<IContextualizable, List<String>>> resources,
        ...
        /*
         * Send the session ID as request ID so that multiple requests can be compounded at the
         * client side for a single response.
         */
    ...
    private Object parseValue(Object value, String key, IContextualizable resource) {
        ...
        if (resource.getServiceCall() != null) {
            ...
            if (argument != null) {
                found = true;
                value = argument.getType() == Type.VALUE
                        ? interpretValue(value)
                        : TypeUtils.convert(value, Utils.getClassForType(argument.getType()));
            }
        }
        if (!found) {
    ...
    private Object interpretValue(Object value) {
        if (value != null) {
            if (NumberUtils.encodesDouble(value.toString())) {
                value = Double.parseDouble(value.toString());
            } else if (value.toString().toLowerCase().equals("true") || value.toString().toLowerCase().equals("false")) {
