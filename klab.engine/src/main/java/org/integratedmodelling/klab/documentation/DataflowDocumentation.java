...
public enum DataflowDocumentation {
    ...
    /**
     * Build documentation for flowchart element after the dataflow has executed. Returns the
     * template in the element but may re-render it to add info that has become available after its
     * creation.
     * 
     * @param element
     * @param context
    ...
    /**
     * Build documentation for an element describing the passed actuator. This may use model
     * documentation templates if existing, or the default for the actuator. Called at
     * initialization before the dataflow is run.
     * 
     * @param element
     * @param actuator
     * @return
    ...
    /**
     * Build documentation for an element describing the passed computation. This may use specific
     * or default templates for services, tables etc.
     * 
     * @param element
     * @param resource
     * @return
    ...
    public String getDocumentation(Element element, Pair<IServiceCall, IContextualizable> resource, IContextualizationScope scope) {
        ...
        if (resource.getSecond().getServiceCall() != null) {
            ...
            String lang = resource.getSecond().getLanguage() == null
    ...
    private VelocityContext getContext(Pair<IServiceCall, IContextualizable> resource, IContextualizationScope scope) {
        ...
        if (resource.getSecond().getServiceCall() != null) {
            ...
            ret.put("language",
                    StringUtils.capitalize(resource.getSecond().getLanguage() == null
                            ? Extensions.DEFAULT_EXPRESSION_LANGUAGE
                            : resource.getSecond().getLanguage()));
