...
import org.integratedmodelling.klab.documentation.extensions.table.compilers.AdditionReductionTableCompiler.SData;
...
//    boolean colTotals = false;
    ...
    @Override
    public void initialize(Parameters<String> parameters, Map<?, ?> tableDefinition,
        ...
        if (parameters.contains("structure")) {
        ...
        if (parameters.contains("summary")) {
            ...
            // values can be sum, mean, std, variance, median, nodata, nodatap, count, nodatac, datac
        }
        ...
        if (this.sourceState == null || this.comparedStates == null || this.comparedStates.size() != 2) {
            throw new KlabIllegalArgumentException(
                    "Summarizing table compiler called with insufficient or wrong arguments");
        }
...
////        builder.setCell(rowReductions, colTotals, totals.reductions);
    ...
    private ITime getTime(ITime overall, Object object) {
        ...
        throw new KlabIllegalArgumentException(
                "pairwise table compiler: cannot infer temporal state from " + object);
    }
