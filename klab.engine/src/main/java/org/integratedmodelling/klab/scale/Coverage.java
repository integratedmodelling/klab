 ...
 * {@link #merge(ITopologicallyComparable, LogicalConnector)} according to coverage semantics. If
 * the coverage is used as a Scale, it is important that a {@link Scale#copy() copy} is made so that
 * any calls to merge() will perform as expected.
 * 
...
public class Coverage extends Scale implements ICoverage {
    ...
    private Coverage(Coverage original, List<Pair<IExtent, Double>> newcoverages, double gain, boolean adopt) {
        super(original.extents);
        this.coverage = Double.NaN;
        this.gain = gain;
        List<IExtent> adopted = new ArrayList<>();
        for (Pair<IExtent, Double> cov : newcoverages) {
            ...
            if (adopt) {
                ...
                } else if (this.getExtent(cov.getFirst().getType()) != null) {
                    adopted.add(this.getExtent(cov.getFirst().getType()));
                }
                this.setExtents(adopted);
            }
        }
    ...
    @Override
    public Coverage merge(ITopologicallyComparable<?> other, LogicalConnector how, MergingOption...options) {
        ...
        return new Coverage(this, newcoverages, gain, true);
    }

    @Override
    public Coverage mergeExtents(ICoverage other, LogicalConnector how) {
        ...
        return new Coverage(this, newcoverages, gain, false);
    }
    ...
    private Pair<IExtent, Double> mergeExtent(Dimension.Type type, IExtent other, LogicalConnector how) {
        ...
        if (how == LogicalConnector.UNION) {
            ...
            IExtent x = orig.equals(other)
                    ? other
                    : ((AbstractExtent) ((AbstractExtent) orig).getExtent()).mergeCoverage(((AbstractExtent) other).getExtent(),
