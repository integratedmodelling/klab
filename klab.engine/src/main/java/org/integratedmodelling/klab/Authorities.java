...
public enum Authorities implements IAuthorityService {
    ...
    private void createAuthority(Authority annotation, Class<?> cls) {
        if (IAuthority.class.isAssignableFrom(cls)) {
            ...
            Logging.INSTANCE
                    .error("Authority annotation " + annotation.id()
                            + " used with non-authority class: ignoring");
        }
    }
    ...
    public IAuthority getAuthority(String authorityId) {
        ...
        if (descriptor == null && authorityId.contains(".")) {
            String meta = Path.getFirst(authorityId, ".");
            if (authorities.containsKey(meta)) {
                AuthorityDescriptor mauth = authorities.get(meta);
                if (mauth.authority instanceof AgencyAuthority) {
                    IAuthority ret = ((AgencyAuthority) mauth.authority)
                            .getSubAuthority(authorityId.substring(meta.length() + 1));
                    if (ret != null) {
                        return ret;
                    }
                }
    ...
    public IAuthority.Identity getIdentity(String authorityId, String identityId) {
        ...
        if (ret == null) {
            for (INodeIdentity node : Network.INSTANCE.getNodesForAuthority(auth)) {
                ret = node.getClient().get(API.AUTHORITY.RESOLVE, AuthorityIdentity.class,
                        API.AUTHORITY.P_AUTHORITY,
     ...
     */
    @Override
    public String getAuthorityCode(IConcept c) {
        String ret = c.getMetadata().get(NS.CONCEPT_DEFINITION_PROPERTY, String.class);
        if (ret.contains(":")) {
           ret = Path.getLast(ret, ':');
        }
        return ret;
    }
