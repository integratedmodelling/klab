...
public enum Ontologies implements IOntologyService {
	 ...
	 * possible, return the top-level "fallback" ontology. Used to establish where
	 * to put derived concepts and restrictions so as to avoid circular dependencies
	 * in the un
	 * 
	 * @param targets
	 * @return
	...
	public Ontology getTargetOntology(Ontology fallback, Object... targets) {
		...
		/*
		 * candidate namespace must import all the others. If not, choose the
		 * highest-level reasoner ontology which imports all.
		 */
