...
public class Prioritizer implements IPrioritizer<ModelReference> {
    ...
    private void buildComparisonStrategy() {
        ...
        IMetadata strategy = scope.getResolutionNamespace() == null
                ? null
        ...
        /*
         * Resolution strategy contains the ordering of sequentially compared criteria and any
         * subjective traits we want to rank. This scans the strategy and builds the comparator
         * using the specified ordering, plus the definition of the aggregated subjective ranking
         * function.
         */
    ...
    /*
     * Compute a customized aggregation of two or more standard criteria. Aggregation is equally
     * weighted. These criteria have been given the same order in the strategy specifications.
     */
     ...
     * 
     * 100 = in observation scenario 75 = in same namespace as context 50-26 closer to same
     * namespace as context 25 = in same project as context 0 = non-private in other visible
     * namespace
     * 
     * TODO #3 should use a hierarchy criterion: something in namespace xxxx.yyyy is satisfied
     * preferentially by something in namespace xxxx unless criteria 1 or 2 are satisfied.
     * 
     * From the docs (still not implemented as such):
     * 
     * 1. Scenario 2. Private in same namespace 3. Public in same namespace 4. Public in parent
     * namespace (in order of distance) 5. Project private in same project 6. Public local or cached
    ...
    public double computeLexicalScope(ModelReference model, ResolutionScope context) {
        ...
        /*
         * between 25 and 50 is attributed to namespace being traceable in dependency chain.
         */
        int ret = 0;
        ...
        /*
         * Workspace priority exists if the model comes from the local workspace or from the same
         * workspace as the resolution.
         */
        boolean workspaceHasPriority = ws != null && (ws.getName().equals(Resources.INSTANCE.getLocalWorkspace().getName())
        ...
        /*
         * the criteria (now 0-50) above occupy 25 points on a partition decided by the workspace
         * criterion
         */
    ...
    /*
     * semantic distance. This makes sure that e.g. a matching abstract model is chosen only after a
     * concrete one is rejected.
     */
    public double computeSemanticDistance(ModelReference model, ResolutionScope context) {

        /*
         * list of traits in common. Don't check the trait value - assumed the same because of the
         * search strategy.
         */
        try {
    ...
    /*
     * trait concordance wrt context n = # of traits shared / #. of traits possible, normalized to
     * 100 TODO REIMPLEMENT AS APPROPRIATE - a compatibility rank
     * 
    ...
    /*
     * scale specificity -> total coverage of object wrt context (minimum of all extents?) <n> =
    ...
    private double[] computeScaleCriteria(ModelReference model, ResolutionScope context) {
        ...
        if (!idxss.containsKey(model)) {
            ...
            if (context.getCoverage().getTime() != null) {
                /*
                 * TODO do the same with time and take the minimum - or should this be the separate
                 * currency value ?
                 */
        ...
        if (model.getShape() != null) {
            ...
            if (space != null) {
                ...
                try {
                    ...
                    ret[1] = 100.0 * (intersection.getArea() / ((Shape) model.getShape()).getStandardizedGeometry().getArea());
                    ret[0] = 100.0 * (intersection.getArea() / cspace.getArea());
                } catch (Throwable t) {
                    ret[1] = 10;
                    context.getMonitor().warn("topology error in computing intersections: probable degenerate spatial extent: " + t.getMessage());
                }
            }
        }
        return ret;
    ...
    public static double[] computeTemporalCriteria(long modelStart, long modelEnd, ITime time) {
        ...
        /*
         * coverage: if non-grid, 100 for covered, 75 - [0-25] for partially covered, 50 - [0-25]
         * distance if covered in infinite tail from or to a single-point beginning or end. If grid,
         * covered.
         */
        // if (time.size() > 1) {
        //
        ...
        if (!mrange.contains(crange)) {
            ...
            if (mrange.isBounded() && crange.isBounded()) {
                ...
                // this subtracts from 25 if the model is in the future, but that shouldn't even get
                // here.
        ...
        /*
         * specificity differs by resolution type (even if generic) and is corrected by the order of
         * magnitude of the nearest multiplier. If the context is universal, return 50.
         */
        ...
        if (compare && crange.isBounded()) {
            ...
            if (time.size() > 1) {
                ...
                distanceFactor /= (time.getResolution().getType().getMilliseconds() * time.getResolution().getMultiplier());
         ...
         * overlapping. Else it's like space. Zero if grid goes with non-grid or the other way
         * around.
         */
    ...
    /*
     * network remoteness -> whether coming from remote KBox (added by kbox implementation) 100 ->
     * local 0 -> remote
     */
    ...
    /*
     * inherency -> level wrt observable: 100 = same thing-ness, specific inherency (model and
     * context are inherent to same thing) 66 = same thing-ness, non-specific inherency 33 =
     * different thing-ness, mediatable inherency
     * 
     * TODO level of inherency at highest level should be modulated by semantic distance between
     * object and context, with 100 reserved for inherent to exactly same object type.
     * 
     */
    ...
    /*
     * scale coherency -> coherency of domains adopted by context vs. the object n = # of domains
     * shared (based on the isSpatial/isTemporal fields) normalize to 100 TODO reimplement this with
     * the geometry
     */
    ...
    /*
     * scale coverage -> of scale in context (minimum of all extents? or one per extent?) 0 = not
     * scale-specific (outside scale will not be returned) (1, 100] = (scale ^ object context) /
     * scale
     */
    ...
    /*
     * subjective concordance = multi-criteria ranking of user-defined metadata wrt default or
    ...
    /*
     * evidence -> resolved/unresolved 100 = resolved from data or object source 75 = resolved but
     * requires dereification 50 = computed, no dependencies 0 = unresolved
     */
     ...
     * numerically with a 0-100 value. Each pair <v,w> of value and weight is interpreted as a
     * "benefit" criterion that contributes to the overall score in a proportion that depends on its
     * weight - the higher the weight, the higher the contribution. All scores are multiplied when
     * still in the [0 1] range. The combination of low values is guaranteed to make the value
     * lower.
     * 
     * It is meant for "benefit" criteria that correlate directly to value, and the weight of each
     * criterion defines the amount of change it can produce in the final index - higher weight,
     * higher importance (the criterion value only changes the (100 - w)/100 proportion of the [0 1]
     * interval). For the index to be meaningful, only indices that refer to exactly the same
     * criteria can be compared - the safe bet for missing values is to make them "average" by
     * giving them value 50.
     * 
     * @param values collection of pairs <value, weight> for each criterion. Both value and weight
     *        must be integers in the 0-100 range.
    ...
    /*
     * call to register ranks that were computed outside this object. Used for model data coming
     * from the remote search service.
     */
