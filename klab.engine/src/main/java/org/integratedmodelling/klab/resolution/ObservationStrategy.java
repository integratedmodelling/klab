...
/**
 * An observation strategy implements all context-dependent inferences on observables (those that
 * are context-independent are available through the configured {@link IObservableService}). It can
 * produce all the model observables that can resolve a stated one, taking into account mode of
 * resolution (for instantiation or resolution), attributes, roles and context, along with the
 * recipe on how to observe them and transform the resulting artifact if necessary. When iterated,
 * returns the original observable when appropriate, then, upon request, produces all the other
 * candidates. Each candidate includes one or more observables and optional computations;
 * non-trivial candidates (with more than one observable and/or any computation) produce an ad-hoc
 * model that is resolved recursively.
 * <p>
 * The observable reasoner can also infer all the actual dependencies of a model, based on the
 * model's stated ones and on its semantics, taking into account roles, abstract status,
 * relationship endpoints etc.
 * 
...
public class ObservationStrategy {
    ...
    /**
     * The observation strategy incarnated by this object. Used by the dataflow compiler to properly
    ...
    public enum Strategy {
        ...
        DIRECT,
        /**
         * Observable has been decomposed into a legitimate observable and one or more filters
         * provided by an attribute resolver, to be applied to it in sequence if resolving.
         */
        FILTERING,
        /**
         * Observable will be contextualized by resolving a set of objects to which the observable
         * is inherent, then applying a dereifying transformation chosen by the runtime.
         */
        DEREIFICATION,
    ...
    /**
     * The result will contain the trivial strategy + up to one alternative, which will be resolved
     * recursively.
     * 
     * @param observable
     * @param scope
    ...
    public static List<ObservationStrategy> computeStrategies(IObservable observable, IResolutionScope scope,
        ...
        /*
         * explore inherency first. If there is explicit inherency (of), we first check for equality
         * of inherency, i.e. X of Y within Y. In this case we just observe X. This just applies to
         * true observables: if the inherency is for an attribute (e.g. normalized of X) we move
         * forward.
         * 
         * If not, we leave the trivial strategy as is (to be resolved by a possible model) and add
         * X within Y, which will generate X of Y in the current context.
         */
        ...
        if (inherent != null) {
            ...
            /*
             * no as-is resolution, just use the operators. Otherwise it becomes messy to match
             * candidates, and the utility of allowing pre-modified source models is doubtful as it
            ...
            /*
             * if the observable was resolved before, use that so we don't have to translate names
             * when we compile the dataflow.
             */
            Observable previous = ((ResolutionScope) scope).getResolvedObservable(target, mode);
            if (previous != null) {
                target = previous;
            ...
            if (!attribute.is(Type.ABSTRACT) && observable.is(Type.COUNTABLE)) {
                 ...
                 * dataflow will discard the outputs that are not what is wanted, and tag others as
                 * irrelevant (hidden) if it's a first instantiation.
                 */
    ...
    /**
     * Any necessary computation to apply to the resulting artifact to produce the requested
     * observation
     */
    ...
    /**
     * Tells the model and the dataflow compiler how to handle the contextualization.
     * 
     * @return
     */
