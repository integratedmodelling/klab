...
public class ResolutionScope implements IResolutionScope {
    ...
    /**
     * The context model, if there, is the instantiator that has created the observation being resolved. Actions and
     * states will be compiled into the dataflow and affect the resolution of subsequent dependencies.
     * 
     * @return
     */
    ...
    public ResolutionScope getChildScope(LogicalConnector connector) {
        ...
        ret.coverage = connector == LogicalConnector.UNION ? Coverage.full(ret.coverage)
     ...
     * @return a new scope
     */
    public ResolutionScope getChildScope(Observable observable, Scale scale, Subject source, Subject target, IModel upstreamModel) {
    ...
    public ResolutionScope getChildScope(Observer observer) throws KlabException {
        ...
        if (observer.getBehavior().hasScale()) {
            ret = new ResolutionScope(Scale
                    .create(observer.getBehavior().getExtents(monitor)), 1.0, ret, false);
        } else {
            ret.coverage = new Coverage(this.coverage);
            ret.coverage.setCoverage(1.0);
        }
