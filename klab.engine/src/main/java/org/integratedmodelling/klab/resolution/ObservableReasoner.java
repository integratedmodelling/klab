...
/**
 * An observable reasoner implements all context-dependent inferences on observables (those that are
 * context-independent are available through the configured {@link IObservableService}). It can
 * produce all the model observables that can resolve a stated one, taking into account mode of
 * resolution (for instantiation or resolution) and context. When iterated, returns the original
 * observable when appropriate, then, upon request it produces all the others lazily.
 * 
 * It can also be used to infer all the actual dependencies of a model, based on the model's stated
 * ones and on its semantics, taking into account roles, abstract status, relationship endpoints
 * etc.
 * 
...
public class ObservableReasoner implements Iterable<Observable> {
  ...
  /**
   * Use this to compute the model's dependencies. Also pass the actual observable being resolved,
   * which may not be the same as the model's observable (due to possible abstract status of the
   * model).
   * 
   * @param model
   * @param observable
  ...
  /**
   * Same as {@link #ObservableReasoner(Observable, ResolutionScope)} when the scope is not
   * available.
   * 
   * @param original
   * @param mode
  ...
  /**
   * Use to compute all the alternative observables that can resolve an original one.
   * 
   * @param original
   * @param scope
   */
  ...
  /**
   * Return all the alternatives except the first, which is expected to be the original observable.
   * 
   * @throw {@link IllegalStateException} when called in a context where alternatives do not apply
  ...
  // private void addRoleImpliedDependency(IConcept implyingRole, IConcept impliedRole,
  // List<Pair<IConcept, Boolean>> ret) throws KlabException {
  //
  // /*
  // * Concrete ones generate a dependency on their applicable observable with the role.
  // */
  ...
  //
  // /*
  // * TODO check - only adding the dep if it's DIRECT or a non-abstract non-countable. Definitely
  ...
  // /*
  // * an abstract observable role fulfills the requested role if the requested role specializes
  // * the provided (adding it means that the model will check the concrete role and provide it as
  // * requested). For a concrete one the model must have a role that specializes the requested.
  // */
  ...
  private List<Observable> selectApplicableDependencies(List<IObservable> dependencies,
