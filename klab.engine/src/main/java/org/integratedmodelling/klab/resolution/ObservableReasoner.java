...
public class ObservableReasoner implements Iterable<CandidateObservable> {
     ...
     * 
     * Each candidate observable should be one or more observables, resolved in AND,
     * i.e. they form the dependencies of an ephemeral model of their observable.
     */
    ...
    private boolean transformationsComputed;
    ...
    private CandidateObservable getDefaultCandidate(Observable original, Mode mode) {
        ...
        if (original.getClassifier() != null) {
             ...
             * separate the two as before. This can coexist with operators; if so, we handle each
             * one at a time. 
             */
    ...
    /**
     * Add any aggregator or the like to an observable that needs it. Do only one per resolution - the
     * 'else' is mandatory!
     * 
     * DO NOT CHANGE THE ORDER - 'by' is always last, so it comes first.
     * 
    ...
    private List<CandidateObservable> inferModelDependencies(Model model) {
        ...
        // // we need the aggregator observation + the adapter after the state is
        // computed
        // }
