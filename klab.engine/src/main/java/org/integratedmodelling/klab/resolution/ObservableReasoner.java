...
public class ObservableReasoner implements Iterable<CandidateObservable> {
    ...
    public static class CandidateObservable {
        ...
        /**
         * If true, this is part of a group of contiguous observables 
         * representing a set of alternative observables in OR. We will resolve all the
         * observables, failing only if none is resolved, and 
         * contextualize the resulting contextualizers lazily
         * until fulfillment of the observation.
         */
        ...
        public void accept(IRankedModel model) {
            ...
            if (this.computation != null) {
                for (IComputableResource computable : computation) {
                    Klab.INSTANCE.getRuntimeProvider()
                            .setComputationTargetId(computable, model.getObservables().get(0).getLocalName());
                }
            }
        }
    ...
    public List<CandidateObservable> getAlternatives() {
        if (model != null) {
            throw new IllegalStateException("ObservableReasoner: getAlternatives() called when alternatives are not computed.");
        }
        ...
        if (observable.is(Type.PRESENCE)) {
            ...
            if (inherent != null && !scope.isBeingResolved(inherent, Mode.INSTANTIATION)) {
                ...
                if (dereificator != null) {
                    ret.add(new CandidateObservable(Observable
                            .promote(inherent), Mode.INSTANTIATION, dereificator));
                }
            }
    ...
    private List<CandidateObservable> selectApplicableDependencies(List<IObservable> dependencies, IResolutionScope context) {
        List<CandidateObservable> ret = new ArrayList<>();
        for (IObservable dep : dependencies) {
            ...
            // }
            // }
            ret.add(new CandidateObservable((Observable) dep, dep.is(Type.COUNTABLE) ? Mode.INSTANTIATION
                    : Mode.RESOLUTION));
        }
        return ret;
    }
