...
public enum Resources implements IResourceService {
    ...
    public boolean loadComponents(Collection<File> localComponentPaths, IMonitor monitor) {
        try {
            ...
            components = new ComponentsWorkspace("components", Configuration.INSTANCE
                    .getDataPath("workspace/deploy"), deployedComponents);
    ...
    */
    ...
    @Override
    public IResource createLocalResource(String resourceId, File file, IParameters<String> parameters, IProject project, String adapterType, boolean forceUpdate, boolean asynchronous, IMonitor monitor) {
    ...
    public IResource createLocalResource(ResourceCRUDRequest request, Monitor monitor) {
        ...
        monitor.info("Start importing resource " + request.getResourceUrns().iterator().next()
        ...
        IResource ret = importResource(urn, project, adapterType, null, parameters, Version
                .create("0.0.1"), new ArrayList<>(), monitor);
    ...
    private IResource importResource(String urn, IProject project, String adapterType, File file, IParameters<String> parameters, Version version, List<IResource> history, IMonitor monitor) {
        ...
        try {
            ...
            if (adapter != null) {
                ...
                Builder builder = validator
                        .validate(file == null ? null : file.toURI().toURL(), parameters, monitor);
                ...
                if (!builder.hasErrors() && file != null) {
                    resourceDatapath = new File(project.getRoot() + File.separator + "resources"
                            + File.separator + resourceDataDir);
                    resourceDatapath.mkdirs();

                    for (File f : validator.getAllFilesForResource(file)) {
                        FileUtils.copyFile(f, new File(resourceDatapath + File.separator
    ...
    private FutureResource importResourceAsynchronously(String urn, IProject project, String adapterType, File file, IParameters<String> parameters, Version version, List<IResource> history, IMonitor monitor) {
    ...
    public Collection<IResource> importResources(URL source, IProject project, @Nullable String adapterType) {
        ...
        for (IResourceAdapter adapter : resourceAdapters.values()) {
            if ((adapterType == null || adapter.getName().equals(adapterType))
                    && adapter.getImporter() != null
        ...
        for (IResourceAdapter adapter : importers) {
            ...
            for (Builder builder : importer
                    .importResources(source.toString(), parameters, Klab.INSTANCE.getRootMonitor())) {
                ...
                if (!builder.hasErrors()) {
                    ...
                    File resourceDatapath = new File(project.getRoot() + File.separator + "resources"
                            + File.separator
                    ...
                    for (File f : builder.getImportedFiles()) {
                        ...
                        builder.addLocalResourcePath(project.getName() + "/resources/"
                                + builder.getResourceId() + "/"
                                + MiscUtilities.getFileName(f));
                    }
                }

                String owner = Authentication.INSTANCE.getAuthenticatedIdentity(IUserIdentity.class)
                        .getUsername();
    ...
    public Pair<IArtifact, IArtifact> resolveResourceToArtifact(String urn, IMonitor monitor) {
        ...
        IObservable observable = Observable
                .promote(OWL.INSTANCE.getNonsemanticPeer("Artifact", resource.getType()));
        IKlabData data = getResourceData(resource, urnp.getSecond(), scale, context
                .getChild(observable, resource));
    ...
    public IKlabData getResourceData(String urn, IKlabData.Builder builder, IMonitor monitor) {
        ...
        if (adapter == null) {
            throw new KlabUnsupportedFeatureException("adapter for resource of type "
                    + resource.getAdapterType() + " not available");
        }
        adapter.getEncoder().getEncodedData(resource, split.getSecond(), resource
                .getGeometry(), builder, Expression.emptyContext(monitor));
    ...
    public ExecutorService getResourceTaskExecutor() {
        if (resourceTaskExecutor == null) {
            ...
            resourceTaskExecutor = Executors
                    .newFixedThreadPool(Configuration.INSTANCE.getResourceThreadCount());
        }
    ...
    class ImporterImpl implements Importer {
        ...
        public IResource finish() {
            return createLocalResource(this.id, this.file, this.parameters, this.project, this.adapter, true, false, Klab.INSTANCE
                    .getRootMonitor());
        }
    ...
    public void moveLocalResource(IResource resource, IProject destinationProject) {
        ...
        ((Resource) resource).setLocalPath(destinationProject.getName()
                + resource.getLocalPath().substring(originalProject.length()));
