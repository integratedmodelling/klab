...
public class Model extends KimObject implements IModel {
    ...
    private Model(IKimModel model, Namespace namespace, IMonitor monitor) {
        ...
        if (!model.getResourceUrns().isEmpty()) {
            ComputableResource urnResource = validate(new ComputableResource(model.getResourceUrns()
                    .get(0), this.isInstantiator() ? Mode.INSTANTIATION : Mode.RESOLUTION), monitor);
            for (int i = 1; i < model.getResourceUrns().size(); i++) {
                urnResource.chainResource(validate(new ComputableResource(model.getResourceUrns()
                        .get(i), this.isInstantiator() ? Mode.INSTANTIATION : Mode.RESOLUTION), monitor));
            }
            this.resources.add(urnResource);
        } else if (model.getResourceFunction().isPresent()) {
            this.resources.add(validate(new ComputableResource(model.getResourceFunction()
                    .get(), this.isInstantiator() ? Mode.INSTANTIATION : Mode.RESOLUTION), monitor));
    ...
    private ComputableResource validate(ComputableResource resource, IMonitor monitor) {
        ...
        if (resource.getClassification() != null) {
            ...
            IClassification classification = Types.INSTANCE
                    .createClassificationFromMetadata(observables.get(0), resource.getAccordingTo());
