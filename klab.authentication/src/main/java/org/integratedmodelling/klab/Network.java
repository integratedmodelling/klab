...
public enum Network implements INetworkService {
	...
	Map<IIdentity.Type, Map<String, INodeIdentity>> klabServices = Collections.synchronizedMap(new HashMap());
	...
	private boolean isNode(INodeIdentity node) {
		return node != null && ((node.getIdentityType().equals(IIdentity.Type.NODE)
	...
	public void buildNetwork(EngineAuthenticationResponse authorization) {
		...
		for (NodeReference node : authorization.getNodes()) {
			...
			try {
				...
				if (isNode(identity) && Services.INSTANCE.getService(IResourceService.class) != null) {
					Services.INSTANCE.getService(IResourceService.class).getPublicResourceCatalog()
							.updateNode(identity);
				}
			} catch (Exception e) {
		...
		/**
		 * Next-gen services. We only need resources for compatible adapters, but we
		 * keep them all for future reference. No check is done on online status.
		 */
		for (NodeReference node : authorization.getServices()) {
			Node service = new Node(node, authorization.getUserData().getToken());
			this.klabServices.computeIfAbsent(service.getIdentityType(), id -> new HashMap<>()).put(service.getName(),
					service);
		}
	...
	/**
	 * Retrieve a k.LAB 1.0 service descriptor with the passed type and name, or
	 * null. The service may be online or not.
	 * 
	 * @param type
	 * @param name
	 * @return
	 */
	public INodeIdentity getKlabService(IIdentity.Type type, String name) {
		return klabServices.containsKey(type) ? klabServices.get(type).get(name) : null;
	}
		...
		for (INodeIdentity node : online) {
			tasks.add(new Callable<K>() {
				...
				public K call() throws Exception {
					return node.getClient().onBehalfOf(session.getUser()).get(endpoint, individualResponseType,
							urlVariables);
				}
			});
		}
	...
	public <T, K, V> T broadcastPost(String endpoint, V request, Class<? extends K> individualResponseType,
		...
		for (INodeIdentity node : online) {
			tasks.add(new Callable<K>() {
				@Override
				public K call() throws Exception {
					return node.getClient().onBehalfOf(session.getUser()).post(endpoint, request,
		...
		for (INodeIdentity node : offlineNodes.values()) {
			try {
				...
				if (l == null || (System.currentTimeMillis() - l) > (1000 * 60 * 10)) {
					Logging.INSTANCE
							.debug("Exception while checking offline node " + node.getName() + ": " + e.getMessage());
	...
	public INodeIdentity getNodeForResource(Urn urn) {
		...
		return chooseNode(getOnlineNodes(Services.INSTANCE.getService(IResourceService.class).getPublicResourceCatalog()
				.getNodes(urn.getUrn())));
	}
