...
The non-semantic artifact, depending on the used builder, could be promoted to the semantic world. For example, when a model is run, the context passed to the LocalDataBuilder could provide an empty state (IState). The builder would then add information of the artifact to the state. Non-semantic versions of builders exist for special purposes (for example when resources must be used from inside a specialized process that does not need to participate in the semantic resolution) and build "raw" **IArtifact**s that  
====
