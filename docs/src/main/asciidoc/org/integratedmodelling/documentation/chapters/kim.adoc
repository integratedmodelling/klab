...
As introduced before, the k.IM language offers only two main types of statements: _concept definitions_ and _model definitions_. Concept definitions create new concepts, automatically connecting them to the k.LAB core ontology and to concepts defined in other namespaces. Once created, a concept exists in any engine that loads the project: there is no way to make a concept private to a namespace, so care is needed.

Model definitions specify how to produce scientific artifacts (_observations_) of specified concepts. They can list other concepts that must be observed in order to make the observation (_dependencies_) and may process their values by defining equations or linking to internal algorithms. Models can use external _resources_ specified by identifiers (_URNs_) as a source of knowledge: in this case, they provide _semantics_ for their resources by defining the concept they incarnate. We sometimes refer to models that use resources as the _semantic annotations_ of those resources.

Although a k.IM namespace can contain both types of statements, these two classes of statements are normally written by different classes of contributors. Only those users that contribute to worldviews - a collaborative process that requires significant time, expertise and logical skills - normally need to write concept definition statements, which compose _worldviews_, i.e. conceptualizations that cover a broad range of disciplines and allow modelers to annotate data and computations. Because worldviews are shared across all k.LAB users that subscribe to them (based on the preferences stored with their certificates), the majority of k.LAB modelers just refer to the worldview concepts to compose _observables_ to use in models. Still, the worldview and its definition in k.IM remains available for everyone to explore (or propose changes to); understanding the basis of concept definitions in k.IM is useful for all users. More details about community processes that can support building the most useful worldviews will given later.

Within these two categories of statements, considerable syntactic variation is possible. Each statement is always introduced by a keyword, i.e. a reserved word that can only be used with their assigned meaning (for example, you can not name a model "temperature" because that is a keyword in k.IM). Keywords are always lowercase and they often correspond to commonly used English words. The editor incorporated in k.Modeler highlights all keywords in a bold purple color (yes, you can change it), to make them easy to recognize as you type.

The keywords that introduce concept definitions represent commonly used base concepts in scientific observations. These correspond to base types in the k.LAB core observation ontology and establish the basic "real world" nature of the observable concept they introduce. Because these core types are well defined and immutable, there is no *+concept+* keyword in k.IM, and all conceptual definitions will start with a concrete type keyword such as *+agent+*, *+process+*, *+temperature+* or *+uncertainty+*. In comparison, _model_ definitions are simpler: the keyword *+model+* introduces most of them. The few exceptions include _non-semantic models_ (introduced by a data type keyword, one of *+number+*, *+boolean+* or *+text+*) and _learning models_, introduced by *+learn+*.

Other keywords in k.LAB are used to modify or connect concepts (_semantic operators_) and to support the definition of models. These are chosen within the prepositions, adverbs, conjunctions and verbs in the English language, so they rarely conflict with user-defined concept names, which are normally English nouns. To efficiently represent observation activities in science, and in order to ensure easy readability and an intuitive match with the English language, k.IM has a high number of keywords: the following table lists all the keywords recognized in k.IM by their role in the language.
...
To finish, _separators_ (;,) are used to terminate or join statements; parentheses and brackets ({} []) may delimit concepts in _observable expressions_, _code expressions_ and tables; some symbols (\#,&,*,+,-,.,/,:,<,!=,=,>,?,@,BC,AD,E,,^,CE,e,l,|,{{,}},${,#[,==,<=,>=,?=) are recognized as _value operators_ or as part of _unit_, _currency_ _classification_, _date_, _number_ and _lookup table_ syntax.

=== Namespace declarations

Every k.IM file specifies a _namespace_ -- essentially a named collection of statements. The namespace must be introduced by a header that starts with the keyword **+namespace+** (an alternative is *++scenario++*, discussed later). Choosing an appropriate name for the namespace is very important: there cannot be two namespaces with the same name in the system, and because k.LAB is a distributed system, namespace names must be unique across the entire network. For this reason we use the convention of making namespace names _paths_, i.e. names made of different parts connected by dots. Only namespaces that are part of worldviews should be single identifiers: all user namespaces should normally start with the name of the institution or project they work in, followed by one or more names that define the logical scope of their content in order of increasing specificity. If you are operating within an institution that has a domain name and many department, you should name it using two or more pieces, starting with the most general (EXAMPLE). This notation, referred to as _reverse domain name convention_, is adopted widely and the multiple "layers" of increasing specificity make it very unlikely to create conflicing names.

The _general form_ of a namespace declaration can be explained using <> for language elements, [] for optional code segments, and {a | b | ...} for alternative forms. The general form for a concept declaration is as follows:
...
When a concept is defined by writing a definition in a namespace, its final _fully qualified name_ will include the namespace name: so if we define the concept Soil in an `earth` namespace, its fully qualified name will be `earth:Soil`. This name will be recognized in all applications that use the worldview that contains it. Within the same namespace (`earth`) where `Soil` is defined, the concept can (and should) be referenced using only its name.

The general form of a concept declaration is as follows:
