...
Inspecting the scores can be very useful to understand, and possibly change, why the resolver pick each specific model. If the expected model does not appear in the list, it probably has errors or is out of scope of the resolution.

==== The context debugger

Since build 234, k.LAB incorporates a simple _context debugger_ that is particularly useful to inspect values and their change when they are part of large and complex contexts, such as raster space. 
