...
/**
 * Collects ObservationReference and ObservationChange messages to keep graphs describing a
 * context's structure. The graphs are updated by subscribing to events that are sent only when a
 * user calls expand() on a graph.
 * 
 * @author Ferd
 *
 */
public abstract class ContextMonitor {
    ...
    /**
     * The context graph always contains groups for objects below root. The getChildren() call has
     * the option of collapsing the group if the size is 1.
     * <p>
     * Calling expand() will physically insert or remove the child observations and should be done
     * when the tree is expanded. Otherwise the count of children will be up to date but the
     * children won't physically be in the tree.
     * 
     * @author Ferd
     *
     */
    public class ContextGraph extends DefaultDirectedGraph<ObservationReference, DefaultEdge> {
        ...
        /**
         * Expand or collapse an observation's children, subscribing or unsubscribing to any changes
         * that involve the resulting tree.
         * 
         * @param observation
         * @param open
         * @return
        ...
        public List<ObservationReference> getChildren(ObservationReference observation, boolean collapseSingletons) {
            ...
            for (DefaultEdge edge : incomingEdgesOf(observation)) {
                ...
                if (collapseSingletons && child.getObservationType() == ObservationType.GROUP && child.getChildrenCount() == 1) {
    ...
    /**
     * Subscribe or unsubscribe to any change events concerning a node of the tree or its siblings.
     * 
     * @param contextGraph
     * @param observation
     * @param open
    ...
    protected abstract List<ObservationReference> retrieveChildren(ObservationReference observation, int offset, int count);
    ...
    /**
     * Define to retrieve the children of the passed observation from the backend. For now gets them
     * all, which is obviously crazy.
     * 
     * @param observation
     */
    ...
    public void register(ObservationReference observation) {
        ...
        String parentId = observation.getParentArtifactId() == null
