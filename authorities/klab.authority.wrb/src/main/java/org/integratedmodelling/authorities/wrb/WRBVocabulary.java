...
/**
 * Vocabulary for all terms; we don't hardcode anything but fill the hashes in
 * at synchronization using the queries and SPARQL endpoint in
 * {@link QueryBuilder}.
 * 
 * If k.LAB is initialized at synchronization, will maintain the wrb ontology
 * where all the concepts are created. Call {@link #setCoreTypes(String, String,
 * String))} before creating the object to establish the base types for
 * qualifiers, specifiers and reference soil groups. If not called, the base
 * attributes and identities will be used.
 * 
...
public class WRBVocabulary {
	...
	/**
	 * anything that needs to change compared to the "official" id can be added here
	 * with its corrected form.
	 */
	public static Map<String, String> corrections = new HashMap<>();

	// if these are defined before the first use, read the vocabulary from the
	// classpath.
...
//    }
	...
	/**
	 * TODO separate by version and implement get(version)
	 * @return
	 */
	...
	/**
	 * Send a SPARQL query and parse the JSON results according to AllegroGraph's
	 * REST API. Cache any successful query and use the cached result, if present,
	 * to handle any http error before throwing an exception.
	 * 
	...
	public static List<Pair<String, URI>> query(String query, String queryId) {
		...
		template.setMessageConverters(
				Arrays.asList(new HttpMessageConverter[] { new MappingJackson2HttpMessageConverter(),
		...
		Object values = null;
		File cached = Configuration.INSTANCE.getDataPath("wrb");
		...
		try {
			...
			if (values instanceof List) {
				try {
					...
					Logging.INSTANCE.error("error writing cached response for " + queryId);
				}
			}
			...
			if (cached.exists()) {
				...
				Logging.INSTANCE.warn("REST service not available: using cached vocabulary");
				try {
					Map<?, ?> response = objectMapper.readValue(cached, Map.class);
					values = response.get("values");
				} catch (Throwable ex) {
					throw new KlabIOException("error reading cached data for " + queryId);
				}
			} else {
				throw new KlabValidationException(e.getResponseBodyAsString());
			}
		}
			...
			for (Object o : ((List<?>) values)) {
				if (o instanceof List && ((List<?>) o).size() > 1) {
					...
					try {
						...
						throw new KlabValidationException("invalid URI from vocabulary: " + uri);
					}
				}
			}
		}
	...
	private List<Pair<String, URI>> readFromCache(String resource) {
		...
		try (InputStream input = new FileInputStream(
				new File(Configuration.INSTANCE.getDataPath("wrb") + File.separator + resource))) {
			...
			throw new KlabIOException("error reading cached data for " + resource);
		}
	...
	private void synchronize() {
		...
		if (useClasspathResources) {
			try {
				Resources.INSTANCE.extractResourcesFromClasspath("/wrb/**", Configuration.INSTANCE.getDataPath("wrb"));
			} catch (Exception e) {
