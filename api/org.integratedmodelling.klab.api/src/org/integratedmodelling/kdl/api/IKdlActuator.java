...
public interface IKdlActuator extends IKdlStatement {
     ...
     * Alternative to {@link IKdlActuator#getNewObservationUrn()} - if one is not null, the other
     * will be. Both can be null (e.g. in instantiating actuators).
     * 
    ...
    /**
     * Return the URN of the observation that we must connect to at this step, if any. Alternative
     * to {@link IKdlActuator#getNewObservationType()} - if one is not null, the other will be. Both
     * can be null (e.g. in instantiating actuators).
     * 
    ...
    /**
     * Abstract actuators are only used as a base for extensions of others in the same dataflow.
     * 
     * @return
     */
    boolean isAbstract();
    ...
    /**
     * For parameter specifications, this tags an input as an expression whose return value is that
     * of the actuator.
     * 
     * @return
     */
    ...
    /**
     * If true, this actuator is a sub-dataflow to resolve each instance for the instantiator parent
     * (if the name is the same as the parent's) or an attribute instantiated by a classifier filter
     * in the computation.
     * 
     * @return
     */
    ...
    /**
     * Unit specs may be added to import statements. These will be validated only for syntax at this
     * stage.
     * 
     * @return
     */
