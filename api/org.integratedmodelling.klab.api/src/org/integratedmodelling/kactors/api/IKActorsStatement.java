...
public interface IKActorsStatement extends IKActorsCodeStatement {
	...
	public enum Type {
		...
		CONCURRENT_GROUP, SEQUENCE, INSTANTIATION, ASSERT_STATEMENT, ASSERTION
	}
	...
	public interface Assert extends IKActorsStatement {
		...
		/**
		 * Assertions are a chain of calls or an expression with an optional comparison
		 * value.
		 * 
		 * @author Ferd
		 *
		...
		public interface Assertion extends IKActorsStatement {
			...
			/**
			 * Value to compare with (null == 'empty' is a legitimate value producing a
			 * non-null IKActorsValue). If null, we are just asserting the absence of errors
			 * and that something non-null and non-false was returned in case there is a
			 * return value.
			 * 
			 * @return
			 */
	...
	public interface Assignment extends IKActorsStatement {
		...
		/**
		 * If true, 'def' was specified, which results in assignment in the local scope.
		 * Otherwise, 'set' was used, which sets values in the actor and should only be
		 * used in init actions.
		 * 
		 * @return
		 */
		boolean isLocal();
