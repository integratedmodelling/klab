 ...
 * correspondent W3C documents.
 * <p>
 * All k.LAB objects are mapped to the OPM according to the following
 * conventions:
 * <p>
 * All entities are {@link IArtifact}s, typically {@link IObservation}s. A model
 * built by k.LAB (such as in learning) is also an artifact: in this case, the
 * model artifact is its {@link IKimModel syntactic specification}, not a
 * {@link IModel runtime} incarnation of the model.
 * <p>
 * Entities are generated by Activities, which in k.LAB are observation
 * activities and typed according to {@link ObservationType}: they may be
 * acknowledgements, computations (either resolutions or instantiations) or
 * detections.
 * <p>
 * The responsibility for Activities is attributed to Agents, which may be a
 * {@link IUserIdentity} (in acknowledgments) or {@link IModel models}.
 * <p>
 * The models' sequential execution is driven by a {@link IDataflow}, which is
 * an entity of type Plan and attributed to the AI in k.LAB, an agent
 * representing the {@link IEngineIdentity} that built and ran it. All models
 * run within a dataflow are associated with it. One provenance graph collects
 * all dataflows run to compute all the artifacts within the same root context.
 * The opportunity to include nodes for partners and delegation will be explored
 * later.
 * <p>
 ...
 * <p>
 * Any {@link IResource}s used by models (data or computations) come with their
