 ...
 * all models being contextualized. During resolution, any change spawns a child
 * scope that is merged with the parent upon its successful resolution. The
 * resolution contexts compute the total coverage, build the provenance graph
 * and harmonize the merged scale as new models are accepted. Successful
 * resolution means a <em>satisfactory</em> coverage, with rules that can be
 * changed by the user.
 * <p>
...
public interface IResolutionScope {
	...
	/**
	 * Resolution mode is the shallowest level of description for the observation
	 * activity that can resolve an observable. The deeper level (according to
	 * odo:Description) are made explicit in {@link IActivity#Description}.
	 * <p>
	 * The resolution scope knows the mode while the observable knows that and the
	 * remaining semantics.
	 */
	public enum Mode {
		/**
	...
	/**
	 * Scope of resolution. For now only used to report the unchanging original
	 * scope.
	 * 
	 * @author Ferd
	 *
	...
	/**
	 * If this scope is resolving a relationship, it must know the source and target
	 * subject for it.
	 * 
	...
	/**
	 * The scope this resolution was started into. Doesn't change across the
	 * resolution graph.
	 * 
	...
	/**
	 * Return the specific observable that resolves the passed one, or null.
	 * Resolution requires identity of concept but not of name or mediators.
	 * 
	 * @param observable
	 * @param mode
	 * @return
