 ...
 * link {@link org.integratedmodelling.klab.api.data.artifacts.IObjectArtifact}s
 ...
 * <p>
 * The end result of {@link #run(IScale, IMonitor) running a dataflow} in a
 * given scale is a
 * {@link org.integratedmodelling.klab.api.provenance.IArtifact}. In k.LAB, this
 * corresponds to either a
 * {@link org.integratedmodelling.klab.api.observations.IObservation} (the usual
 ...
 * {@link IObservation observations}, i.e. semantic artifacts. But if those
 ...
 * REST calls.
 * <p>
 * A dataflow is the top-level {@link IActuator actuator} of a k.LAB
 * computation. It adds top-level semantics to the actuator's contract. Only a
 * dataflow can be run and serialized from the API.
 * <p>
...
public interface IDataflow<T extends IArtifact> extends IActuator {
	...
	/**
	 * The scale of the resolution from which this dataflow was built. It will never
	 * be null and will at most encompass the context of resolution.
	 * 
	 * @return
	 */
	...
	/**
	 * The applicable scale of the dataflow, which can be much broader than the
	 * scale of resolution. If
	 * {@link org.integratedmodelling.klab.api.resolution.ICoverage#isEmpty() its
	 * coverage is empty}, the dataflow will produce an
	 * {@link org.integratedmodelling.klab.api.provenance.IArtifact#isEmpty() empty
	 * artifact} when run. Otherwise the coverage reflects the applicable scale of
	 * the dataflow, i.e. the range of extents and resolutions where it applies. If
	 * null, the dataflow can be used universally. A resource built for this
	 * dataflow will have a geometry reflecting this coverage.
	 *
	 ...
	 * {@link org.integratedmodelling.klab.api.runtime.IRuntimeProvider} and return
	 * the resulting artifact.
	 *
	 * @param scale   the scale of contextualization. Assumed (and not checked)
	 *                compatible with the scale of the resolution that generated
	 *                this dataflow. The dataflow's perdurants will initially be
	 *				  resolved in the initialization scale.
	 *                TODO the scale should be checked against the coverage and the
	 *                empty artifact should be returned if incompatible.
	 * @param monitor a
	...
	/**
	 * Return the KDL source code for the dataflow. If the dataflow has been read
	 * from a KLD stream, return the original code, otherwise reconstruct it by
	 * decompiling the dataflow.
	 *
