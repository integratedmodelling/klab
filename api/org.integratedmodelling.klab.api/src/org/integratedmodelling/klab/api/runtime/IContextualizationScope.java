...
public interface IContextualizationScope extends IParameters<String> {
	 ...
	 * artifact is null. FIXME should be a folder
	 *
	 * @return the target artifact, an {@link IObservationGroup} when the
	 *         computation is an instantiations.
	 */
	...
	/**
	 * Use to retrieve a state correspondent to a concept when the observable has a
	 * unit, currency or range and a specific other mediator is wanted. The result
	 * state will either be the original state (if the mediator is the same as the
	 * original observable's) or a mediating state that will convert the values in
	 * both get() and set() operations. The mediation will be aware of scale, so it
	 * is OK to ask for units that aggregate or distribute values over space or
	 * time. Asking for a non-null mediator when the state does not admit one, or
	 * the reverse, will raise an exception. Asking for a state that does not exist
	 * in the scope will simply return null.
	 * 
	 * @param concept
	 * @param unit
	 * @return
	...
	/**
	 * Like {@link #getState(IConcept, IValueMediator)} but referencing the state by
	 * name instead of concept.
	 * 
	 * @param name
	 * @param unit
	 * @return
	...
	/**
	 * Get any artifacts that incarnate the passed concept, using transitive
	 * reasoning.
	 * 
	 * @param <T>
	 * @param affected
	...
	/**
	 * Return the identifiers of all states known to the current model, with their
	 * local names in it. If there is no current model, return all state identifiers
	...
	/**
	 * Code can call this at monitorable points; any installed triggers in the
	 * inspector will be activated when matching, if the inspector is armed in the
	 * session and listening. See {@link IInspectors} for what to pass.
	 * 
