...
public interface IObservable extends ISemantic, IResolvable {
	 ...
	 * <p>
	 * NOTE: the builder's method <em>all</em> return the same builder, not a child
	...
	/**
	 * The name is never null and is meant for human and code consumption. It can be
	 * explicitly set using the 'named' k.IM clause, and is always a simple
	 * lowercase identifier. If no 'named' clause is present, a name is computed
	 * similarly to {@link #getReferenceName()} but without using disambiguating
	 * namespaces, therefore not guaranteeing a 1:1 correspondence to the semantics
	 * but with enough predictability to not have to use 'named' in simple
	 * situations to refer to the observable. It's always a lowercase identifier
	 * usable for coding in k.IM, k.DL, and most languages. Even if 'named' is
	 * given, the name may be different from the stated because of disambiguation
	 * when the observable is used in a dataflow.
	 *
	...
	/**
	 * The reference name is the default name and only depends on the contents of
	 * the observable. It is uniquely related to the semantics. It may be modified
	 * for disambiguation in the observables used when creating dataflows. It does
	 * not correspond to {@link #getName()}, which is meant for human consumption.
	 * 
