...
public interface IConcept extends IKnowledge {
	...
	/**
	 * The quick version of the reasoner-based
	 * {@link org.integratedmodelling.klab.api.knowledge.IKnowledge#is(ISemantic)}
	 ...
	 * reasoner if installed, and only follow transitive superclass relationships if
	 * not.
	 *
	...
	/**
	 * Return the (only) parent class, or throw an unchecked exception if there's
	 ...
	 *
	 * This is used quite a bit in kbox queries, so it pays to make it fast and/or
	 * memoize results.
	 *
	...
	/**
	 * Return min,max cardinality of property when applied to this concept. -1 on
	 * either end indicates no cardinality given.
	 *
	...
	/**
	 * Called on an abstract concept to retrieve all the children that are concrete
	 * and disjoint, meaning those for which is(this) is true but both a.is(b) and
	 * b.is(a) is false.
	 *
	...
	/**
	 * Return the concept's canonical definition in terms of worldview concepts.
	 * This will correspond to the fully qualified concept name for concepts that
	 * have been declared directly in the worldview, or to their normalized k.IM
	 * declaration when the concept has been created by composing other concepts.
	 * Concepts that are not primary carry their definition in an annotation
	 * property. A concept definition can always be turned into the correspondent
	 * concept as long as the same worldview is loaded.
	 *
	...
	/**
	 * If the concept is {@link Type#UNION} or {@link Type#INTERSECTION}, this will
	 * return the list of operands. Otherwise it will return a list containing only
	 * the concept itself.
	 * 
	 ...
	 * terms of being able to use it to resolve an observation of this. The most
	 * direct resolution (concepts are equal wrt resolution) causes a 0 return
	 * value. Any higher value defines a semantic distance between two compatible
	 * concepts. Incompatible concepts return negative values.
	 * 
	 * The distance measure should take into account whether the distance comes from
	 * indirect observable, context, traits etc. with appropriate weights. Roles
	 * don't enter the resolution calculation.
	 * 
	 * @param concept
	 * @return
	 */
