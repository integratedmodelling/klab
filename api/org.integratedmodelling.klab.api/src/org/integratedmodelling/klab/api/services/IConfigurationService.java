...
public interface IConfigurationService {
    ...
    /**
     * The logging level (linked to property klab.logging.level, default ERROR) controls which
     * notifications sent to {@link IMonitor monitors} are also logged.
     * 
    ...
    /**
     * The notification level (linked to property klab.logging.level, default INFO) controls which
     * notifications sent to {@link IMonitor monitors} are sent to subscribers.
     * 
     * @return the level of logging for monitor notifications.
     */
    ...
    /**
     * Return the proportion of error (0-1) allowed when subsets of grids are created during scale
     * mediation. If the error in a subsetting operation is higher than what is returned, a much
     * more expensive non-conformant grid mediator will be used instead of snapping the subgrid to
     * the original one for 1-to-1 rescaling.
     * 
    ...
    /**
     * If true, each instance created by instantiatiors will be resolved independently using the
     * k.LAB network. This may make for more accurate resolution when instances and context have
     * widely different scales, but will use much more time and resources if the instances are many.
     ...
     * {{@link #getProperties()}, this one shold look first if the property has been defined through
     * a Java system variable (-Dppp=vvv) and if so, returns that value independent of what the
