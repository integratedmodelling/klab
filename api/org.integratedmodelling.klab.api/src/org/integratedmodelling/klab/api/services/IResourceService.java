...
/**
 * The <code>IResourceService</code> service provides access to all URN-identified non-semantic
 * assets. It also bridges to read-only semantic assets such as observations and concepts when
 * identified through URNs.
 *
...
public interface IResourceService {
    ...
    /**
     * Get a calculator for the passed resource, or null if the resource adapter can't make one.
     * 
     * @param resource
     * @return
     */
     ...
     * resources are created by the {@link IResourceValidator validator} of an
     * {@link IResourceAdapter adapter}, and must be published before they can be shared. A project
     * containing local URNs cannot be published on a k.LAB node.
     *
    ...
    /**
     * Resolve a resource with the option of passing a local URN with just the local name and a
     * target project to look into. This enables resolution of project-local resources using only
     * the local name as URN, which for file-based resources will just be the file name.
     * 
     * @param urn
     * @param project
     * @return
    ...
    /**
     * Low-level data retrieval using a URN and a geometry. The result is set in the returned
     * IKlabData and won't have any semantics.
     * 
     * @param urn
     * @param geometry
     * @param monitor
     ...
     * resolvable to a correspondent resolvable when possible.
     *
     * @param urn either a formal URN or one of the abbreviated forms recognized in k.IM (such as a
     *        concept identifier)
     * @param scale scale of resolution, used to attribute proper default units to extensive
     *        observables when they are created from concepts.
    ...
    /**
     * Resolve the URN and return true if the resolution is successful and the resource is online,
     * false otherwise.
     * 
     * @param urn
    ...
    /**
     * Submit a resource for publication to the node identified by nodeId, which must be an online
     * node on the network. Return an open ticket that will be closed when publication is finished.
     * 
     * @param resource
     * @param nodeId
     * @param publicationData any user suggestions about name, namespace, catalog and permissions.
    ...
    /**
     * Return the directory where the resource is located on the filesystem. This will differ for
     * local (project-based) and public resources (in nodes). Universal resources (klab:....) have
     * no file location and will return null.
     * 
     * @param resource
     * @return
     */
    ...
    /**
     * Create a resource containing the current dataflow in the passed scope, which must be a
     * runtime scope created by resolution.
     * 
     * @param scope
     * @param resourceId
     * @param project
    ...
    /**
     * Localize a resource so that it can properly be used in the passed context. This may return
     * the same resource or a rescaled/filtered subset. The resulting resource will be inserted in
     * the provenance graph for the applicable scale.
     * 
     ...
     * 
     * @param resource
     * @param observation the contextualized artifact (including semantics if an instance of
     ...
     * @param scale the scale of contextualization (normally that of the scope)
     ...
     */
    IResource contextualizeResource(IResource resource, Map<String, String> urnParameters, IScale scale, IArtifact observation,
