...
public interface ITime extends IExtent {
	...
	/**
	 * May be null in partially specified extents. Returns the timestep BEYOND the
	 * end of computation - the one that the time reaches, not the one it computes
	 * last. The latter is returned by getLast().
	 *
	 * @return end time
	 */
	ITimeInstant getEnd();
	...
	/**
	 * Time extents record the actual changes in the observations they describe.
	 * This method checks if there have been one or more changes in the artifact's
	 * state during the time passed.
	 * 
	 * @param time
	 * @return
	 */
