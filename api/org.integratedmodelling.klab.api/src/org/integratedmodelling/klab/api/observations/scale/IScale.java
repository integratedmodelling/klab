...
public interface IScale extends ILocator, IGeometry, ITopology<IScale> {
	...
	/**
	 * Merge in another scale to return a new scale that best represents the common
	 * traits in both, seeing the passed scale as a constraint. Used to build the
	 * "common" scale of a dataflow before contextualization, where the passed scale
	 * is that of the desired context and this is the scale of a model or
	 * computation used in it.
	 * 
	 ...
	 */
	@Override
	IScale at(Object... dimensions);

	/**
	 * Return whether the passed dimensions are regular, i.e. the extent in each
	 * sub-extent is the same. If no dimensions are passed, check them all. If a
	 * non-existent dimension is passed, return true for it.
	 * 
	 * @param dimensions
	 * @return
	 */
	boolean isRegular(IGeometry.Dimension.Type... dimensions);
