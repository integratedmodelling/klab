 ...
 * compatible scale. Models that provide semantics for a resource inherit its
 * coverage through the resource's geometry.
 * <p>
 * Resolution of a URN (operated by the configured {@link IResourceService}
 * returns a IResource, which can be coupled with a {@link IGeometry} to yield
 * an immutable {@link IKlabData artifact} that a semantic {@link IModel} can
 * process into an {@link IObservation}. The machinery that enables a resource's
 * contextualization is provided through the runtime wrapper
 * {@link IContextualizable}, which also wraps any other type of information
 * source, including literal values and plug-in contextualizers.
 * <p>
 * Since 2021, resources can also contain {@link ICodelist}s, which can be
 * referenced directly and reused from other resources. These can be used to
 * translate values provided by services or computations into their production
 * values at contextualization.
 *
 * @author Ferd
 * @version $Id: $Id
 */
