...
public interface IResourceImporter {
	...
	/**
	 * Return whether multiple resource URLs can be accepted to build a single
	 * resource (e.g. to build collections of timeslices). If this returns true, the
	 * importer should implement {@link IMultipleResourceImporter}.
	 * 
	 ...
	 * unrecognized, an empty collection should be returned without error. User data
	 * can include a <code>regex</code> entry that permit filter resources by name
	 * 
	...
	/**
	 * Return a list of triples ID/Description/FileExtension for all the formats of
	 * file export that this adapter supports for the passed observation. The first
	 * triple will be interpreted as the default export format.
	 * 
	 * @param observation
	 * @return
	 */
	 ...
	 * format will always be one of those returned by
	 * {@link #getExportCapabilities(IObservation)} for the same observation.
	 * 
	 * @param file
	 * @param observation
	 * @param locator     TODO
	 * @param format
	 * @param monitor     TODO
	 * @return the file if successful, null otherwise. The file should be reassigned
	 *         as some output formats may require to output an archive or another
	 *         file different from the requested. Should only throw exceptions when
	 *         the file can't be written or any parameters are null or invalid.
	 */
	File exportObservation(File file, IObservation observation, ILocator locator, String format, IMonitor monitor);

	/**
	 * Return a map of ID/Description for all the formats of file export that this
	 * adapter supports for the passed resource. Pass a null resource to obtain
	 * export formats that are valid for any (viable) resource for this adapter.
	 * 
	 * @param observation or null. If null, this method must return all universal
	 *                    export formats.
	 * @return
	 */
	 ...
	 * @param observation
	 * @param format
	 * @return true if successful. Should only throw exceptions when the file can't
	 *         be written or any parameters are null or invalid.
	 */
	boolean exportResource(File file, IResource resource, String format);

	/**
	 * When appropriate, write the passed observation (which comes from a resource
	 * that uses this adapter) to a character stream using the passed writer.
	 * 
	 * @param writer
	 * @param observation
	 * @param locator
