...
public interface API {
	...
	/**
	 * Streamlined k.LAB 1.0 services API for those services we can use here.
	 */
	interface KLAB_1 {

		String API_1_BASE = "/api/v1";

		/**
		 * Values for the knowledgeClass parameters. Do not change without coordinating
		 * with k.LAB 1.0
		 */
		enum KnowledgeClass {
			CONCEPT, OBSERVABLE, MODEL, DEFINITION, RESOURCE, NAMESPACE, BEHAVIOR, SCRIPT, TESTCASE, APPLICATION,
			ONTOLOGY, OBSERVATION_STRATEGY, OBSERVATION_STRATEGY_DOCUMENT, COMPONENT, PROJECT, WORLDVIEW, WORKSPACE,
			CONCEPT_STATEMENT, OBSERVATION;
		}

		interface RESOURCES {

			/**
			 * The RESOLVE endpoint always returns a ResourceSet with the full dependency
			 * closure for the intended asset. The URN may also be a comma-separated list of
			 * URNs. Returns a ResourceSet, not available here.
			 */
			String RESOLVE = API_1_BASE + "/resolve/{knowledgeClass}/{urn}";

			/**
			 * DELETE endpoint with obvious usage.
			 */
			String DELETE = API_1_BASE + "/delete/{knowledgeClass}/{urn}";

			/** PUT endpoint to ingest an asset for addition, update or replacement */
			String SUBMIT = API_1_BASE + "/submit/{knowledgeClass}/{submissionMode}/{urn}";

			/**
			 * GET endpoint to retrieve a list of assets of a given type. A POST endpoint
			 * may include a query as payload.
			 */
			String LIST = API_1_BASE + "/list/{knowledgeClass}";

			/**
			 * RETRIEVE endpoints are GET endpoints that return the full asset definition
			 * for the passed URN. In k.LAB 0.11 cross-communication the values should be
			 * ingested as {@link Map}s and converted.
			 */
			String RETRIEVE = API_1_BASE + "/retrieve/{knowledgeClass}/{urn}";
		}
	...
	/**
	 * STOMP endpoint for client/server notifications to session receivers. Handled
	 * through Websockets protocol.
	 * 
	 * <br/>
	 ...
	 * Engines also have ticket management but only local Websockets clients can
	 * access it.
	 * 
	 * @author Ferd
	 *
	...
	public static interface HUB {
		...
		public static interface PARAMETERS {
			...
			/**
			 * URL PARAMETER for user role entry service, lists all the users with a
			 * particular role
			 */
	...
	public interface PUBLIC {
		...
		public enum Export {
			...
			STRUCTURE,
			/**
			 * Data can be JSON (e.g. GeoJSON for geo observation groups), TIFF for raster
			 * data, PNG for rendered images (requires viewport parameter), CSV or Excel for
			 * tabular views.
			 */
			DATA,
			/**
			...
			LEGEND,
			/**
			 * Report can be PDF or MS Word. If the view parameter is specified, only the
			 * corresponding
			 * {@link: org.integratedmodelling.klab.api.documentation.IReport.View} will be
			 * returned.
			 */
			REPORT,
			/**
			 * Dataflow and provenance can be text (k.DL source), JSON (Elk diagram), or PNG
			 * (image, requires viewport parameter)
			 */
			DATAFLOW,
			/**
			 * Full provenance includes any provenance records inherited from resources plus
			 * process, agent, tasks and plan nodes. Same Accept headers as dataflow, if
			 * text is requested the output will be in k.IM entities with RDF mappings.
			 */
			PROVENANCE_FULL,
			/**
			 * Simplified provenance only includes artifacts with scale-annotated
			 * wasDerivedBy relationships.
			 */
		 ...
		 * Observation path variable should contain the context ID for those request
		 * that apply to the entire context, like report, dataflows etc. The Accept
		 * header selects the format, which must be appropriate for the content
		 * requested.
		 * <p>
