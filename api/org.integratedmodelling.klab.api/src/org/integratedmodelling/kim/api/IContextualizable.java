...
/**
 * A contextualizable is the declaration of a resource that can be compiled into a processing step
 * for a dataflow. In k.IM this can represent:
 * <p>
 * <ul>
 ...
 * <li>a conversion between a source and a target value semantics (e.g. unit or currency)</li>
 * </ul>
 * <p>
 * Contextualizables have an artifact type and a declared geometry which determines which phases of
 * a dataflow they apply to.
 * <p>
 * It is the runtime's task to turn any computable resource into a uniform k.DL service call. The
...
public interface IContextualizable extends IKimStatement, IProvenance.Node {
    ...
    public static class InteractiveParameter {
        ...
        public String toString() {
            return "InteractiveParameter [id=" + id + ", functionId=" + functionId + ", description=" + description + ", label="
                    + label + ", type=" + type + ", initialValue=" + initialValue + ", values=" + values + "]";
        }
    ...
    /**
     * Contextualizables carry the trigger that they were declared with. Those that represent
     * "default" computables for a model, such as resources, will report
     * 
     * 
     * @return
     */
    ...
    /**
     * This should produce a resource through the resource service and host it internally for
     * successive calls. Called only if {@link #getType()} is resource and {@link #getUrn()} is not
     * null. The implementation must take care of contextualizing resources appropriately, including
     * merged resource sets.
     * 
     * @return
     */
    ...
    /**
     * Give the resource a chance to adjust itself to the passed scope and target. If nothing is
     * needed, return self. This is called by the runtime before every use of the resource in an
     * actuator, and the result is notified to the contextualizer that will use it.
     * 
     * @param target
     * @param scope
     ...
     * means "send this artifact through this mediator".
     * <p>
    ...
    /**
     * A lookup table translating the inputs. Only one among getLiteral(), getServiceCall(),
     * getUrn(), getClassification(), getAccordingTo(), getLookupTable() and getExpression() will
    ...
    /**
     * An implicit classification built by matching values of an annotation property to subclasses
     * of the observable. Only one among getLiteral(), getServiceCall(), getUrn(),
     * getClassification(), getAccordingTo(), getLookupTable() and getExpression() will return a
     * non-null value.
     * 
    ...
    /**
     * A URN specifying a remote computation. Only one among getLiteral(), getServiceCall(),
     * getUrn() and getExpression() will return a non-null value.
     * 
     * @return the urn
     */
    String getUrn();
     ...
     * must be satisfied within the model where the computation appears. These will be made
     * available in appropriate form (scalar or not) by the runtime environment.
     * 
    ...
    /**
     * Add the resolution mode from the originating model to disambiguate resources that can be used
     * in more than one.
     * 
     * @return
     */
    ...
    /**
     * Only meaningful if this computable is a condition computing a (scalar or distributed)
     * boolean, this specifies whether this condition was given with negative ('unless' instead of
     * 'if') semantics.
     * 
    ...
    /**
     * This will return the geometry incarnated by the computable. It should normally return a
     * scalar geometry except for resources and services.
     * 
     * @return the geometry
     */
    ...
    /**
     * If true, this defines an accessory variable rather than a dependency. The targetId is the
