...
/**
 * A monitor represents the context of computation and is used to establish the current
 * identity as well as to send messages to any subscribers. Monitors may be root-level, 
 * engine-level, session-level, context-level or task-level.
 * <p>
 * The API meant to operate within tasks normally gets a monitor as a parameter in each
 * call. If not, the root monitor can be {@link IRuntimeService#getRootMonitor() obtained from the runtime}.
 * <p>
 * An important function of the monitor is to obtain the current identity that owns the computation. This
 * is done through {@link #getIdentity()}. From that, any other identity (such as the network session, the 
 * engine etc., up to the node and partner that owns the engine) can be obtained.
 * <p>
...
public interface IMonitor {
     ...
     * of these.
     *
     * In addition, you can pass a statement to communicate errors in k.IM, or other objects that can
     * be sent and used as necessary.
     *
     * @param o a {@link java.lang.Object} object.
     */
    void error(Object... o);
